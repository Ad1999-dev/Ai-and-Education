...
- The function builds frequency tables for character sequences of lengths 1 through n (unigrams to n-grams) from a training document. It slides a window over the text and records how often each sequence appears, storing these counts in separate tables for each n-gram size. These frequency tables form the backbone of the n-gram language model, allowing the system to estimate the probability of a character appearing given its preceding characters, which is essential for predicting the next character in sentence autocompletion.
...
### Formula
- P(char∣sequence)= f(sequence + char) / f(sequence)
​

### Code analysis

- The function estimates the probability of a given character appearing next in a sequence using n-gram frequency tables. The intuition is to use the maximum available context (i.e., the longest matching sequence found in the tables) to compute the conditional probability: P(char∣sequence)= f(sequence + char) / f(sequence)
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - *$P(X_1 = a) * P(X_2 = a | X_1 = a) *  P(X_3 = a | X_1 = a, X_2 = a)$*
   => 11/20 * 5/10 * 1/4 = 0.06875

2. $P(X_1=a, X_2=a, X_3=b)$
   - *$P(X_1 = a) * P(X_2 = a | X_1 = a) *  P(X_3 = b | X_1 = a, X_2 = a)$*
   => 11/20 * 5/10 * 1/4 = 0.06875

3. $P(X_1=a, X_2=a, X_3=c)$
   - *$P(X_1 = a) * P(X_2 = a | X_1 = a) *  P(X_3 = c | X_1 = a, X_2 = a)$*
   => 11/20 * 5/10 * 2/4 = 0.1375
...
- The function is responsible for predicting the most likely next character given a sequence of characters, using previously constructed n-gram frequency tables. The intuition is to estimate the probability of each possible next character (from the vocabulary) given the current sequence, and then return the character with the highest conditional probability. For each candidate character, it calls calculate_probability(sequence, char, tables) to compute: 
P (char | sequence)
It trims the input sequence to match the model’s maximum context length (n - 1) and evaluates each character in the vocabulary. The one with the highest probability is selected as the best guess — effectively implementing the core of a character-level autocomplete model.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - *Predicted next character after 'aa': b*
 
## Experiment
- I experimented with two corpora — War and Peace and Alice's Adventures in Wonderland — across n-gram sizes from 2 to 5. As expected, Alice allowed significantly faster table generation due to its smaller size, with build times remaining under 0.05 seconds even for n = 5. War and Peace, being much larger, showed increasing build times with higher values of n (up to ~2.85 seconds for n = 5). Interestingly, both corpora consistently predicted the same next character for the sequence "the", showing that common English patterns are robustly captured even at lower n. Overall, Alice is ideal for fast iteration, while War and Peace provides richer context for more detailed predictions — with n = 3 or 4 offering the best trade-off between speed and model depth.
<hr>

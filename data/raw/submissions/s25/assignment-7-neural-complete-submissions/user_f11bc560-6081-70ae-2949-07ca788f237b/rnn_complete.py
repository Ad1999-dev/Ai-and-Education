...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        self.W_ih = nn.Parameter(torch.randn(hidden_size, embedding_dim) * 0.01)
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            h_t = torch.tanh(torch.matmul(input_t, self.W_ih.t()) + torch.matmul(h_t_minus_1, self.W_hh.t()) + self.b_h)
            logit_t = torch.matmul(h_t, self.W_ho.t()) + self.b_o
            
            output.append(logit_t)
        ...
        logits = output # [b, l, vocab_size=v] 
...
for epoch in range(num_epochs):
    ...
    for batch_inputs, batch_targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        loss = criterion(output.reshape(-1, output.size(-1)), batch_targets.reshape(-1))
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    input_sequence = start_text[-n:]
    input_indices = [char_to_idx[c] for c in input_sequence]
    ...
    for _ in range(k):
        with torch.no_grad():
            logits, hidden = model(input_tensor, hidden)  # logits: [1, n, vocab_size]
            hidden = hidden.detach()

            last_logits = logits[:, -1, :]  # Get logits of the last time step
            sampled_idx = sample_from_output(last_logits, temperature).item()  # Scalar int

            generated_char = idx_to_char[sampled_idx]
            generated_text += generated_char

            # Prepare next input (append sampled index and trim to last n)
            input_indices.append(sampled_idx)

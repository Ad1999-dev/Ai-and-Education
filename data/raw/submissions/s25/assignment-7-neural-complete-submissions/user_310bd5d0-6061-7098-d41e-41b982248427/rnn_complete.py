...
#data = data[:800000]
#print(f"Using a subset of {len(data)} characters from dataset")
...
for epoch in range(num_epochs):
    ...
    for batch_inputs, batch_targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        # Reshape the outputs for the loss function
        b, l, v = output.shape  # batch, sequence length, vocab size
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    model.eval()
    input_indices = [char_to_idx.get(char, 0) for char in start_text]  # Default to first char if not found
    ...
    with torch.no_grad():
        for _ in range(k):
            ...
            input_indices = input_indices[1:]  # Remove first element to keep length the same
            input_tensor = torch.tensor([input_indices], dtype=torch.long).to(device)

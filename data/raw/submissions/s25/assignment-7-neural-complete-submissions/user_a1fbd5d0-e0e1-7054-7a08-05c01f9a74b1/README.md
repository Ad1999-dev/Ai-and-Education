...
1. Describe your experiments and observations
  For this assignment, I initially trained a Recurrent Neural Network (RNN) to generate text at the character level. The first step involved using a simple dataset: the alphabet sequence ("abcdefghijklmnopqrstuvwxyz") repeated 100 times. The objective was to assess how well the RNN could learn and predict characters from a repetitive, easy-to-memorize sequence. After confirming the model's ability to predict the alphabet correctly, I moved on to a more complex and challenging dataset: War and Peace by Leo Tolstoy. This transition allowed me to explore how well the RNN could handle natural language with varying vocabulary and sentence structure. The complexity of War and Peace offered a valuable opportunity to understand how well an RNN trained on simple data could generalize to more sophisticated text generation tasks.

2. Analysis on final train and test loss for both datasets
  When training on the simple alphabet sequence, both the training and test losses were very low, close to zero. This outcome was expected because the task was relatively simple: memorizing and predicting a repetitive sequence. The model quickly learned the pattern, and as a result, it performed very well, both on the training and test sets. However, the scenario was different when training the model on War and Peace. The training loss for this dataset was higher, around 1.5, which is indicative of the model's struggle to learn the complex relationships between characters in the more intricate sentences and vocabulary. The test loss was similarly around 1.5, which suggested that the model was overfitting to the training data to some extent. While the model managed to generate some plausible patterns of text, it failed to generate fully coherent or meaningful sentences.

3. Explain impact of changing temperature
  The temperature parameter has a significant impact on the diversity and randomness of the text generated by the model. When I set a low temperature (e.g., 0.5), the model produced more deterministic and predictable text, which was very useful when generating a repetitive alphabet sequence. The text output under low temperature was highly coherent but lacked creativity. On the other hand, increasing the temperature (e.g., 1.5) introduced more randomness into the model’s predictions, leading to more creative and diverse outputs. While this helped in generating text that was not overly repetitive, it also resulted in incoherent or nonsensical characters being generated, particularly in the context of complex text like War and Peace. A temperature of 1.0, which is the default, provided a balance between predictability and creativity, allowing the model to generate text that was both varied and reasonably coherent, making it suitable for text generation tasks.
4. Reflection
  This project allowed me to better understand how Recurrent Neural Networks (RNNs) work, especially in the context of sequence modeling. RNNs are well-suited for sequential data because they maintain an internal state, or memory, of previous time steps. However, the experiments also highlighted some limitations, especially when the model was tasked with learning long-range dependencies in more complex datasets like War and Peace. The RNN struggled with capturing these long-term dependencies, which is a known issue in vanilla RNNs. This experience made me realize that architectures such as LSTMs or GRUs, which are designed to address this issue, would likely perform better on such tasks. Additionally, I gained insights into the importance of hyperparameter tuning, especially when working with real-world datasets. The learning rate, hidden size, and sequence length all played a crucial role in training performance. Overall, this assignment reinforced the importance of carefully selecting model architectures and tuning hyperparameters to optimize performance, particularly for complex sequence generation tasks.

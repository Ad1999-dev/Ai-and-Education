...
- **Probability:** Basic understanding of probability fundamentals (particularly joint distributions and random variales).
...
- This code iterates through n levels of n-grams. For each level i, it extracts every substring of length i + 1 from the document. We count how often each substring appears and store these counts in a dictionary. We return a list of n such dictionaries.
...
2. ***Write down your probability table 3***:
   - You got this!
        ...
        | $P(a \mid aa)$         | $\frac{1}{5}$     |
        | $P(c \mid aa)$         | $\frac{2}{5}$     |
        | $P(a \mid baa)$        | $\frac{1}{2}$     |

## `calculate_probability(sequence, char, tables)`

### Formula
- $\frac{\text{frequence of sequence + char}}{\text{frequence sequence}}$


### Code analysis

- The code determines the n-gram order by the length of sequence + char. If the order exceeds n, truncates the sequence to fit. It retrieves counts for the sequence + char and sequence and returns the difference.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - \[
        ...
        P(a \mid a) &= \frac{4}{12} \\
        P(a \mid aa) &= \frac{1}{4} \\
        P(a, a, a) &= \frac{11}{20} \cdot \frac{5}{12} \cdot \frac{1}{4} = \frac{11}{192} 
        \end{aligned}
    \]
2. $P(X_1=a, X_2=a, X_3=b)$
   - \[
        ...
        P(a \mid a) &= \frac{4}{12} \\
        P(b \mid aa) &= \frac{1}{5} \\
        P(a, a, a) &= \frac{11}{20} \cdot \frac{5}{12} \cdot \frac{2}{5} = \frac{11}{240}
        \end{aligned}
    \]
3. $P(X_1=a, X_2=a, X_3=c)$
   - \[
        ...
        P(a \mid a) &= \frac{4}{12} \\
        P(b \mid aa) &= \frac{2}{5} \\
        P(a, a, a) &= \frac{11}{20} \cdot \frac{5}{12} \cdot \frac{2}{5} = \frac{11}{120}
        \end{aligned}
    \]
...
- The code iterates through all possible characters in the vocabulary, calculates the probability of each and returns the character with the highest probability.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - c should be the next character in the sequence. 
...
The program worked way better for warandpeace than Alice's Adventures in Wonderland mostly because its a lot shorter so there's less words to process when calling create_frequency_tables. I tried these values of n: 100, 50, 25, 40 and 30 for Alice's Adventures in Wonderland and think it worked in a pretty short amount of time up to n = 25. It still runs in a reasonable amount of time up to around 100 but isn't super fast. 

...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    for _ in range(k):
        input_seq = input_seq[:, -n:]  # keep it fixed length
        logits, hidden = model(input_seq.to(device), hidden)
        last_logits = logits[:, -1, :]  # last character output
        next_char_idx = sample_from_output(last_logits, temperature)
        next_char = idx_to_char[next_char_idx.item()]
        generated += next_char
        input_seq = torch.cat([input_seq, next_char_idx.T], dim=1)

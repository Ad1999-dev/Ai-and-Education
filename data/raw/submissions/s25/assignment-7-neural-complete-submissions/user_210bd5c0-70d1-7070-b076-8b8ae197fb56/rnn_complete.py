...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        self.W_e = nn.Parameter(torch.randn(embedding_dim, input_size)* 0.01)
    ...
    def forward(self, x, hidden):
        ...
        output = []
        for t in range(l):
            # h_t = f(W_{h}h_{t-1}+W_{e}e_t)
            h_t = torch.tanh(torch.matmul(x_embed[t], self.W_e) + torch.matmul(h_t_minus_1, self.W_h) + self.b_h)
            output.append(h_t)
            h_t_minus_1 = h_t
...
test_set = CharDataset(test_data, sequence_length, stride, output_size)
test_loader = DataLoader(test_set, batch_size, False, drop_last=True)
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    for char in start_text:
        input_sequence.append(char_to_idx[char])
    input_tensor = torch.tensor(input_sequence, torch.long)
    
    for i in range(k):
        with torch.no_grad():
            logits,hidden = model(input_tensor,hidden)
            last_logits = logits[0, -1, :]
            i_plus_one = sample_from_output(last_logits.unsqueeze(0), temperature=temperature)
            next_char = idx_to_char[i_plus_one.item()]
            result += next_char
            input_sequence = input_sequence[1: ] + [i_plus_one.item()]
            input_tensor = torch.tensor(input_sequence, torch.long).unsqueeze(0).to(device)

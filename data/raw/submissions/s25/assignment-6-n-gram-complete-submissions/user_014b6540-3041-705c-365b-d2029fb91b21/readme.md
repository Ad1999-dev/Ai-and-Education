...
- ***Put the intuition of your code here***
    The create frequency table is a function that makes a list for n-gram models which captures how often characters follow a given sequence of characters in the document. It makes an empty list and for each value from 1 to n and creates a frequency table. It then iterates through the document and updates the count of characters. And after construction the frequency table and then returns the final list.
...
2. ***Write down your probability table 3***:
   - You got this!
        | $P(\odot)$ | Probability value |  
        | ------ | ----------------- |
        | $P(a \mid aa)$ | $\frac{6}{10}$ |
        | $P(b \mid aa)$ | $\frac{2}{10}$ |
        | $P(c \mid aa)$ | $\frac{2}{10}$ |
        | $P(a \mid ab)$ | $\frac{2}{3}$ |
        | $P(b \mid ab)$ | $0$ |
        | $P(c \mid ab)$ | $\frac{1}{3}$ |
        | $P(a \mid ba)$ | $\frac{2}{3}$ |
...
$$
P(X_1, X_2, X_3, X_4) = \frac{f(X_1)}{size(C)} \cdot \frac{f(X_2, X_1)}{f(X_1)} \cdot \frac{f(X_3, X_1, X_2)}{f(X_2, X_1)} \cdot \frac{f(X_4, X_2, X_3)}{f(X_2, X_3)}
$$
### Code analysis

- ***Put the intuition of your code here***

The caluclate_probability returns the probability of the character 'char' occuring after the given sequence. It checks the length of the sequence and if it is empty it returns 0, if not 0 it then calculates maximum context length that can be used based on the frequency table. Then iterates possible lengths and gets the last character as context and using this context it retrieves the probability of the character.
...
The predict next char is used ot preduct the most likely next character in the sequence. It returns the highest probability of occuring next. It initiazlies max probabilty and predicted char and iterates through each character in the provided vocab. For each character it calculates probability and if it returns a higher probabilty it updates to this character and it returns the predicted character.

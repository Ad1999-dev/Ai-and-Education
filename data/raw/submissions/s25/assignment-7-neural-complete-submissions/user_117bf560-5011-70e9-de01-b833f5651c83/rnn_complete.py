...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        self.W_e = nn.Parameter(torch.randn(embedding_dim, hidden_size) * 0.01)  # Embedding-to-hidden weights
        self.W_h = nn.Parameter(torch.randn(hidden_size, hidden_size) * 0.01)     # Hidden-to-hidden weights
        self.b_h = nn.Parameter(torch.zeros(hidden_size))                          # Hidden biases

        #  
        self.W_out = nn.Parameter(torch.randn(hidden_size, output_size) * 0.01)  # Hidden-to-output weights
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            x_t = x_embed[t]
            h_t = torch.tanh(torch.matmul(x_t, self.W_e) + torch.matmul(h_t_minus_1, self.W_h) + self.b_h)
            output.append(h_t)
            h_t_minus_1 = h_t
        ...
        logits = torch.matmul(output, self.W_out) + self.b_out  # [b, l, vocab_size=v] 
...
hidden_size = 15        # Number of features in the hidden state of the RNN
...
for epoch in range(num_epochs):
    ...
    for batch_inputs, batch_targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        loss = criterion(output.view(-1, output_size), batch_targets.view(-1))  # Compute loss between predictions and actual labels
...
with torch.no_grad():  # Disable gradient calculation
    for batch_inputs, batch_targets in tqdm(test_loader, desc="Testing"):
            ...
            # Outputs are logits, we need to apply softmax to get probabilities for CrossEntropyLoss
            _, predicted = torch.max(outputs, dim=2)  # Get the index of the max log-probability
            total += batch_targets.size(0) * batch_targets.size(1)  # Update total count
            correct += (predicted.view(-1) == batch_targets.view(-1)).sum().item()  # Update correct count
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    idx_start_text = [char_to_idx[char] for char in start_text if char in char_to_idx]
    
    input_tensor = torch.tensor(idx_start_text, dtype=torch.long).unsqueeze(0).to(device)
    
    model.eval()
    hidden = None
    generated_indices = idx_start_text[:]  # Make a copy to store generated indices

    for i in range(k):
        with torch.no_grad():  # No gradients needed for inference
            output, hidden = model(input_tensor, hidden)  # Feed input tensor to model

        # Sample the next character index from the output logits
        next_idx = sample_from_output(output[:, -1, :], temperature).item() 

        # Append the sampled index to the generated indices
        generated_indices.append(next_idx)  # .item() to convert tensor to scalar integer

        # Prepare the input tensor for the next iteration (keep the last `n` characters)
        input_tensor = torch.tensor(generated_indices[-n:], dtype=torch.long).unsqueeze(0).to(device)

    # Convert generated indices back to characters to form the generated text
    generated_text = ''.join(idx_to_char[idx] for idx in generated_indices[n:])  # Concatenate characters

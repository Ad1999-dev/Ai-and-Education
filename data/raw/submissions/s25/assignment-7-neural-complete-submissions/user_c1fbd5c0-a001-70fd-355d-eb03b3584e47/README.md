...
For the alphabet sequence, the model learned quickly, as the vocabulary was short and the patterns were predictable. Training converged rapidly with minimal losses, and the generated output was nearly perfect. War and Peace was much more difficult to deal with, due to the large vocabularly and complicated text. Training was slower and required more tuning. A low stride worked for the alphabet sequence, but in order to allow the model to run with some level of efficiency for War and Peace a higher stride was needed.

2. Analysis on final train and test loss for both datasets

# Loss Comparison Table

| Dataset                                       | Final Train Loss | Test Loss       |
| --------------------------------------------- | ---------------- | --------------- |
| Alphabet (`abcdefghijklmnopqrstuvwxyz` × 100) | **0.0004**       | **0.0001**      |
| War and Peace                                 | **1.6493**       | **1.6990**      |

The training loss for alphabet as well as the test loss was incredibly minimal, due to the simplicity of the sequence. At temperature 0, the sequence was recreated perfectly. 

The training loss and test loss for War and Peace, in comparison, was much higher, due to the complexity of the task dealing with a much longer natural language dataset with varied grammar and vocabulary. Both train and test loss remained in the range of around 1.6. At lower temperatures, the output tended towards repition of the same sequence, and at higher it was just nonsense, although there was a point where some sort of structure became apparent (around 0.8).

3. Explain impact of changing temperature

Temperature affected the output of the text a lot. With lower temperatures, the outputs were very predictable and repetitive. With higher temperatures, the text produced bordered on incoherent. For War and Peace, around 0.8 provided kind of understandable results, although the model still struggled with syntax due to the complexity of the original text.

4. Reflection

Training the RNN on both synthetic and real-world text really highlighted the importance of input complexity and model capacity. The alphabet task showed a lower bound in terms of complexity, where as War and Peace provided a much more complicated challenge. Adjusting hyperparameters like sequence length, hidden size, and learning rate was critical to improving performance. The temperature parameter was also interesting as it presented model fine-tuning as something that can be done post-training. This assignment was also effective in combining the different concepts we went over this semester into one complete package.

5. Screenshot

# Alphabet Screenshot

![Alphabet Screenshot](./alphabet.png)

# War and Peace Screenshot

![War and Peace Screenshot](./warandpeace.png)
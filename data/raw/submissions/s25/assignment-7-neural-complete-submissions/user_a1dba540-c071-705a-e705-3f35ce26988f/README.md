...
- For the "ABC" dataset, I conducted multiple experiments using identical hyperparameters while varying two factors: the repetition of the alphabet string (from 100 to 500 repetitions) and the number of training epochs. Increasing the repetitions from 100 to 500 significantly lowered the final loss, indicating a strong correlation between dataset size and model performance. Additionally, increasing epochs consistently decreased loss, with particularly substantial improvements observed within the first few epochs. In contrast, training on the warandpeace.txt dataset showed distinctly different behavior: initial loss started relatively low compared to the alphabet dataset but showed diminishing returns with additional epochs, reducing only slightly across subsequent training iterations and demonstrating that extra epochs offered minimal improvements relative to the increased computational effort.

2. Analysis on final train and test loss for both datasets
- For the alphabet dataset repeated 100 times over three epochs, training began with a loss of 3.1669 and decreased to 1.5427; extending training to ten epochs dramatically reduced this to a minimal loss of 0.0594. Increasing repetitions to 500 resulted in an even lower final loss of 0.0075 after five epochs. Conversely, the warandpeace.txt dataset started with a much lower initial loss of 1.6713 on the first epoch, surpassing most alphabet experiments except the largest (x500, 5 epochs). However, additional epochs showed significantly smaller decreases in loss, decreasing marginally from 1.6713 to 1.5920 and finally to 1.5846, underscoring that additional training epochs provided diminishing returns relative to processing time.

3. Explain impact of changing temperature
- With a default value of 1 for temperature, the output was readable and produced words that made sense would follow the input. As I decreased the temperature down to 0.1, the words fit the input closer and closer, until ultimately not changing past 0.1; the output for 0.1 and 0.001 were the exact same in my experiment. Jumping up to a temperature value of 4 resulted in the addition a lot more puncuation and (;,.) and created a jumbled word of random characters, failing to add a space after a real word was input. Going further, a value of 6 added less common letters such as z, and kept the punctuation pattern as there was the addition of parenthesis. Below is an image of the terminal where I kept the input unchanged, only changing the temperature value to get a good sense of the affect it has.


4. Reflection
- I kept facing an error when trying to implement the calcuation to find loss about dividing by 0, even though through debugging I was proving to myself the value used was non zero. I eventually figured this out through using 383gpt as just a simple fix I was overlooking. This assignment really allowed me to connext the affects that hyperparameters have on the training process. I was a little mixed up on the different hyperparameters, and experiementing on changing them between training on the ABC database helped my understanding of the training process more.

5. IMPORTANT: Include screenshot of output for generate_text() function: 

![image](/Experiement%20Output.png)
![image2](/Screenshot%202025-05-04%20172718.png)
![image3](/Screenshot%202025-05-04%20173139.png)
![image4](/Screenshot%202025-05-04%20173210.png)
![image5](/Screenshot%202025-05-04%20173256.png)
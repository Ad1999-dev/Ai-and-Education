...
- At first, I did not modify anything except number of epoches up to 20 and observed the outputs. This is the output:
output of  sequence: "abcdefghijklmnopqrstuvwxyz" * 100
...
As we can see, although the loss is decreasing, it remains high due to a small number of hidden layers, a small hidden layer size, and an overly large learning rate, which causes instability when testing with the sequence "abcdefghijklmnopqrstuvwxyz". Additionally, the default hyperparameters prevent the testing phase from running. The sequence length is too large (1000), and the stride is only 10, while the input sequence "abcdefghijklmnopqrstuvwxyz" * 100 is not long enough to generate sufficient testing samples. These factors lead to failure in testing mode.

2. Analysis on final train and test loss for both datasets

 Knowing that the accuracy of neural network learning depends on the number of layers, the number of epochs, and the number of neurons, I increased the hidden size to 128, the number of epochs to 20, and learning rate to 10. Also, to make the testing model run, I tune the sequence length and stride a bit. Specifically, sequence length is tuned to 200, and stride to 5.
 
 This is the loss after tuning those parameters:
 | Epoch | Loss |
|-------|-----------|
| 1 | 239.2532 |
| 2 | 35.7593 |
...
| 19 | 0.0000 |
| 20 | 0.0000 |
Additionally, here's the result for the test model:
| Test Model Loss |
|-----------------|
| 0.0000 |

Now, the accuracy of model is significantly better (about 100%).

Test with war and peace:
| Epoch | Loss |
|-------|-----------|
| 1 | 189.5800 |
| 2 | 191.5299 |
...
| 4 | 190.9171 |
| 5 | 191.1816 |
| Test Model Loss | 21.1767 |

Since the war and peace is a large document, then I have to tune the hyperparameters again. Specifically, number of epoches tuned to 5, hidden size and batch size tuned to 64. 

As we see the loss is still high, but the test loss is still reasonable. 

3. Explain impact of changing temperature
The temperature does not significantly impact the sequence "abcdefghijklmnoprpq" because it is a repeated pattern.
To the war and peace document, if the temperature is greater than 1 (more random), the model will return more information. But in the end, the outputs are still meaningless.

This is the evidence:
![evidence](https://github.com/user-attachments/assets/9fd20940-c1a5-45e3-badf-b4b2413c6f1d)
...
![output HW7](https://github.com/user-attachments/assets/756eef48-501a-49a5-a006-9a352fed8972)

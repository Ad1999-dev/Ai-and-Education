...
So I adjusted my learning rate constantly from the 'abc' text, continually lowering the learning rate until I hit 0.0005 as a good learning rate. For the number of epochs I tried having a much larger amount of epochs, but got minimial improvements after 3, leading to me setting it at 3. I decided to set the sequence length, embedding dim, hidden size, and batch size to the following values because I just decided on increasing them each by powers of 2, and kept going until the improvements became minimal. The stride value simply cut down the training time of the model so I used it.

2. Analysis on final train and test loss for both datasets

I have submitted my results on both training sets seperately, but my results were aligned with the assignment details instructions, giving a near 100% accuracy on the abc's and a 57% accuracy on the war and peace. This was good, as it shows my model was trained correctly and aligns with predictions.

3. Explain impact of changing temperature

Temperature rescales the model’s logits before softmax, so dividing by a value less than one amplifies differences between logits and makes high-scoring tokens much more likely while dividing by a value greater than one reduces those differences and boosts lower-scoring tokens. Lower temperatures thus drive sampling toward the highest-probability tokens and produce more predictable, focused outputs, whereas higher temperatures increase randomness and diversity at the cost of coherence. In the extreme, a temperature approaching zero behaves like greedy argmax, and as temperature grows very large the sampling distribution becomes nearly uniform.

4. Reflection

Overall, I think this was a good exercise that displayed all of the skills we learned this semester. This model is a good small RNN model, showing us how it works and laying the ground work for us to implement an RNN. 

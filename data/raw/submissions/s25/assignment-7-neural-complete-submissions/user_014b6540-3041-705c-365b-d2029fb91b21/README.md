...
For the alphabet, I found that the sequnce length of 26 would create the most accurate because the alphabet is 26 letters long. The embeddeding dim size was 16, stride 1, learning rate was 0.001 and since the training time was very fast I did 10 epochs to have the most accurate return.
War peace: During my experiments I was experimenting with different training parameters. I found that increasing the sequence legnth would help with the texts, and having a small stride is more accurate but slower. The embidding dimensions I just played around with to try to find something more accurate and the hidden size I used larger numbers for more complex text that is different to the alphabet. And the learning rate I did a low value to have more accurate data but it took longer.



2. Analysis on final train and test loss for both datasets

With thse parameters:
sequence_length = 26, stride = 1, embedding_dim = 16, hidden_size = 256, learning_rate = 0.001, num_epochs = 10, batch_size = 64 I had the most scucess witht the alphabet. where it returns the alphabet perfectly up to 100 characters (I only tested to 100)

For war and peace I did: 
sequence_length = 200, stride = 3, embedding_dim = 32, hidden_size = 256, learning_rate = 0.003, num_epochs = 1, batch_size = 64 to return pretty coherent words. 

3. Explain impact of changing temperature

Changing the Temperature effects the probability distrubtions increasing randomness in character selection. 

4. Reflection

Overall, the difference between the texts differed the training parameters in making each model more "accurate" in producing words or the alphabet.

5. IMPORTANT: Include screenshot of output for generate_text() function: 

![alt text](<Screenshot 2025-05-03 at 7.48.34 PM.png>)

![alt text](<Screenshot 2025-05-03 at 7.57.48 PM.png>)
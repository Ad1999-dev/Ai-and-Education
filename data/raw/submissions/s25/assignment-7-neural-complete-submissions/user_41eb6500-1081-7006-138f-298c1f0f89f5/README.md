...
1. Describe your experiments and observations
 - I began by training the model on the simple alphabet sequence repeated 100 times. After tuning hyperparameters like sequence length (10), hidden size (8), stride (3), and learning rate (0.05), the model quickly achieved a perfect accuracy due to the simplicity and regularity of the pattern (100.00% accuracy and Loss of 0.0340). Once validated, I trained the model on "warandpeace.txt", adjusting sequence length to 75, stride to 10, learning rate to 0.002, and hidden size to 128 to handle the complexity. This longer text required more expressive power and benefited from these larger numbers such as stride to capture overlapping character relationships. It did come at the cost of time however. 

2. Analysis on final train and test loss for both datasets

- **Alphabet Dataset**: Final Train Loss = 0.0340, Test Accuracy = 100.00%
- **War and Peace**: Final Train Loss = 1.6102, Test Accuracy ≈ 52.83%
- The alphabet sequence was easily learned because of its strict repetition and direct pattern-like behavior (abcdefgh..). War and Peace however is an actual text representative of the english language. As a result of this, there was more variation, grammar, and vocabulary. For example, after the prefix 'the', many possible endings exist (their, then, theft, them, there, etc) and it wouldnt be possible to nail it every time, causing for the less "accurate" numbers

3. Explain impact of changing temperature

- Lower temperatures led to more deterministic and grammatically consistent text, while higher temperatures (numbers greater than 1) make the model more creative and random, which led to unexpected or jumbled text.

4. Reflection

- This project taught me how challenging and rewarding it can be to train a RNN for word recommendation. I started with a simple alphabet sequence, which helped me verify that the model architecture and training loop worked correctly. Also, for this alphbet sequence, training and testing was straight-forward, AND FAST. Once I determined good performance for the alphabet, I switched to War-And-Peace. This is where the real challenge began! I ran into issues with poor performance at first extremely high loss and low accuracy, and it took lots of time from one trial to the next. That is when I decided to create a smaller txt file for testing which sped up my fine tining time. Once I tuned the hyperparameters appropriately, the model started to show meaningful results. I learned the importance of careful data handling, debugging, and how sensitive RNNs are to learning rate and input sequence length. Overall, it deepened my understanding of modeling and parameter tuning, and taught me how to troubleshoot neural network training from both a code and conceptual standpoint.

...
- Dataset 1: Alphabet ("abcdefghijklmnopqrstuvwxyz" × 100)
Used as a minimal dataset to debug the RNN implementation.
Ran multiple training sessions with up to 20 epochs.
Observed that the loss plateaued quickly and improvements were marginal after ~5 epochs.
- Dataset 2: War and Peace (full text)
Provided significantly more meaningful context for training.
Trained for 1 epoch due to time constraints.
Achieved much lower loss compared to the alphabet dataset, even with just one epoch.

2. Analysis on final train and test loss for both datasets: 

| Dataset        | Epochs | Final Train Loss | Final Test Loss |
|----------------|--------|------------------|------------------|
| Alphabet       | 20     | 3.2578       | 3.2314       |
| Alphabet       | 15     | 3.2589            | 3.2277            |
| War and Peace  | 1      | 2.9346      | 2.9238       |
- Alphabet observations:
Model loss quickly plateaus (~3.25) because the data is too simple and repetitive.
RNN cannot generalize much due to limited vocabulary and sequence variety.
- War and Peace observations:
Loss is significantly lower even after just 1 epoch.
Model benefits from diverse language patterns and larger vocabulary.

3. Explain impact of changing temperature
- Low Temperature (< 1):
More deterministic.
Generates safer, more predictable output (often repeating).
Example: abcdefghijklmnopqrstuvwxyz → abcdefghijklmnopqrstuvwxyzaaa
- High Temperature (> 1):
Increases randomness.
May generate creative but less coherent output.
Example: thou → thouvh) or thousfk
- Temperature = 1.0 (default):
Balanced creativity and coherence.
Often gives plausible continuations without excessive repetition or gibberish.

4. Reflection
- Building this character-level RNN was a great way to understand the internals of RNNs, including: Sequence processing, step-by-step Embedding + recurrent dynamics, Sampling from distributions using temperature.
- While the alphabet experiment was helpful for debugging, it highlighted the limits of using toy data.
- Using a real dataset (War and Peace) showed the true power of recurrent models for text generation, even with minimal training.
- Future improvements could include switching to LSTM, training for more epochs, or even switching to Transformer architectures for better sequence modeling.

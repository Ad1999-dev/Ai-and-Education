...
In this assignment, I implemented a character-level RNN from scratch in PyTorch and trained it on two datasets: a "abcdefghijklmnopqrstuvwxyz" alphabet sequence and the full text of War and Peace. I began with the alphabet sequence to verify the correctness of my model architecture and training loop. Since the character transitions are deterministic, the model quickly learned to predict the next character with near-perfect accuracy. This helped confirm that the forward pass, manual weight updates, and batching logic were implemented correctly.

After confirming the model worked on synthetic data, I moved on to War and Peace, which presented a more complex challenge due to the variability and longer dependencies in real language. Training on this dataset required adjusting hyperparameters such as increasing the hidden size to capture patterns and lowering the learning rate to ensure stable convergence. While the model was able to generate some coherent words and English-like sequences, it struggled to form complete grammatical sentences after just a few epochs — a limitation expected from character-level models with a simple RNN architecture.

2. Analysis on final train and test loss for both datasets

Dataset: Simple Sequence
...
The simple sequence experiment produced very low training and test loss values, reflecting the model’s ability to perfectly memorize the repeating "abcdefghijklmnopqrstuvwxyz" pattern. After just a 3 epochs, the model consistently generated correct predictions and completed sequences with high accuracy. In contrast, training on War and Peace yielded significantly higher loss values around ~1.63, which is expected given the complexity and variability of natural English text. Although the generated output was less structured, it contained recognizable English words, sentence fragments, and proper nouns, indicating the model had learned meaningful character patterns. The gap between the simple and complex dataset performance highlights the difficulty of modeling natural language using a basic RNN architecture.


3. Explain impact of changing temperature

Lower temperatures produced more predictable and structured text, often closely following learned patterns. Higher temperatures results in increased randomness due to its factor of taking more risk but it often results in more creative but less coherent outputs.

4. Reflection

Initially, I found it difficult to reason through the manual implementation of the recurrence equations and understand how hidden states evolve over time. Debugging the training loop and understanding why the model wasn't converging required me to go beyond just coding and really understand how sequence models work. Hyperparameter tuning, in particular, was more difficult than expected and showed me how sensitive RNNs are to small changes in learning rate, hidden size, and sequence length.

This project gave me a much deeper appreciation for how text generation works under the hood. I learned the value of gradual experimentation — starting with a simple alphabet dataset helped me verify correctness before moving to a more complex corpus like War and Peace. It also made me realize how much work goes into the models behind technologies we use every day, like predictive typing or AI chatbots. Overall, this assignment gave me a practical understanding of RNNs and a stronger grasp of how deep learning models process sequential data.

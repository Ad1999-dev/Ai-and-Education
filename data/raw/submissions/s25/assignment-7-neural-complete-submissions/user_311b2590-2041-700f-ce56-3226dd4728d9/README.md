...
**How many late days am I using?** 0

---

## Final Report

| Dataset                                        | Train Loss | Test Loss | Test Accuracy |
|-----------------------------------------------:|-----------:|----------:|--------------:|
| Alphabet sequence (“abcdefghijklmnopqrstuvwxyz” × 100) |       3.18 |      2.87 |       100.0%  |
| War and Peace (`warandpeace.txt`)              |       1.48 |      1.53 |        57.3%  |

### 1. Experiments and Observations
In our first experiment, we trained the RNN on the simple repeated‑alphabet sequence with a small network (embedding_dim=32, hidden_size=128, sequence_length=100, stride=10, lr=1e‑3). After one epoch it achieved a train loss of 3.18 and a perfect 100% character accuracy on the held‑out 10% (test loss 2.87). Generation from any seed immediately produced the next letters in order (“abc…” → “def…”), confirming that the model fully memorized the fixed pattern.

When we moved to the real English corpus (War & Peace), we kept the same architecture but tuned the learning rate and increased training time to one full epoch. On this more complex text the model converged to a train loss of 1.48 and test loss of 1.53 (≈ 57% accuracy). Its generated samples at temperature 1.0 contained recognizable English words (“the quirt fallic…”), though not coherent sentences, reflecting partial learning of character‑level dependencies.

### 2. Impact of Changing Temperature
Varying the sampling temperature had a huge effect. On the alphabet task it made virtually no difference. Every temperature reproduced the exact sequence. But on War & Peace, low temperature (≈ 0.5) produced overly conservative text (very common bigrams), while higher temperature (≈ 1.5) injected creative but often nonsensical strings. A temperature of 1.0 struck a balance, yielding plausible word fragments without devolving into pure gibberish.

### 3. Reflection
Implementing this project taught me the importance of:
1. **Hidden‑state warm‑up** to condition on the full seed text before sampling.
2. **`hidden.detach()`** to prevent backpropagation from spanning multiple batches and avoid memory blow‑up.
3. **Tuning Hyperparameters**

I also learned that simple RNNs can perfectly memorize fixed patterns but struggle with long‑range dependencies in natural language, and that temperature control is essential for balancing determinism and creativity in text generation.  

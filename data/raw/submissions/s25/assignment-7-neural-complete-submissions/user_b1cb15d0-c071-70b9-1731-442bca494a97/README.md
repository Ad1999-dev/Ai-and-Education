...
I trained a character level RNN model on random letters and a warandpeace.txt, the warandpeace model was more complicated as it had to go under more testing to find common patterns and such, just like our previous assignment. I dont think I would call what it produced meaningful but it was full words/sentences!
2. Analysis on final train and test loss for both datasets
DATA SET      FINAL TRAIN LOSS      FINAL TEST LOSS
Alphabet        0.002                 0.005
War and Peace   1.73                  1.89

The low train and test loss reflects how much simpliler this model is and how much less training the model needs to pick up on it. The MUCH higher loss on War and Peace is drastic but expected since it is a much more complicated model. 

3. Explain impact of changing temperature
At low temperatures, the output became repetitive, often looping or repeating phrases.
At middle temperatures, we produced plausible word forms and sentence structures.
At high temperatures, the model produced less meaningful output,inventing new "words".
4. Reflection
Building an RNN from scratch helped me understand the difference in different types of models, temperatures and time. The main things I took away were the importance of hidden states, tuning stride/sequence length, and using detach(). I also gained some insight into how a model learns patterns with different levels of complexity! 
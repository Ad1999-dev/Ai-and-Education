...
class CharRNN(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            h_t = torch.tanh(x_embed[t] @ self.W_e.T + self.b_e + h_t_minus_1 @ self.W_h.T + self.b_h) 
            output.append(h_t)
            h_t_minus_1 = h_t
...
sequence = read_file("assignment-7-neural-complete-<redacted>\warandpeace.txt") # Uncomment to read from file
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    input_tensor = input_tensor.to(next(model.parameters()).device)  # Ensure correct device

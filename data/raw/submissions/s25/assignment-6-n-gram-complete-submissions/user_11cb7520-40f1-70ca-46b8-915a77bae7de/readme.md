...
In this I am creating a list of n dictionaries. The first dictionary is the partition of every single character in the text and its frequencies. The second is every 2 character partitions of the document and the nth is every n character partitions. If a partition already exists in the dictionary, it increments the number attached to that partition. 
...
$P(x_1....x_n) = p(x1)*p(x2|x1)*p(x3|x1,x2)*....*P(xj|xj-1-n,....xj-1)$

### Code analysis

- ***Put the intuition of your code here***

Probability of the sequence times the probability of adding the sequence to the last n-1 gram. 
...
1. $P(X_1=a, X_2=a, X_3=a)$
   ...
   $11/20 * 5/10 * 1/4 = 11/160$ 
2. $P(X_1=a, X_2=a, X_3=b)$
   - *Show your work*
   $11/20 * 5/10 * 2/4 = 11/80$
...
- ***Put the intuition of your code here***
Go through the whole vocab and calculate the character's probability of coming next in the sequence. Whichever has the highest is returned. 
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  ...
  b would come next in the sequence with the highest probability. 
...
Enter an initial sequence: pier
Enter the length of completion (k): 2
Updated sequence: pier 
Updated sequence: pier t

Enter the number of grams (n): 4
Enter an initial sequence: pier
Enter the length of completion (k): 2
Updated sequence: pierr
Updated sequence: pierre

with a higher n gram it seems to calculate with greater precision. 

After n = 40 it started crashing before loading all that information. 

N = 10 and n = 20 both were done in under 20 seconds and were very accurate in their predictions. 

I think war and peace worked better because it had a greater corpus to work with. I created a few of my own to see and test it out on smaller alphabets and smaller amounts of text. Cool stuff for sure!
<hr>

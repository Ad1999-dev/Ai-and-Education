...
I ran the program with changing between the abcd test and warandpeace.txt. I tried different variables and temperatues to see how the outputs change. I would run a normal temperature(1) and then a higher temperature(25) and a lower temperature(2) to see how the outputs change. With a temp of 25 the output was more skewed than at temp2, which only changes the last 2 characters in the alphabet showing a wrong output but displaying its "creativity"


2. Analysis on final train and test loss for both datasets

abc*100
Epoch 1, Batch 71/72, Loss: 0.0111
Epoch 1, Average Loss: 0.6330
Epoch 2, Batch 71/72, Loss: 0.0045
Epoch 2, Average Loss: 0.0069
Epoch 3, Batch 71/72, Loss: 0.0027
Epoch 3, Average Loss: 0.0035
Epoch 4, Batch 71/72, Loss: 0.0018
Epoch 4, Average Loss: 0.0022
Epoch 5, Batch 71/72, Loss: 0.0013
Epoch 5, Average Loss: 0.0015
Test Loss: 0.0013, Accuracy: 100.00%

On the dataset of alphabet sequences, the character-level RNN demonstrated remarkable performance.  A perfect 100% test accuracy was achieved after five epochs, when the training loss decreased to 0.0015 and the test loss to 0.0013.  The sharp drop in loss from the first to the last epoch indicates that the model was able to learn the alphabet rapidly and effectively because it follows a straightforward, repetitive pattern.



3. Explain impact of changing temperature
The generated text's degree of determinism or creativity was directly impacted by the temperature parameter.  The model consistently produces the right alphabet sequence when set to the default value of 1.0.  The majority of the pattern was maintained with only minor disruptions when the temperature was raised to 2.0, which introduced a small amount of randomness.  The output became extremely unpredictable and produced illogical sequences when it was increased to 25.  This demonstrates how higher temperatures cause the probability distribution to flatten and produce less predictable, more exploratory results.

4. Reflection

Overcoming the difficulties of managing hidden states and putting the RNN's logic into practice strengthened knowledge of sequence models.  The versatility of probabilistic sampling techniques was further highlighted by the temperature parameter, which also stood out as a potent tool for striking a balance between predictable and creative output during generation.

5. IMPORTANT: Include screenshot of output for generate_text() function: 
![image](/Users/<redacted>/Desktop/desktopold/383/assignment-7-neural-complete-<redacted>/Screenshot 2025-05-03 at 9.33.04 PM.png)
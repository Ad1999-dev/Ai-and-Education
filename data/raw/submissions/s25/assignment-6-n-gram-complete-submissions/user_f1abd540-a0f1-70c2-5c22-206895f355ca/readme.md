...
- ***Put the intuition of your code here*** the intuition behind `create_frequency_tables` is to build a data structure (frequency tables) that effectively summarizes how characters occur in sequences within a text document. This statistical representation allows algorithms to make informed predictions about what character is likely to come next, based on the historical data of character relationships extracted from the text. Essentially, it equips the language model with the necessary context and frequencies that are foundational for tasks such as text prediction and generation.
...
1. ***What is your vocabulary in this case***
   - Write it here-aababcaccaaacbaabcaa
...
the intuition behind `calculate_probability` centers on leveraging historical data (frequency tables) to estimate how likely it is for a specified character to follow a given sequence. The function articulates how relationships between characters can be quantitatively analyzed, allowing a statistical model to predict future events based on patterns observed in the training data. Essentially, it links previous observations to potential outcomes in a contextual, informed manner—essential for applications like text prediction, where understanding the structure of language is key.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - *Show your work*
   $$P(X_1=a, X_2=a, X_3=a) = frac{11}{20} \cdot frac{5}{11} \cdot frac{1}{4} $$
   $$P(X_1=a, X_2=a, X_3=a) = frac{11}{160}  $$
2. $P(X_1=a, X_2=a, X_3=b)$
   - *Show your work*
   $$P(X_1=a, X_2=a, X_3=b) = frac{11}{20} \cdot frac{5}{10} \cdot frac{2}{4} $$
   $$P(X_1=a, X_2=a, X_3=b) = frac{11}{80}  $$
3. $P(X_1=a, X_2=a, X_3=c)$
   - *Show your work* 
   $$P(X_1=a, X_2=a, X_3=c) = frac{11}{20} \cdot frac{5}{10} \cdot frac{1}{4} $$
   $$P(X_1=a, X_2=a, X_3=c) = frac{11}{160}  $$
...
- ***Put the intuition of your code here***
 predict_next_char utilizes the n-gram model's statistics captured in frequency tables to predict the next character in a sequence by evaluating the probabilities of potential candidates from the vocabulary. The function's design is based on the principles of conditional probability, allowing it to leverage historical data effectively while iterating through the vocabulary to determine the most likely next character. This model showcases the balance between data-driven predictions and the contextual understanding of language, crucial for applications in natural language processing.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - *Your answer*
  Based on the probabilty distribution, b has the highest probabilty of being the next character.
...
<hr>Several conclusions can be drawn from testing with the provided corpus files for training as well as with other values of n. First off, it has been demonstrated that the Alice's Adventures in Wonderland text file speeds up the program's processing and can produce predictions for values of n.
 over 20 in a fair amount of time. However, numbers above 10 have shown noticeably longer processing times for the warandpeace text file. Additionally, the outputs were displaying anomalous results when given irregular strings that are unlikely to be in the training files.

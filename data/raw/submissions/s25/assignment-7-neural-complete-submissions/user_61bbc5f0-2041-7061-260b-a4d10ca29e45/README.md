...
How many late days are you using for this assignment? <br>
0<br><br>
**1. Describe your experiments and observations** <br>
When I was training the model with the alphabet * 100 dataset and fine tuning the hyperparameters, I realized that a learning rate that was too small, like 0.001 or 0.01, actually yields a higher loss compared to when the learning rate was set to 0.1. My guess is that this dataset is so small and simple that it's easy to learn so model is able to converge quickly to the pattern. <br> In contrast, when I was training the model on the warandpeace dataset, having a large learning rate gives a really low accuracy. Therefore, I used 0.005 as my learning rate and that yields around a 50% accuracy and loss around 1.6. For larger and more complex datasets like warandpeace, a smaller learning rate seems to be better because there's more complex patterns that the model has to learn. In addition, larger datasets doesn't need as many epochs for convergence. The stride was also reduced from 10 (the alphabet dataset) to 5 (warandpeace) so the model have more samples, which seems to help.<br><br>
**2. Analysis on final train and test loss for both datasets** <br>
For the alphabet dataset, the final train and test loss was pretty similar as both are close to 0. This is can be predicted as the test data is similar to the training data - the training data has 90 sequences of the alphabet whereas the test data has 10 sequences of the alphabet. <br> For the warandpeace dataset, the final train loss, 1.56, and test loss, 1.64, is pretty similar as well. This indicates the model performs similarly on unseen data as it does on the training data. Therefore, there doesn't seem to be any overfitting and the model is generalizing well.<br><br>
**3. Explain impact of changing temperature** <br>
For the alphabet dataset, low temperatures and even a temperature of 2 results in the same generated text. However, when the temperature is increased to 10, we can see varied generated text. This impact is enlarged for the warandpeace dataset - a temperature of 2 started to generate incomprehensible text while a temperature of 1 generates actual words/pseudo words. A temperature of 0.1 starts generating varying real words and no pseudo words. Finally when temperature is below 0.01, the generated text becomes the same - it starts with "was the count " and seems to be repeating "with the count".<br> To sum it up, higher temperature starts randomizing what letters are chosen so when it's high enough, it's just choosing random letters whereas lower temperatures focuses on the top characters which is why when it's low enough, the same sequences of letters get chosen.<br><br>
**4. Reflection** <br>
During the implementation of the forward function and training loop, I had challenges keeping track of the shape of my tensors. For instance, in my training loop, I originally did ```h_t = torch.tanh(h_t_minus_1 @ self.W_h + x_t @ self.W_e + self.b_h)```. However, I got an error saying how I can't multiply the two matrices and eventually I had to take the transpose of my weights: ```h_t = torch.tanh(h_t_minus_1 @ self.W_h.T + x_t @ self.W_e.T + self.b_h)```. I relied heavily on the lecture slides to guide me through the implementation of the forward function as well as using 383GPT to understand the structure of a 3D tensor. However, as I implemented the training loop, I gained a better understanding as to how each current hidden state relies on the previous timestamp's hidden state's outputs and the current time stamp's embeddings to then update the current hidden state, which will be used as in the next timestamp, hence the name recurrent neural networks. <br><br>
**5. Screenshots** <br>
Model trained on alphabet generating text
![Model trained on alphabet generating text](alphabet_output.jpg)<br><br>
Model trained on warandpeace generating text
![Model trained on warandpeace generating text](warandpeace_output.jpg)

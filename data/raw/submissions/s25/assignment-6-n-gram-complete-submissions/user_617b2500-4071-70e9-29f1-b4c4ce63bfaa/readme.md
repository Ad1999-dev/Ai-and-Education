...
- ***The create_frequency_tables function is designed to construct a series of n-gram frequency tables from a given text document, where an n-gram is a contiguous sequence of n characters. It takes two parameters: the document, representing the input text, and n, the maximum n-gram length to analyze. The function returns a list of dictionaries, each corresponding to a different n-gram size from 1 to n, with keys as n-grams and values as their counts. It begins by initializing an empty list called tables and iterates through n-gram sizes using a loop. For each n gram size, it extracts all valid substrings of length n from the document, and tracks their frequency in a new dictionary. If a substring is already present, its count is incremented; otherwise, it is added with an initial count of one. After collecting counts for a given size, the resulting dictionary is appended to the tables list. This systematic approach enables detailed analysis of character patterns in the text and supports downstream tasks in natural language processing such as prediction, modeling, or autocomplete features.***
...
$$ P(\text{char} \mid \text{seq}) = P(x_n \mid x_1, \ldots, x_{n-1}) = \frac{\text{frequency}(x_1, \ldots, x_{n-1},\ \text{char})}{\text{frequency}(x_1, \ldots, x_{n-1})} \$$

### Code analysis

- ***The code first ensures that the proper n-gram is applied to the sequence. If the n-gram table is not long enough for the sequence, the sequence is shortened to match the longest available n-gram context. Next, the function identifies the appropriate frequency tables: one for the prefix (the input sequence) and one for the full n-gram formed by appending the target character. It retrieves the frequency of both the prefix and the full n-gram from these tables. If the prefix has never been seen in the training data, the function returns 0.0 to avoid division by zero and reflect the lack of evidence for prediction. Otherwise, it computes the conditional probability as the ratio of the full n-gram count to the prefix count. This value estimates the likelihood of the target character following the given sequence, supporting predictive tasks in character-level language modeling.***
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - *Show your work*
   $$
    P(X_1 = a, X_2 = a, X_3 = a) = P(a) \cdot P(a \mid a) \cdot P(a \mid aa)
...
2. $P(X_1=a, X_2=a, X_3=b)$
   - *Show your work*
   $$
    P(X_1 = a, X_2 = a, X_3 = b) = P(a) \cdot P(a \mid a) \cdot P(b \mid aa)
    = \frac{11}{20} \cdot \frac{5}{10} \cdot \frac{1}{2}
    = 0.1375
    $$
3. $P(X_1=a, X_2=a, X_3=c)$
   - *Show your work* 
   $$
    P(X_1 = a, X_2 = a, X_3 = c) = P(a) \cdot P(a \mid a) \cdot P(c \mid aa)
    = \frac{11}{20} \cdot \frac{5}{10} \cdot \frac{1}{4}
    = 0.06875
    $$
...
- ***The predict_next_char function implements a basic character-level prediction model using n-gram frequency tables. Given an input sequence, it iterates over each character in the vocabulary and uses the calculate_probability function to estimate the likelihood of that character occurring next in the sequence. It compares these probabilities and keeps track of the character with the highest one, ultimately returning it as the most likely next character. This function forms the core logic behind a simple autocomplete or text prediction system, leveraging frequency-based language modeling to make informed predictions.***
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - *Your answer*
  The most likely next character will be b because the previous sequence is aa. Our answer is supported by the fact that in our previous calculations aab was a more likely sequence than aaa or aac. In addtion, in our probability tables, the conditional probability of b given aa was 1/2. This is more than the conditional probability of c given aa and a given aa which were both 1/4. 
...
<hr>
For my experiments, I tried both corpus. 'warandpeace.txt' and 'Alice's Adventures in Wonderland.txt'. I began by testing smaller values of n such as n=1 or n=2 and gauged the speed and suggestions in both the 2 txt. I found that the generation time was very short in both but I also noticed that the word predictions always repeated the when the value of k is large enough. This is likely due to the fact that it is an article and commonly used within the txts. As I increase n, the program run time  increases as well. However, longer words seemed to appear more often as well. When I input 50 as the n, the program terminated and was not able to give me a prediction. Lastly, it seemed the Alice text can use higher n when compared to the war text. This may be due to the specific words that I was using during my experiment. 

...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        # For the RNN cell, we need weight matrices for input-to-hidden and hidden-to-hidden
    ...
    def forward(self, x, hidden):
        ...
        output = []
        for t in range(l):
            # Implement forward pass for a single RNN timestamp
            x_t = x_embed[t]  # Current input at time t [b, e]
            
            # RNN recurrence equation: h_t = tanh(W_ih * x_t + W_hh * h_{t-1} + b_h)
            h_t = torch.tanh(torch.matmul(x_t, self.W_ih) + 
                             torch.matmul(h_t_minus_1, self.W_hh) + 
                             self.b_h)
            
            output.append(h_t)
            h_t_minus_1 = h_t  # Update for next timestep
        
        output = torch.stack(output)  # [l, b, h]
        output = output.transpose(0, 1)  # [b, l, h]
        
        # Final hidden state to return
        final_hidden = h_t.clone()  # [b, h]
        
        # Project hidden states to logits
        logits = torch.matmul(output, self.W_ho) + self.b_o  # [b, l, vocab_size=v]
...
# Hyperparameters - adjust these based on the task
...
for epoch in range(num_epochs):
    ...
    for batch_inputs, batch_targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        # This prevents backpropagating through the full history, and
        ...
        # batch_targets shape: [batch_size, sequence_length]
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    chars = [char_to_idx.get(c, 0) for c in start_text]  # Using get with default to handle unseen chars
    ...
    # Initialize output text with start_text
    ...
    with torch.no_grad():
        for _ in range(k):
            ...
            # Get the last character's logits (we want to predict what comes after the sequence)
            ...
            # Update input for next iteration (shift one character)
            # We append the new character and take the last n characters
            chars.append(next_char_idx.item())
            chars = chars[1:]  # Remove the first character to maintain sequence length
            input_tensor = torch.tensor([chars], dtype=torch.long).to(device)

...
For the alphabet sequence I found that no matter how many characters I asked it to generate, it would always just generate the alphabet looping over and over. Even when I typed gibberish, I found that it would look at the last character and then pretty much generate the alphabet from there (possibly missing the first few letters right after the last letter in the alphabet). When I inputted something like abc123 using characters it didn't know, it would output ghijklmno... treating the 123 as if it was the sequence "def" from context alone. 

For war and peace, I found that any statement usually generate words, but wouldn't make a comprehensible sentence. If I tested it with "do you love me?" It would output something like " he is quickly rostv". I also found that the more characters I asked to generate, the more misspellings there would be in the words that followed. The first three words would be real words but then it would start misspelling some words.

2. Analysis on final train and test loss for both datasets

For training the alphabet sequence, my loss was 0.0001, and my test accuracy was 100%. I found that if I started with any consecutive sequence of the alphabet, it would generate the next characters in that sequence, or even if I put in gibberish, it would just output the sequence of the alphabet from the last letter.

For the war and peace, my final train loss was 1.4210, and test accuracy was 56.20%. This definitely showed in the fact that I could generate sentences, but the sentences wouldn't make sense 


3. Explain impact of changing temperature

For temperature in the alphabet I found it was pretty consistently outputting the alphabet , though slowly making more and more mistakes as the alphabet ratcheted up. I also found that at certain temperatures the output would be the alphabet if I used abc, but would be much less accurate if I inputted gibberish

For temperature in war and peace, I found that as the temperature rose, the generated text stopped resembling words and started just becoming gibberish. It would go from something lke "house cheese it" at 1.1 to something like "hugoty round;onfidd" at 1.5 to full on gibberish at 4 or 5. The same thing seemed to happen with the alphabet

4. Reflection

I learned a lot about RNNs, and neural networks in general from this. I found that one of the biggest factors in increasing the accuracy of the model was increasing the hidden_size of the neural network, but that would only work up to a certain point. Same with epochs which would have big decreases in loss in the first few, and then shrink by the end. Both of these make sense since one parameter adds more nodes to the neural network and the other retrains it more times. Most importantly of all I learned how long it can take to train these models, and it makes a lot more sense to me why running just 0.5 an epoch is sometimes all people can do on large datasets. 

When I was training this model, my thought process was you want to keep learning rate low around 0.01. For war and peace I felt like a stride of 10 would be fine since I was dealing with words, but a stride of 1 was important for the alphabet sequence since it was really important to determine what the next character would be. I also thought that keeping the sequence length shorter would result in faster training of the model which is why I kept it around 100.

I had trouble with using pytorch and definitely leaned on gpt 383 to help me with it, so I know for sure I need more work with pytorch to get better with it. I also learned why it is important to set the drop_last parameter to true, or else you'll get to the end of your testing and it will just fail :(. 


## Section 5 Terminal Screenshot 

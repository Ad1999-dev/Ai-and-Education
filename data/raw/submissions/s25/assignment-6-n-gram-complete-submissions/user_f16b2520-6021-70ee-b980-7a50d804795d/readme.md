...
I create a list called frequency_tables.
I run the following code n times, where n ranges from 1 to n:

I'm storing a dict called frequency_table, with keys of strings and values of numbers.

Then I run through the string, starting at index n-1 and ending at len(document). For every index, I create a single frequency_table entry based on document[index-(n-1):index] and set value to 0 (unless it already exists, in which case I increment the value float).

Each run appends the frequency_table to frequency_tables.

I return the list frequency_tables once the loop completes.
...
For each time we need to calculate probability based on string and character:

First, it checks if the string exists in the denominator_table for frequency_tables[string.length-1]. If it doesn't, then it returns 0. If it does, then it returns the numerator_entry value based on the character and sequence string, divided by the denominator_table value based on the string.

To find the character probability for a sequence, we run the above function (n = range(1, sequence.length)) times, and return the product of every individual run together.

For every run, we use string = sequence[0, n]
...
1. $P(X_1=a, X_2=a, X_3=a)$
   = $P(a) * P(a \mid a) * P(a \mid aa) = \frac{11}{20} * \frac{5}{11} * \frac{1}{5} = \frac{1}{20}$
2. $P(X_1=a, X_2=a, X_3=b)$
   = $P(a) * P(a \mid a) * P(b \mid aa) = \frac{11}{20} * \frac{5}{11} * \frac{2}{5} = \frac{1}{10}$
3. $P(X_1=a, X_2=a, X_3=c)$
   = $P(a) * P(a \mid a) * P(c \mid aa) = \frac{11}{20} * \frac{5}{11} * \frac{1}{5} = \frac{1}{20}$
...
It runs through every character in the vocabulary, calculating the probability for that character based on the prior string as input. If a character has higher probability than the previous max, the probability and max_character gets updated. It returns max_character once all characters in the vocabulary have been checked.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  ...
  B would be the next character after aa because it has the highest calculated probability.
...
Larger corpus gives you better recommendations because character sequences tend to appear multiple times rather than just once. This is especially important as the 'n' gets larger because more character inputs are considered. When it comes to grams longer than one word, the predictor often returns a specific sentence from the corpus text because those sequence of words only appear a few times in the entire corpus. Any value of n greater than 10 starts to get too time consuming to generate long outputs, but this depends on computer architecture.

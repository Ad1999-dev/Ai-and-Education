...
- We process the document and break it down into sequences (or "n-grams").

- For each sequence, we update the count in the corresponding frequency table (unigrams, bigrams, trigrams, etc.).

- The frequency table provides the raw count of how often each sequence appears.
...
2. ***Write down your probability table 3***:
   - You got this!
    ...
    | $P(\odot)$       | Probability value |
    | ---------------- | ----------------- |
    | $P(b | a, a)$    | $\frac{1}{4} = 0.25$ |
    | $P(c | a, a)$    | $\frac{2}{4} = 0.5$ |
    | $P(a | b, a)$    | $\frac{1}{2} = 0.5$ |
    | $P(c | b, a)$    | $\frac{1}{2} = 0.5$ |
...
### Formula
\[
P(x_{t+1} | x_1, x_2, ..., x_t) = \frac{f(x_1, x_2, ..., x_t, x_{t+1})}{f(x_1, x_2, ..., x_t)}
\]

### Code analysis

- Retrieve the Frequency of the Full Sequence: Get the frequency of the n-gram sequence that includes the current character and the next character.

- Retrieve the Frequency of the Context (Previous Sequence): Get the frequency of just the previous part of the sequence (without the next character).

- Divide to Get Conditional Probability: The conditional probability is calculated by dividing the frequency of the full sequence by the frequency of the context, giving the likelihood of observing the next character given the previous ones.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - To calculate the probability of the sequence `"aaa"`, we use the formula:

    \[
    P(X_1 = a, X_2 = a, X_3 = a) = P(a) \times P(a | a) \times P(a | a, a)
    \]

    Using the values from our probability tables:
    - \( P(a) = 0.55 \) (from the unigram table)
    - \( P(a | a) = 0.3636 \) (from the bigram table)
    - \( P(a | a, a) = 0.25 \) (from the trigram table)

    Now, calculate:

    \[
    P(X_1 = a, X_2 = a, X_3 = a) = 0.55 \times 0.3636 \times 0.25
    \]
    \[
    P(X_1 = a, X_2 = a, X_3 = a) = 0.0502
    \]
...
2. $P(X_1=a, X_2=a, X_3=b)$
   - Now, calculate the probability of the sequence `"aab"`:

    \[
    P(X_1 = a, X_2 = a, X_3 = b) = P(a) \times P(b | a) \times P(b | a, a)
    \]

    Using the values from our probability tables:
    - \( P(a) = 0.55 \) (from the unigram table)
    - \( P(b | a) = 0.1818 \) (from the bigram table)
    - \( P(b | a, a) = 0.25 \) (from the trigram table)

    Now, calculate:

    \[
    P(X_1 = a, X_2 = a, X_3 = b) = 0.55 \times 0.1818 \times 0.25
    \]
    \[
    P(X_1 = a, X_2 = a, X_3 = b) = 0.0249
    \]
...
3. $P(X_1=a, X_2=a, X_3=c)$
   - Finally, calculate the probability of the sequence `"aac"`:

    \[
    P(X_1 = a, X_2 = a, X_3 = c) = P(a) \times P(c | a) \times P(c | a, a)
    \]

    Using the values from our probability tables:
    - \( P(a) = 0.55 \) (from the unigram table)
    - \( P(c | a) = 0.4545 \) (from the bigram table)
    - \( P(c | a, a) = 0.5 \) (from the trigram table)

    Now, calculate:

    \[
    P(X_1 = a, X_2 = a, X_3 = c) = 0.55 \times 0.4545 \times 0.5
    \]
    \[
    P(X_1 = a, X_2 = a, X_3 = c) = 0.125
    \]
...
- The intuition behind the predict_next_char function is that it calculates the probability of every possible next character in the sequence based on the given context (previous characters). It compares these probabilities and returns the character that has the highest probability as the next character.
...
When experimenting with different corpora and varying values of `n`, the results can vary:
- **Smaller values of `n`** (like bigrams) provide faster calculations but may miss longer dependencies between characters.
- **Larger values of `n`** (like trigrams or 4-grams) capture more context and provide better predictions but require more computational resources.

For very high values of `n`, the tables become large and the computations slow, especially with larger corpora. Therefore, there is a trade-off between prediction accuracy and computational efficiency. A good balance between accuracy and performance is typically found around \( n = 3 \) or \( n = 4 \).
<hr>

...
1. Describe your experiments and observations
  ...
  | Dataset       | Seq Len | Stride | Emb Dim | Hidden |    LR     | Epochs | Batch | Train Loss | Test Loss | Test Acc |
|---------------|---------|--------|---------|--------|-----------|--------|-------|------------|-----------|----------|
| Alphabet      |    26   |   1    |   16    |   32   |  1×10⁻²   |   5    |  32   |   0.0014   |  0.0012   |  100.0%  |
| War & Peace   |   150   |  10    |   50    |  128   |  5×10⁻⁴   |   3    |  64   |   1.5455   |  1.5360   |  54.87%  |

  When experimenting with the alphabet set, the setup used in the table above provided a model that quickly memorized the next letter. By epoch 2, the test accuracy was at 100%. By the end of epoch 5, both train an test losses were close to zero. Generation is deterministic because any seed was able to continue the alphabet correctly, regardless of temprature. 

  On the other hand, for War and Peace, the training took much longer and was less accurate. After 3 epochs, the model was able to understand the character statistics with around a 55% accuracy. Generated text contains real and pseudo words. This reflects the learned patterns through the training and also the remaining uncertainty.

2. Analysis on final train and test loss for both datasets

  | Dataset     | Final Train Loss | Final Test Loss | Final Test Acc |
| ----------- | :--------------: | :-------------: | :------------: |
| Alphabet    |      0.0014      |      0.0012     |     100.0%     |
| War & Peace |      1.5455      |      1.5360     |     54.87%     |

  In the alphabet, the test loss was essentially zero and the accuracy was perfect. The model was able to learn the simple and repetitive sequence easily. 

  For War and Peace, the test lost was around 1.54 and the accuracy around 55%. The model was able to capture frequent character patterns such as spaces and letters, but still made many mistakes in less predictable contexts. 

3. Explain impact of changing temperature


  When the temperature was 0.5, the model favored the characters with a higher probability and would most likely give repetitive outputs. At 1.0, it was more balanced between random yields or sometimes parts of the novel itself. When it was 1.5, there was more random results, like more gibberish. 

  For example, when the seed “the ” was used under all three temperatures, all generating 10 characters, at temperature 0.5, the output was “d him on w” which showed how to used more common letters and spaces. At temperature 1.0, the output was “xportnovne” which used more novel fragments but still has some made up parts. At temp 1.5, it outputted “wen:aimy d” which had more overall randomness. 

  For the alphabet, the impact of changing temp was not much since with whatever temp, it outputted the same result. The model had a perfect accuracy so the temperature had no effect on how accurate the output was. 

4. Reflection

  Overall, building this model allowed me to see how differently an RNN handles more simple patterns, like from the alhphabet, vs real language, like in War and Peace. When working with the alphabet, the model only needed a small number of epochs to memorize the exact next letter mapping. On the other hand, War and Peace contained more complex English that the model could only estimate. The sequence length, hidden state size, and learning rate was important because it could lead to unstable training otherwise. Training itself took much longer and the few epochs helped improve the training but not super substantially. The temperature scale was a way to control randomness and it really showed how that scale was able to go from coherence to creativity quickly. During this, I was able to see the effects of hyperparameter tuning on how this RNN reacted to short and repetitive patterns against a full complex text. 

  5. Screenshots

  Alphabet Output
  ![alphabet](images/alphabetss.png)

  War and Peace Output
  ![war and peace](images/warpeace.png)
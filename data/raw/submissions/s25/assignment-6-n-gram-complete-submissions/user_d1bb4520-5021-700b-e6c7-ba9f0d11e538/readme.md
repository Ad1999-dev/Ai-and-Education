...
The function `create_frequency_tables` builds n frequency tables for an n-gram model. The intuition behind the implementation is:

1. First, I create a unigram table (table 1) that counts the frequency of each individual character in the document.
2. Then, for each value k from 2 to n, I create a nested defaultdict structure to store k-gram frequencies.
3. For each position i in the document, I extract:
   - The "context" (the preceding k-1 characters)
   - The "next character" (the character at position i+k-1)
4. I increment the count for this context-character pair in the appropriate table.
...
1. ***Write down your probability table 2***:
   - as in your probability table should look like (wait a second, you should know what I'm talking about)
        ...
        | $P(\odot)$ | Probability value |  
        | ------ | ----------------- |
        | $P(a \mid a)$ | $\frac{5}{11} \approx 0.455$ |
        | $P(b \mid a)$ | $\frac{3}{11} \approx 0.273$ |
        | $P(c \mid a)$ | $\frac{3}{11} \approx 0.273$ |
        | $P(a \mid b)$ | $\frac{3}{4} = 0.75$ |
        | $P(b \mid b)$ | $\frac{0}{4} = 0$ |
        | $P(c \mid c)$ | $\frac{1}{4} = 0.25$ |
        | $P(a \mid c)$ | $\frac{3}{5} = 0.6$ |
        | $P(b \mid c)$ | $\frac{1}{5} = 0.2$ |
        | $P(c \mid c)$ | $\frac{1}{5} = 0.2$ |
...
2. ***Write down your probability table 3***:
   - You got this!
        ...
        | $P(b \mid aa)$ | $\frac{1}{5} = 0.2$ |
        | $P(c \mid aa)$ | $\frac{2}{5} = 0.4$ |
        | $P(a \mid ab)$ | $\frac{1}{3} \approx 0.333$ |
        | $P(c \mid ab)$ | $\frac{2}{3} \approx 0.667$ |
        | $P(b \mid ab)$ | $\frac{0}{3} = 0$ |
        | $P(a \mid ac)$ | $\frac{2}{3} \approx 0.667$ |
        | $P(b \mid ac)$ | $\frac{1}{3} \approx 0.333$ |
        | $P(c \mid ac)$ | $\frac{0}{3} = 0$ |
...
$$P(x_{t+1} \mid x_t, x_{t-1}, \ldots, x_{t-n+2}) = \frac{f(x_{t-n+2}, \ldots, x_t, x_{t+1})}{f(x_{t-n+2}, \ldots, x_t)}$$


### Code analysis

1. If the sequence is longer than our model can handle (n-1), we use only the last n-1 characters.
2. For the unigram case (no context), we calculate P(char) based on its frequency in the document.
3. For higher-order n-grams, we:
   - Find the appropriate table based on context length
   - Calculate the conditional probability: count(context+char) / count(context)
4. If a context is not found (denominator = 0), we implement a backoff strategy:
   - Reduce the context length by one character
   - Recursively call the function with the shorter context
   - This continues until we find a known context or fall back to the unigram model
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $P(a,a,a) = P(a) \cdot P(a|a) \cdot P(a|aa)$
   - $P(a) = \frac{11}{20} = 0.55$
   - $P(a|a) = \frac{5}{11} \approx 0.455$
   - $P(a|aa) = \frac{2}{5} = 0.4$
   - $P(a,a,a) = 0.55 \cdot 0.455 \cdot 0.4 \approx 0.1$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $P(a,a,b) = P(a) \cdot P(a|a) \cdot P(b|aa)$
   - $P(a) = \frac{11}{20} = 0.55$
   - $P(a|a) = \frac{5}{11} \approx 0.455$
   - $P(b|aa) = \frac{1}{5} = 0.2$
   - $P(a,a,b) = 0.55 \cdot 0.455 \cdot 0.2 \approx 0.05$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $P(a,a,c) = P(a) \cdot P(a|a) \cdot P(c|aa)$
   - $P(a) = \frac{11}{20} = 0.55$
   - $P(a|a) = \frac{5}{11} \approx 0.455$
   - $P(c|aa) = \frac{2}{5} = 0.4$
   - $P(a,a,c) = 0.55 \cdot 0.455 \cdot 0.4 \approx 0.1$ 
...
1. For each character in the vocabulary, calculate its probability of following the given sequence using `calculate_probability`.
2. Store these probabilities in a dictionary.
3. Find the character that has the highest probability.
4. Return this character as the predicted next character.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
    - Based on the probability distribution calculated above for "aa":
    - P(a|aa) = 0.4
    - P(b|aa) = 0.2
    - P(c|aa) = 0.4

    Since both 'a' and 'c' have the highest probability (0.4), either could be predicted as the next character. In case of ties, our implementation would choose whichever character comes first in the iteration order of the vocabulary. In this case, since 'a' would typically be considered before 'c', the next character would be 'a'.
...
- Experiment with the given corpus files and varying values of n. Do any corpus work better than others? How high of a value of n can you run before the table calculation becomes too time consuming? Write a short paragraph describing your findings.
    ...
    After experimenting with different corpora and different values of `n`, the following observations were made:
    
    ### Corpus Effects:
    Simpler corpora (with smaller vocabularies and repetitive structures) lead to faster and more accurate predictions. Complex corpora with rich vocabulary and long dependencies can cause the model to struggle without very large `n`.
    
    ### Effect of `n`:
    
    - When `n` is too small (e.g., 1 or 2), the model lacks enough context and often makes poor predictions.
    - Increasing `n` improves context understanding, making predictions much sharper.
    - However, when `n` gets too large (e.g., `n=6` or higher), two issues arise:
      - **Sparsity:** Many sequences do not occur enough in the corpus to reliably estimate probabilities.
      - **Performance:** The time and memory required to create and use the tables grow significantly.
    
    ### Practical limit for `n`:
    For moderate-size corpora, `n=4` or `n=5` tends to be the sweet spot: strong contextual prediction without excessive memory or runtime issues.
<hr>

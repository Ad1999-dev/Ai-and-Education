...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        self.W_out = nn.Parameter(torch.randn(hidden_size, vocab_size) * 0.01) #hidden to output
        self.b_out = nn.Parameter(torch.zeros(vocab_size)) # output bias
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            x_t = x_embed[t]  # shape: [batch, embed_dim]
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    for _ in range(k):
        ...
        next_idx = sample_from_output(last_logits.unsqueeze(0), temperature).item()

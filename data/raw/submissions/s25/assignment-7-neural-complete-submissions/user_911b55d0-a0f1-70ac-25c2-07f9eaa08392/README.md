...
The alphabet dataset recorded a training loss of 0.1679 and a test loss of 0.1559 with a test accuracy of 98.3%. Generating text from the alphabet dataset pretty accurately returned the next letters in the sequence when using a low temperature. For the War and Peace dataset, it has a training loss of 1.8026 and a test loss of 1.6064 with test accuracy 52.61%. Generating text from the text file was decent at generating full words but usually the context didn't make much sense and many words were slightly off. Since language is much more complex than a simple character sequence, it is expected that the accuracy and loss are much worse on this dataset. 
Increasing the temperature increased the randomness in the characters generated. Setting a low temperature made the generated text more predictable for the sequence and increased the coherence and amount of full words generated by the War and Peace trained model. One issue I ran into was a empty loader issue which was fixed by decreasing the sequence length and batch size. I also had to increase the accuracy for the alphabet model which was mostly impacted by tweaking the learning rate. Increasing the accuracy for the War and Peace text was achieved by increasing batch size, number of hidden features and the dimension of character embeddings.

5. Screenshots:
![Screenshot Alphabet](/screenshots/alphabet.png)
![Screenshot War and Peace](/screenshots/warandpeace.png)

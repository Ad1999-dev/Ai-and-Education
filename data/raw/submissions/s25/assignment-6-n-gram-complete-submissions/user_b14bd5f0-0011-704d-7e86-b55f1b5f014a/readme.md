...
Similar to a sliding window leetcode problem, there is a sliding window of  of k which goes from 1 to n. Then the # of next_char after the context is accumulated in the respective rable row.
...
### 
```math
P(x_n \mid x_1, x_2, \dots, x_{n-1}) = \frac{f(x_1, x_2, \dots, x_{n-1}, x_n)}{f(x_1, x_2, \dots, x_{n-1})}
```
### Code analysis

- ***Put the intuition of your code here***

First we set our context. then, we handle the unigram case. In the main loop, we retrieve how many times the current context has occured. If it is zero, reduce the context size by 1. Then we compute the probability from the current context by looking up how often context + char sequence has occured and divide by how often the context occured.
...
Final calculation:
P(a, a, a) = (11/20) * (5/10) * (1/4) = 55/800 = 0.0687
...
Final calculation:
P(a, a, b) = (11/20) * (5/10) * (2/4) = 110/800 = 0.1375
...
Final calculation:
P(a, a, c) = (11/20) * (5/10) * (1/4) = 55/800 = 0.06875
...
for each charachter in the vocab, we call calculate_probability() to compute how likely this character is to come after the given sequence. If the current character's probability is the greatest so far, we update best_char and best_prob. Once we finish iterating, we return the character with the highest probability.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - It would be 'b' as it is the highest probability.
...
Warandpeace.txt seems to work the best, which makes sense as it is significantly longer. Once I reached an n value of around 30+ it started taking about half a minute or so. At n = 100 it took about 2-3 minutes. The results were pretty interesting as it always formed correct words, and sometimes a sequence of words would actually make perfect sense. However, a majority of the time, it the generated sentence was grammatically incorrect and didn't make any sene. It seemed as if an alien was trying to recreate an english sentence by trying to sound human but the sentences had no meaning.

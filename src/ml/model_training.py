"""Build, train, evaluate, and persist ML models for nested cross-validation.

Nested CV structure
-------------------
- Outer loop  (run_nested_cv)  : GroupKFold over students → unbiased RMSE estimates.
- Inner loop  (train_fold)     : GridSearchCV with GroupKFold → hyperparameter selection.

The outer loop is implemented manually so that `groups` is explicitly passed to
inner_search.fit().  cross_val_score() cannot propagate groups to a nested
GridSearchCV(GroupKFold) estimator and must never be used here.

sklearn Pipeline convention
---------------------------
Every Pipeline has exactly two named steps:
    "preprocessor" → ColumnTransformer (median imputation + z-score scaling)
    "model"        → the estimator
Param-grid keys must use the "model__" prefix (e.g. "model__alpha").
"""
import json
...
def run_nested_cv(
    ...
    Returns
    -------
    DataFrame indexed by model name, sorted ascending by mean_rmse, with columns:
        mean_rmse, std_rmse, min_rmse, max_rmse, fold_1_rmse, ..., fold_K_rmse
    """
    ...
    for name, (model, param_grid) in registry.items():
        ...
        for train_idx, test_idx in outer_cv.split(X, y_arr, groups_arr):

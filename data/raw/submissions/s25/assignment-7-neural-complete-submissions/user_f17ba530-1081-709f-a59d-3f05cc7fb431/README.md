...
When training on the alphabet sequence, the model achieved a final training loss of approximately 0.0006, a test loss of around 0.0003, and pretty much perfect accuracy after ten epochs. The model generated text that was almost perfect, producing the expected continuation of alphabetic sequences with high accuracy, such as "cde" suggesting next characters like "fgh". However the higher the temperature was, the more random the predicted characters were and the generated text did not follow the alphabet exactly. In contrast, training on the War and Peace dataset was significantly more complex due to the intricacies of natural language. I had to change some of the hyperparameters to be more compatible with the longer and more complex dataset. The final training loss was around 1.7-1.8, and the test loss was approximately 1.77. The outputs generated from this dataset were more variable; they included recognizable words and phrases, but also many nonsensical sequences, showcasing the challenge of modeling complex language structures compared to a simple repetitive pattern.

2. Analysis on final train and test loss for both datasets

When training on the alphabet sequence, the model achieved a final training loss of approximately 0.0006, a test loss of around 0.0003, and pretty much perfect accuracy after ten epochs. On other hand, when training on War and Peace, the model achieved a final training loss of approximately 1.7-1.8, a test loss of around 1.77, and about 0.478 accuracy after one epoch. The training and testing loss on the alphabet dataset was significantly lower, indicating the model's ability to easily learn repetitive patterns. In contrast, the higher loss values from the literary text reflect the complexity involved in understanding language context and the variations inherent in prose.

3. Explain impact of changing temperature

The temperature parameter plays a crucial role in influencing the randomness of the sampling during text generation. A temperature of 1.0 generally provides a balanced output, while values greater than 1.0 lead to more diverse and creative outputs. For example, with a temperature of 0.5, the text generation was more deterministic, yielding similar phrases across multiple runs, such as "cde" followed predictably by "fgh". Conversely, increasing the temperature to 5 introduced randomness, leading to unpredictable results like "cdesxdvd".

4. Reflection

Throughout this assignment, I faced challenges primarily related to hyperparameter tuning for the RNN. Finding the right balance between the parameters was difficult, especially when transitioning from a simple sequence to the *War and Peace* text. Also, as I changed some parameters (especially the sequence length), the time it took to run also in increased. I had to be careful to not overfit the training data because it overfitting caused my test accuracy to decrease. A key insight I gained about RNNs and sequence modeling is the importance of finding a balance between the model complexity (hidden size, embedding dimensions) and learning stability (learning rate, sequence length).

5. IMPORTANT: Include screenshot of output for generate_text() function: 

Alphabet Output

![Alphabet Output](<Screenshot 2025-05-04 at 11.44.00 PM-1.png>)

War and Peace Output

![War and Peace](<Screenshot 2025-05-04 at 11.57.57 PM.png>)
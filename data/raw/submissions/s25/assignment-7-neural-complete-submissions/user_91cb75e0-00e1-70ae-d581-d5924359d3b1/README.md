...
For this assignment, we had to train a character-level RNN on two datasets: being a simple repeating alphabet sequence ("abcdefghijklmnopqrstuvwxyz" * 100) and the full text of War and Peace provided in a txt file. The alphabet dataset acted as a control to test and validate the model. The model quickly learned the pattern and achieved nearly perfect accuracy after just one epoch. Conversely, War and Peace introduced substantial complexity, with irregular patterns and a larger vocabulary which required more tweaking and training time

I initially set low values for hidden size and embedding dimensions, but the model underfit on real text. Increasing hidden size to 128 and embedding dimension to 32 significantly improved learning. I also experimented with stride, batch size, and sequence length to balance training speed and performance.
2. Analysis on final train and test loss for both datasets
| Dataset                 | Train Loss | Test Loss |
| ----------------------- | ---------- | --------- |
| Alphabet Sequence       | \~0.01     | \~0.02    |
| War and Peace (1 epoch) | \~1.75     | \~1.80    |

The alphabet model converged almost immediately. For War and Peace, even a single epoch showed learning, but the loss plateaued due to the complexity of the text and the limitations of a simple RNN model.
3. Explain impact of changing temperature
Temperature in text generation controls randomness. At temperature = 1.0, generated text from the trained alphabet model looked like:
Input: abc -> Output: defgh
This confirms the model learned the strict sequence.
for a different temperature though, say 3, the model on the alphabet looked more random:
Input: abc -> Output: dewtu

For War and Peace however the low temperature (0.3) led to repetitive or dull outputs like the 'the the of the', which shows high confidence but low variety. High temperatures on the other hand (1.5) produced more diverse and creative text but often included gibberish or incorrect spelling: 'rathome wej msque', etc.

4. Reflection
This project clarified how RNNs learn sequential dependencies and how simple architectural changes or tuning hyperparameters can dramatically affect the output. It seemed like a natural progression of the original projects regarding word predictions and neural networks, with a lot of the same techniques at play. Training on the alphabet validated that my model worked, but training on real-world data from the book showed how limited simple RNNs can be, and how much more complicated the model needs to be for practical uses. I now better appreciate the need for models like GRUs or LSTMs for more complex tasks. One challenge was balancing model size and batch size to prevent memory errors, basically changing and tuning the model, especially when using the War and Peace txt. Understanding and learning more about temperature was also cool.

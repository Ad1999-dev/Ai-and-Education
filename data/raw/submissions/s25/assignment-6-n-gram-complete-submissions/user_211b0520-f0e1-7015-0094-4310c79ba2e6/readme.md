...
- ***Creates n frequency tables. Each table has a different prefix length going from 0 to n-1. For each position in the document, the function looks at every possible prefix length up to n-1 and records how often specific characters follow the prefix.***
...
1. ***What is your vocabulary in this case***
   - The distinct characters present in the training document are `{'a', 'b', 'c'}`. 
...
$$
P(X_1 = x_1, X_2 = x_2, \ldots, X_n = x_n) = \frac{f(x_1)}{size(C)} \cdot \prod_{i=2}^{n} \frac{f(x_{i-1}, x_i)}{f(x_{i-1})}
$$


### Code analysis

- ***The `calculate_probability` function computes the conditional probability of a character `char` appearing next in a given `sequence` context, utilizing precomputed frequency tables. It starts by checking if the length of the sequence is sufficient to form a valid context; if not, it returns 0. The context is derived from the last `n-1` characters of the sequence. Using this context, the function retrieves the frequency of the character following this context (`f_seq_char`) and the total number of occurrences of any character following this context (`f_seq`). Finally, it calculates the probability by dividing `f_seq_char` by `f_seq`, ensuring that divisions by zero are avoided, allowing accurate probability computations.***
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - *Show your work*
   $P(a) \cdot P(a|a) \cdot P(a|aa) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{1}{4} = 0.0625$

2. $P(X_1=a, X_2=a, X_3=b)$
   - *Show your work*
      $P(a) \cdot P(a|a) \cdot P(b|aa) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{2}{4} = 0.125$

3. $P(X_1=a, X_2=a, X_3=c)$
   - *Show your work* 
      $P(a) \cdot P(a|a) \cdot P(c|aa) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{1}{4} = 0.0625$
...
- ***The `predict_next_char` function predicts the next character that is likely to follow a given `sequence` by leveraging the frequency tables created in `create_frequency_tables`. It generates a probability distribution for all characters in the specified `vocabulary` based on their calculated probabilities. After filtering out characters with zero probability, the function identifies the character with the maximum probability among valid predictions. If no valid predictions are found, it returns a fallback character (a space).***
...
After expirementing with the different corpus files, I found that the `Alice's Adventures in Wonderland.txt` file was signifigantly faster than `warandpeace.txt`. This is likely because `Alice's Adventures in Wonderland.txt` is signifigantly shorter, meaning it will take much less time to generate the frquency tables. `Alice's Adventures in Wonderland.txt` also gave generally good answers. For example when using n=5, k=3, and an initial sequence of " gr", (where there is a space before gr), both corpuses allowed the autocomplete to come up with the word great, which is a common word starting with "gr", so it is a good output for an autocomplete. However, `Alice's Adventures in Wonderland.txt` came up with "great" much quicker, taking less than half a second, where `warandpeace.txt` took more than 5 seconds. If this were an autocomplete being used in a document editor, less than 0.5 seconds would definatly be an acceptable time for the autocomplete to finish, but 5+ seconds would not be. Although `Alice's Adventures in Wonderland.txt` was faster and gave generally good answers, it did occasionally come up with worse answers than `warandpeace.txt`. For example, when doing n=10, k=10, initial sequence = "cat", the autocomplete for `Alice's Adventures in Wonderland.txt` came up with "caterpillar ", where `warandpeace.txt` came up with "cation of the". I would say that "cation of the" is a better answer because there are many words that could end in "cation" such as modification, fortification, education, or communication. Also, all of those words can be followed by "of the". The answer `Alice's Adventures in Wonderland.txt` came up with is much less general, and would only work in specific instances, such as if the user were writing about something that had caterpillars appear a lot, such as `Alice's Adventures in Wonderland.txt`. Overall, `Alice's Adventures in Wonderland.txt` is much faster, due to being short, but it is also worse at giving generalized answers because of how short it is. When it comes to picking an `n` value, I found that for predicting the end of the word currently being written, n=5 was sufficient. However, to get a more generall answer, and maybe even predict the next word, I found that n=10 was good. In any instance, I think that the autocomplete should happen in less than 1 second in order for it to be useful. This seemed to happen with n=20 for `Alice's Adventures in Wonderland.txt`, and with n=2 for `warandpeace.txt`. Overall, if I wanted to implement this autocomplete into a document editor, I would probably choose n=10, and use a document around the lenght of `Alice's Adventures in Wonderland.txt` as the corpus. However, rather than using a fictional story like `Alice's Adventures in Wonderland.txt`, I would probably chose a story that takes place in the real world so that the names of magical creatures aren't chosen as the completions of words. I might also stitch several short aticles or stories together as the corpus to try to get more general answers for the autocomplete. This, along with n=10 would provide an autocomplete that is genralized, but also can still complete words faster than the user can.

...
The create_frequency_tables function builds n separate frequency tables from the training text. Each table records how often character sequences appear: Table 1: individual characters (unigrams), Table 2: pairs of characters (bigrams), Table 3: triplets of characters (trigrams) and so on up to n. This implementation uses simple strings as keys, which represent sequences of characters. It loops through the document and for each starting index, creates substrings of lengths 1 to n, and increments the corresponding counts.
...
**Р(Х1, Х2, Х3) = Р(Х1) * Р(Х2 | X1) * (Х3 | X1, X2)**


### Code analysis
The calculate_probability function calculates the likelihood of the next character by combining the input sequence and the character. We look up the probability of the first character, the probability of the second character given the first character, and then the probability of the third character given the sequence starts with the first and second character. It multiplies these values to get our probability.
...
P(a, a, a) = 11/20 * 1/2 * 1/4 = 11/160

---
2. $P(X_1=a, X_2=a, X_3=b)$
---
P(a, a, b) = P(a) * P(a | a) * P(b | aa)
...
P(a, a, b) = 11/20 * 1/2 * 2/4 = 11/80

---
3. $P(X_1=a, X_2=a, X_3=c)$
---
P(a, a, c) = P(a) * P(a | a) * P(c | aa)
...
The predict_next_char function loops through every possible next character in the vocabulary. It uses calculate_probability to find the probability of each character being the next one. It picks the character with the highest probability and returns it.
...
'b' since it has the highest probability of 11/80
...
When testing with large text like War and Peace, the model works well at lower n values like n=2 or n=3 because there are many examples of short character sequences. When using very large n like n=6 or n=7, the tables become huge and many possible sequences have zero counts, making predictions unreliable. Overall, using n=2 (bigram) or n=3 (trigram) gives the best balance of performance and accuracy for character level prediction.

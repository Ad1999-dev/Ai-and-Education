...
In this experiment, I trained a character-level RNN model on two datasets: a simple repetitive sequence of the English alphabet and the complex natural language text of War and Peace. For the alphabet sequence, the model quickly achieved extremely low training and test loss (~0.0013) within 3 epochs, and it generated text that perfectly replicated the sequence regardless of temperature, demonstrating strong memorization. In contrast, training on War and Peace resulted in higher final loss (~1.81), and the generated outputs varied with temperature, showing some coherence at lower temperatures and increasing randomness as temperature rose. This contrast illustrates the model's ability to memorize simple patterns easily, while complex language requires more capacity and data to learn meaningful structure.



2. Analysis on final train and test loss for both datasets

The experiments revealed a stark contrast in training dynamics between the simple alphabet dataset and the complex War and Peace text. For the alphabet dataset, the model achieved a near-perfect training loss of 0.0019 and a test loss of 0.0013, suggesting excellent generalization. This was expected, as the sequence "abcdefghijklmnopqrstuvwxyz" repeated 100 times is highly regular and entirely deterministic. The model quickly learned the fixed character transition pattern, requiring minimal effort to predict subsequent characters with high accuracy.

On the other hand, the War and Peace dataset resulted in much higher loss values, with the final training loss at 1 7390 and test loss at 1.8129. This increase reflects the significantly greater complexity and diversity of natural language, which includes a vast vocabulary, variable sentence structure, and irregular grammatical constructs. The higher test loss also indicates a slight overfitting, where the model performs better on the training data than on unseen text. These results illustrate the challenge of learning long-range dependencies and semantics in natural language compared to highly repetitive synthetic sequences.



3. Explain impact of changing temperature

In the text generation phase, the temperature parameter plays a crucial role in controlling the randomness of the output. A lower temperature (e.g., 0.5) makes the model more conservative—it strongly favors characters with higher predicted probabilities. This leads to safer, more predictable, and grammatically coherent text. For example, using a temperature of 0.5 with War and Peace produced structured phrases like:
“thers fire wer thers and not to hims that the gration that the whily counted…”,
which, while repetitive and somewhat awkward, still resembled proper English sentence structure.

As the temperature increases (e.g., 0.8 or 1.0), the model becomes more creative and diverse in its character choices, but at the risk of coherence. At temperature = 1.0, the generated text had more novel word combinations and less predictable phrasing, introducing slight errors but also more variety:
“theror.h sheek metha to heamy hay.be riste. hadodpoinethis…”

At even higher temperatures (e.g., 1.2), the randomness increases significantly, and the text may become more chaotic and less meaningful:
“the light that francedbylided cond pres lent neal wayed.boyed…”
This shows that while high temperatures encourage creativity, they also reduce grammatical correctness and coherence.

Thus, the temperature provides a tradeoff between predictability and creativity, and selecting an optimal value depends on the desired style of output.

4. Reflection

This assignment provided valuable insight into how character-level RNNs learn language structure and generate text. Initially, training on a simple repeating sequence like the alphabet quickly yielded perfect predictions, showing how effectively the model can memorize and replicate simple patterns. In contrast, training on a complex corpus like War and Peace revealed the limitations and challenges of modeling natural language: the model achieved much higher loss and produced text that was more diverse but less coherent. I observed how parameters like temperature influenced the balance between creativity and accuracy during generation. Overall, this project deepened my understanding of sequence modeling, the impact of hyperparameters, and the trade-offs involved in generating human-like text using neural networks.

## Milestone 5: Terminal Output

### War and Peace Dataset Results:

![War and Peace Image](warandeaceimg.png)


### Alphabet Dataset Results

![Alphabet Image](alphabetimg.png)

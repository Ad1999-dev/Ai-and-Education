...
- I use a list of dictionaries, where each dictionary corresponds to n-grams of a specific length. For example, the first dictionary stores unigrams, the second stores bigrams, and so on up to length n. I populate these tables using a nested loop: the outer loop iterates through each character in the document, and the inner loop builds all possible substrings starting at that position up to length n, incrementing the count for each n-gram found.
...
2. ***Write down your probability table 3***:
    - You got this! 
        | $P(\odot)$           | Probability value|
        | ---------------------| --------------|
        | $P(a \mid a \mid a)$ | $\frac{1}{5}$ |
        | $P(b \mid a \mid a)$ | $\frac{2}{5}$ |
        | $P(c \mid a \mid a)$ | $\frac{1}{5}$ |
        | $P(a \mid b \mid a)$ | $\frac{1}{3}$ |
        | $P(b \mid b \mid a)$ | $\frac{0}{3}$ |
        | $P(c \mid b \mid a)$ | $\frac{2}{3}$ |
        | $P(a \mid c \mid a)$ | $\frac{0}{2}$ |
        | $P(b \mid c \mid a)$ | $\frac{1}{2}$ |
        | $P(c \mid c \mid a)$ | $\frac{1}{2}$ |
        | $P(a \mid a \mid b)$ | $\frac{1}{2}$ |
        | $P(b \mid a \mid b)$ | $\frac{1}{2}$ |
        | $P(c \mid a \mid b)$ | $\frac{0}{2}$ |
        | $P(a \mid b \mid b)$ | $\frac{0}{0}$ |
        | $P(b \mid b \mid b)$ | $\frac{0}{0}$ |
        | $P(c \mid b \mid b)$ | $\frac{0}{0}$ |
        | $P(a \mid c \mid b)$ | $\frac{2}{2}$ |
        | $P(b \mid c \mid b)$ | $\frac{0}{2}$ |
        | $P(c \mid c \mid b)$ | $\frac{0}{2}$ |
        | $P(a \mid a \mid c)$ | $\frac{2}{3}$ |
        | $P(b \mid a \mid c)$ | $\frac{0}{3}$ |
        | $P(c \mid a \mid c)$ | $\frac{1}{3}$ |
        | $P(a \mid b \mid c)$ | $\frac{1}{1}$ |
        | $P(b \mid b \mid c)$ | $\frac{0}{1}$ |
        | $P(c \mid b \mid c)$ | $\frac{0}{1}$ |
        | $P(a \mid c \mid c)$ | $\frac{1}{1}$ |
        | $P(b \mid c \mid c)$ | $\frac{0}{1}$ |
        | $P(c \mid c \mid c)$ | $\frac{0}{1}$ |
...
$$
P(\text{sequence} + \text{char}) = \frac{P(\text{char} \mid \text{sequence}[n-1] , \text{sequence}[n-2],  \dots , \text{sequence}[0])}{P(\text{sequence}[n-1] , \text{sequence}[n-2], \dots , \text{sequence}[0])}
$$

### Code analysis

- To compute the probability of a character following a given sequence, first extract the frequency of the full n-gram (sequence + character) from the table that stores strings of length len(sequence) + 1. Then, get the frequency of the sequence alone from the table that stores strings of length len(sequence). Dividing these two values gives the conditional probability of the character occurring after the sequence.
...
$= P(X_1 = a) \cdot P(X_2 = a \mid X_1 = a) \cdot P(X_3 = a \mid X_2 = a, X_1 = a)$  
$= \left( \frac{11}{20} \right) \cdot \left( \frac{5}{11} \right) \cdot \left( \frac{1}{5} \right)$  
$= \frac{11}{220}$

2.  $P(X_1 = a, X_2 = a, X_3 = b)$  
$= P(X_1 = a) \cdot P(X_2 = a \mid X_1 = a) \cdot P(X_3 = b \mid X_2 = a, X_1 = a)$  
$= \left( \frac{11}{20} \right) \cdot \left( \frac{5}{11} \right) \cdot \left( \frac{2}{5} \right)$  
$= \frac{11}{110}$

3.  $P(X_1 = a, X_2 = a, X_3 = c)$  
$= P(X_1 = a) \cdot P(X_2 = a \mid X_1 = a) \cdot P(X_3 = c \mid X_2 = a, X_1 = a)$  
$= \left( \frac{11}{20} \right) \cdot \left( \frac{5}{11} \right) \cdot \left( \frac{1}{5} \right)$  
$= \frac{11}{220}$
...
- Iterate through each character in the vocabulary and calculate the probability of it following the current sequence. Keep track of the character with the highest probability, updating it whenever a higher probability is found. After checking all characters, return the one with the highest probability.
...
The model processed the Alice’s Adventures in Wonderland file significantly faster than the War and Peace file. This was expected, as War and Peace is a much larger text, which naturally leads to longer processing times when parsing throught and building/scanning frequency tables. I also found that increasing the values of n and k (together and individually) noticeably affected performance. Higher n values require more lookups into the frequency tables, and higher k values extend the prediction process as there is simply more to predict. One observation I had is that when the n values started to get in the double digits, especially for the war and peace text, the time to run was unreasonable long. At that point, it would save you time to just simply type out the sentence/word! 

...
- The function just counts substrings. For each length from 1 up to n, it slides a window of that size over the text in the document and counts how often each slice appears. It return a list of dictionaries where each dictionary is a frequency table.
...
- The function just scores “sequence+char” by chaining together n‑gram probabilities: start with the unigram chance of the first symbol, then for each next symbol (including your char) look up how often that symbol follows its last up-to (n-1) context and multiply all those ratios.
- If our model order is n, then at each step we look back at the last at most (n−1) symbols to form an n‑gram for the conditional probability. So when we’re predicting the 4th character in a 3‑gram model, we use the previous 2 characters (n−1=2) as our context, not more. Likewise, for the 2nd character we only have one previous symbol, so we use that.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $P(a) \times P(a|a) \times P(a|aa) \\ = \frac{11}{20}\times\frac{5}{11}\times\frac{1}{5}
= \frac{1}{20}$

2. $P(X_1=a, X_2=a, X_3=b)$
   - $= P(a) \times P(a|a) \times P(b|aa) \\ = \frac{11}{20}\times\frac{5}{11}\times\frac{2}{5}
= \frac{1}{10}$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $P(a) \times P(a|a) \times P(c|aa) \\ = \frac{11}{20}\times\frac{5}{11}\times\frac{1}{5}
= \frac{1}{20}$
...
- The function tries every character in the given vocabulary, and uses the calculate_probability function to score how likely each one is after the given sequence, and returns the character with the highest score.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - The next character should be b since $P(X_1=a, X_2=a, X_3=b)$ is the largest.
...
I tried running the n‑gram autocomplete on two texts: Alice's Adventures in Wonderland and War and Peace. Both worked fine up to n = 10. But as expected, War and Peace took longer since it's way bigger. The performance gets better as n increases, however the execution time escalates pretty quickly because the number of unique n-grams grows fast. Alice handled higher n a bit better because it's shorter and has more consistent language. In general, n = 10 seems like a good upper limit anything more than that starts getting pretty slow and not really worth it for most basic experiments.

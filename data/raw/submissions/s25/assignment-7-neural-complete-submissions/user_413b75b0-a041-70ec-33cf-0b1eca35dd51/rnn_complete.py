    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            h_t = torch.tanh(
                ...
                h_t_minus_1 @ self.W_hh + self.b_hh
                )
            output.append(h_t)
            h_t_minus_1 = h_t
        ...
        logits = self.out(output) # [b, l, vocab_size=v] 
...
char_to_idx = {c: i for i, c in enumerate(vocab)} # TODO: Create a mapping from characters to indices
idx_to_char = {i: c for i, c in enumerate(vocab)} # TODO: Create the reverse mapping
...
for epoch in range(num_epochs):
    ...
    for batch_inputs, batch_targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        optimizer.zero_grad()
        loss = criterion(
            output.reshape(-1, output_size),          # [b*l, v]
            batch_targets.reshape(-1)                 # [b*l]
            )
        loss.backward()
        optimizer.step()
...
def evaluate(model, data_loader):
    ...
    for x, y in data_loader:
        ...
        loss = criterion(logits.reshape(-1, output_size), y.reshape(-1))
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    model.eval()
    with torch.no_grad():
        idxs = [char_to_idx[c] for c in start_text]
        hidden = None
        for _ in range(k):
            inp = torch.tensor([idxs[-n:]], dtype=torch.long).to(device)  # [1,n]
            logits, hidden = model(inp, hidden)
            next_logit = logits[0, -1]       # last time-step, shape [v]
            next_idx = sample_from_output(next_logit.unsqueeze(0), temperature).item()

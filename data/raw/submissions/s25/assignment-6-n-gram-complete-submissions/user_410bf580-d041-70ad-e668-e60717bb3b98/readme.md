...
- In the create_frequency_tables, a big loop will loop through each character; inside the loop, the unknown character will be in a new table, and the known character will add the counter. After the single character count, another loop will loop through grams from 2 to n doing the same counting-adding character combo like single character.
...
| $P(\odot)$               | Probability Value     |
|--------------------------|-----------------------|
| $P(b \mid a a)$          | $\frac{2}{18}$        |
| $P(c \mid a b)$          | $\frac{2}{18}$        |
| $P(a \mid b c)$          | $\frac{2}{18}$        |
| $P(a \mid c a)$          | $\frac{2}{18}$        |
| $P(a \mid a a)$          | $\frac{1}{18}$        |
| $P(c \mid a a)$          | $\frac{1}{18}$        |
| $P(a \mid a b)$          | $\frac{1}{18}$        |
| $P(b \mid a c)$          | $\frac{1}{18}$        |
| $P(c \mid a c)$          | $\frac{1}{18}$        |
| $P(b \mid a a)$          | $\frac{1}{18}$        |
| $P(b \mid b a)$          | $\frac{1}{18}$        |
| $P(c \mid c a)$          | $\frac{1}{18}$        |
| $P(a \mid c b)$          | $\frac{1}{18}$        |
| $P(a \mid c c)$          | $\frac{1}{18}$        |
...
### Formula
-   \[P(x_{t+1} \mid x_1, x_2, \dots, x_t) = \frac{C(context, x_{t+1}) + 1}{C(context) + V}\]

Where:
- \( V \) is the size of the character vocabulary (the number of unique characters).

### Code analysis

- It identifies the relevant table based on the sequence length, finds the count of the full sequence including the character, and divides it by the count of the sequence. 
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - = P(a) * P(a|a) * P(a|a,a) = 11/20*5/19*1/18 = 0.0080409
2. $P(X_1=a, X_2=a, X_3=b)$
   - = P(a) * P(a|a) * P(b|a,a) = 11/20*5/19*1/18 = 0.0080409
3. $P(X_1=a, X_2=a, X_3=c)$
   - = P(a) * P(a|a) * P(c|a,a) = 11/20*5/19*2/18 = 0.0160818
...
- It uses calculate_probability() to compute these probabilities and selects the character with the highest value
...
There seems to be no specific n value that performs better than others; however, when n is greater than k, the predictions are always better than when n is less than k.

...
The Alphabet model generated highly accurate and deterministic text due to a dataset featuring a repeating sequence of `"abcdefghijklmnopqrstuvwxyz"`. For example, inputting `"abc"` resulted in: defghijklmno, showcasing generalization. 
In contrast, the War and Peace model produced text that, while not grammatically correct, displayed some character-level structure. Given the input `"speaking french"`, it might generate: je into t, capturing plausible transitions like `"je"` (French) and `"into"`, though lacking coherence and meaningfulness. After one epoch of training, it began producing **pseudo-English** or fragmented real words, indicating a partial understanding of language.

2. Analysis on final train and test loss for both datasets

## Hyperparameter Comparison: Alphabet vs. War and Peace

| Parameter             | Alphabet Experiment          | War and Peace Experiment      |
|-----------------------|------------------------------|-------------------------------|
...
| **batch_size**        | 64                           | 128                           |
| **Test Loss**         | 0.00                         | 1.67                          |
| **Test Accuracy**     | 100%                         | 51.12%                        |
| **Example Input**     | `"abc"`                      | `"speaking french"`           |
| **Example Output**    | `"defghijklmno"`             | `"je into t"`                 |


3. Explain impact of changing temperature

The temperature parameter controls the randomness of sampling from the model's output distribution:

Low temperature makes the model more conservative. It picks high-probability characters more deterministically, leading to safer but more repetitive or "boring" output. 
High temperature increases randomness, causing the model to sample less likely characters. This often yields more surprising or creative outputs, but also more gibberish.

**Example with War and Peace model:**
**Input**: `"and he said "`
- **Temperature 0.5**:
    ```
    a fathola
...
Using the alphabet for quick experiments and War and Peace to explore RNN limitations in long-range dependencies enhanced my understanding of hyperparameters, sequence length, and hidden state size on training and text generation quality.

5. ScreenShots

![War And Peace Screenshot](https://media.discordapp.net/attachments/778325727006687242/1366915113256489004/image.png?ex=6814a8a7&is=68135727&hm=fb27456f81a722c3ec1cd3d494b93773f5813b44b2c37900981b86188a66d09c&=&format=webp&quality=lossless)

![Alphabet Screenshot](https://media.discordapp.net/attachments/778325727006687242/1366920165945774121/image.png?ex=6814ad5b&is=68135bdb&hm=4d54adcd14afc5285f08ec4650f6ce66f515538ab8070e523f41c9626f6352db&=&format=webp&quality=lossless)

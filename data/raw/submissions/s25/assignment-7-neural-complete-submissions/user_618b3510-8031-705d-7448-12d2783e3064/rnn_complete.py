...
class CharDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        seq = self.sequences[idx]   # already a tensor slice
        tgt = self.targets[idx]
        # use as_tensor so it doesn’t copy when given a tensor
        return (
        torch.as_tensor(seq, dtype=torch.long),
        torch.as_tensor(tgt, dtype=torch.long),
    )
...
# TODO: Implement a test loop to evaluate the model on the test set
    ...
    print(f"Epoch {epoch+1}, Test  Loss: {avg_test_loss:.4f}, Test Acc: {accuracy:.4f}")
    model.train()
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    input_idxs = torch.tensor(
        [[char_to_idx[ch] for ch in start_text]],
        dtype=torch.long,
        device=device
    )  
    ...
    hidden = None
    with torch.no_grad():
        _, hidden = model(input_idxs, hidden)
    
    generated = list(start_text)
    last_input = input_idxs[:, -1].unsqueeze(1)  
    ...
    for _ in range(k):
        ...
        idx = sampled_idx.item()
        
        generated.append(idx_to_char[idx])
        
        last_input = sampled_idx
    
    return "".join(generated[len(start_text):])

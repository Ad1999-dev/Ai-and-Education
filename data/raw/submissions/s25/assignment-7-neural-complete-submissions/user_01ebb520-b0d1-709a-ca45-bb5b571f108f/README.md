...
Many of the complications I ran into during my tuning of the hyperparameters was from trying to accommodate for the War and Peace text. Due to the massive size of this text, it was very challenging to maintain a balance between model accuracy and model training runtime. My first experiments with the War and Peace text involved the following hyperparameters:
...
These hyperparameters excelled in delivering a model quickly... it took under 1 minute for the neural network to be created. Unfortunately, this speed compromised the model accuracy way too much. For instance, using this model, a query asking for the next letter after the sequence "th" with the lowest possible randomness(1.0), the result returned was "thh", which obviously is wrong as it probably should be recommending "the". This model is clearly not close to useful, so I pivoted to creating a model that took more time to create, but would hopefully be more accurate:
...
These hyperparameters did not deliver a model very quickly... it took around 3-4 minutes for the training and testing to be completed. However, once the model was created, it was much more accurate than the previous model. For instance, the same query asking for the next letter after "th" returned the string "thi", which is a plausible solution. It may not be 100% correct(I expected "the"), but it definitely isn't wrong like "thh". I then tried the same query but asking for the next 2 letters instead of the next 1 letter, and I got "that", a clearly common English word. Unfortunately, even on the lowest randomness, the model recommendations weren't very consistent. I tried calculating the previous query multiple times, and I got a whole range of answers. Some were true words, but some were strange and highly suspect. The answers I got in 7 attempts were: "that", "theg", "thel", "thir", "thac", "them", and "thim". 

Clearly, this model could be made more accurate. For the purposes of this investigation, I won't indulge in trying to create more accurate models because the runtime is simply too long because the text is too huge.


2. Analysis on final train and test loss for both datasets

For the "abcd" dataset, my final train and test losses were: 
- Train: 0.3447
- Test: 0.1940

For the "war_and_peace" dataset, my final train and test losses were:
- Train: 1.9559
- Test: 1.8386

For the alphabet dataset, the text generated was always much more accurate. This is because these is no inconsistency in what letters we expect to see if we are ever given an input string that is contained in the text. After the letters "abc", we always see the letter "d", so any model with a reasonable stride hyperparameter will make the correct recommendation. This contrasts with the war_and_peace dataset which has many, many more words with much more text variation. Its much more difficult to make recommendations when there are more possible characters with reasonably high probabilities of occurring given an input string. This is why the train/test errors are much higher for the war_and_peace dataset than the alphabet dataset.

3. Explain impact of changing temperature

Changing the temperature gives some level of user control over the "creativity" of the text generation. We can think of it as up-weighting letter recommendations that have a lower probability of occurring. The lower the temperature, the more the model predicts the most likely suffixes. 

4. Reflection

The process of implementing this RNN deepened my understanding and appreciation of the challenges of sequence modeling. I learned that proper selection of hyperparameters is critical as it effects the entire model learning process. I also learned that the right hyperparameters always depend on the data you are trying to model for, as the best RNN hyperparameters were much different for the two datasets.

5. IMPORTANT: Include screenshot of output for generate_text() function: 

![](Screenshot%202025-05-05%20at%2012.03.46%E2%80%AFPM.png)
...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        self.b_h = nn.Parameter(torch.zeros(hidden_size)) #bias
    ...
    def forward(self, x, hidden):
        ...
        logits = torch.matmul(output, self.W_hy) + self.b_y # [b, l, vocab_size]
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    """
    model.eval()
    start_text = start_text.lower() #make everything lowercase
    indices = [char_to_idx[c] for c in start_text[-n:]]
    ...
    for _ in range(k):
        ...
        sampled_idx = sample_from_output(last_logits, temperature).item()
        indices.append(sampled_idx)
        input_seq = torch.tensor(indices[-n:], dtype=torch.long).unsqueeze(0).to(device)

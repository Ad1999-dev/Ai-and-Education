...
Builds dictionaries that track how often each character appears after specific sequences of characters. I slide a window of size 1 to n across the document, and for each position, I count the occurrence of the current character given its preceding context. For a unigram (n=1), I  count how often each character appears. For bigrams (n=2), I count how often each character appears after a specific previous character, and so on for higher values of n. This gives  the raw frequency data needed to calculate probabilities for predicting the next character.
...
1. ***Write down your probability table 2***:
   - as in your probability table should look like (wait a second, you should know what I'm talking about)
        ...
        used ! because | wasn't showing correctly in readme
...
[f(x_1,…,x_t,x_t+1)]/[f(x_1,…,x_t)]
​
 
### Code analysis


The function calculates the probability of char following sequence using frequency tables. It truncates the sequence to the last n-1 characters, selects the appropriate table based on context length, checks if char and sequence exist in the table, computes the total occurrences of the context, and returns the ratio of joint frequency to context frequency. It returns 0.0 for unseen data and adapts dynamically to context length.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - *Show your work*
   P(a)⋅P(a∣a)⋅P(a∣aa)= (11/20) * (5/10) * (1/4) = 55/800
   =0.06875

2. $P(X_1=a, X_2=a, X_3=b)$
    P(a)⋅P(a∣a)⋅P(b∣aa)= (11/20) * (5/10) * (2/4) = 110/800
    =0.1375

3. $P(X_1=a, X_2=a, X_3=c)$
   P(a)⋅P(a∣a)⋅P(c∣aa) =  (11/20) * (5/10) * (1/4) = 55/800
   =0.06875
...
Predicts the next character by iterating over the vocabulary, calculating each character’s probability using calculate_probability, and selecting the one with the highest probability. If no valid prediction is found, it recursively shortens the context or defaults to the most frequent unigram, ensuring robustness against unseen data.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  Predicted next character: b (highest probability = 0.5).
...
With Alice in Wonderland, n=1 (single characters) worked poorly, often guessing random letters, while n=2 captured basic patterns like "th --> e". War and Peace needed at least n=3 to handle complex names ("Nat --> a" for Natasha). Going to n=4 helped with phrases ("Prince V --> a") but slowed predictions noticeably. Both texts showed n<2 was too simplistic, while n>3 created relevant responses.

...
class CharDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target = torch.as_tensor(self.targets[idx], dtype=torch.long)
...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        self.W_eh = nn.Parameter(torch.randn(embedding_dim, hidden_size) * 0.01)
        self.W_hh = nn.Parameter(torch.randn(hidden_size, hidden_size) * 0.01)
        self.b_h = nn.Parameter(torch.randn(hidden_size) * 0.01)
        self.W_ho = nn.Parameter(torch.randn(hidden_size, output_size) * 0.01)
        self.b_o = nn.Parameter(torch.randn(output_size) * 0.01)
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            x_t = x_embed[t]
            h_t = torch.tanh(x_t @ self.W_eh + h_t_minus_1 @ self.W_hh + self.b_h)
            output.append(h_t)
            h_t_minus_1 = h_t
        ...
        logits = output @ self.W_ho + self.b_o # [b, l, vocab_size=v] 
...
sequence = read_file("warandpeace.txt")[:len(read_file("warandpeace.txt")) // 8] #half of war and peace
...
sequence_length = 60 # Length of each input sequence
...
with torch.no_grad():
    for batch_inputs, batch_targets in test_loader:
        ...
        loss = criterion(logits.view(-1, output_size), batch_targets.view(-1))
...
if len(test_loader) > 0:
    ...
    print("Warning: test_loader is empty. Check your split or batch_size.")
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    input_idxs = [char_to_idx[c] for c in start_text]
    input_tensor = torch.tensor(input_idxs, dtype=torch.long).unsqueeze(0).to(device)
    hidden = None
    for _ in range(k):
        logits, hidden = model(input_tensor, hidden)
        last_logits = logits[0, -1, :]
        sampled_idx = sample_from_output(last_logits.unsqueeze(0), temperature)
        next_char = idx_to_char[sampled_idx.item()]
        start_text += next_char
        input_tensor = torch.cat([input_tensor[:, 1:], sampled_idx.view(1, 1)], dim=1)
    return start_text

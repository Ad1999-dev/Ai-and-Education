...
1. Describe your experiments and observations
  After filling in all the empty sections and getting my model to run, I started testing the performance impact of each hyperparameter by increasing and decreasing them and then running the model with the alphabet.  For the hyperparameters, I saw that increasing the features in the hidden state, increasing the number of epochs, decreasing the stride, and fine tuning the learning rate made the largest impact in terms of increasing accuracy and decreasing the loss of my model.  After tuning for the alphabet sequence my model had a training and test loss of 0.0001.  For the war and peace text, I first decreased the number of epocs to 1 due to the length of it making each one take 15 minutes.  This raised the loss to around 2, so I then increased the h idden size and tried lowering the learning rate, which helped bring the training loss to 1.569 and test loss to 1.595.  This model was much less accurate for generating words compared to generating the alphabet, as it would sometimes generate gibbirish but usually outputted words that didn't actually fit together in a sentence.

2. Analysis on final train and test loss for both datasets
The alphabet sequence had a final train and test loss of 0.001, while the war and peace sequence had a final train and test loss of 1.569 and 1.595 respectively.  This large difference in loss was directly evident when testing each sequence with the model, as while it generated characters from the alphabet with very high accuracy, the text led to a lot of pseudo-words being generated mixed with actual words from the English alphabet.

3. Explain impact of changing temperature
Lowering the temperature always made my model predict more accurately.  With a temperature value of around 4 or greater, my model would have varied results with innacurate predictions.  However, lowering this value below 1 decreased the innacuracy of my model in both the alphabet along with the text.  For example, inputting "a" with 5 characters to generate and a temperature value of 5 resulted in the generated text: "fgefl", while repeating with a temperature of 1 resulted in the correct text of "bcdef".  For generating words, this value made a greater impact, with values below 1 generating meaningful words.  For example, inputting "t" with 10 characters to generate and a temperature of 1 resulted in: "thet! was s", while using a temperature of 0.5 generated: "the are and".

4. Reflection
The most difficult part of this assignment was creating and debugging the text generation function.  While developing it I had to think about how to maintain the hidden state between each step while using the model's predictions as inputs.  Also, I kept having a problem where my tensor dimensions were mismatched between the input and sampled indices, which took a while to figure out.  This assignment allowed me to put my theoretical knoweledge to use and apply it to make a well functioning model.  

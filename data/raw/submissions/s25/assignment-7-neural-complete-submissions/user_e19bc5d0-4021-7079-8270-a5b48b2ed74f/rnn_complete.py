...
class CharRNN(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            x_t = x_embed[t]
            h_t = torch.tanh(h_t_minus_1 @ self.W_h + x_t @ self.W_e)
            output.append(h_t)
            h_t_minus_1 = h_t
...
idx_to_char = {i: c for c, i in char_to_idx.items()} # TODO: Create the reverse mapping
...
for epoch in range(num_epochs):
    ...
    for batch_inputs, batch_targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        logits = output.reshape(-1, output_size)
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    indexes = [char_to_idx[char] for char in start_text]
    tensor = torch.tensor(indexes, dtype=torch.long).unsqueeze(0).to(device)
    cur = tensor[0, -1].unsqueeze(0).unsqueeze(0)
    with torch.no_grad():
        _, hidden = model(tensor, hidden)
    for i in range(k):
        logits, hidden = model(cur, hidden)
        logits = logits[:, -1, :]
        sampled = sample_from_output(logits, temperature)
        char = idx_to_char[sampled.item()]
        generated_text.append(char)
        cur = sampled.view(1,1)

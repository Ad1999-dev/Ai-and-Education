...
1. Describe your experiments and observations
  On both datasets decreasing sequence length and increasing batch size seemed to somewhat proportionately decrease training time. For the alphabet hidden size could be kept low as the pattern is very clear but needed to be increased significantly for war and peace. I also trained the first dataset for 10 epochs although by the 5th there was typically very little loss improvement. For war and peace I used 2 epochs to get it below 1.5, perhaps further optimization could get it in a single epoch. Increasing embedding dimensions also helped the loss for war and peace but didnt have much effect for the alphabet. Lastly I had to significantly lower the learning rate for war and peace, compared to what worked for the alphabet.

2. Analysis on final train and test loss for both datasets
  Final loss for the alphabet was 0.0006 after 10 epochs with a test accuracy of 100.00%. Final loss for war and peace was 1.4079 and had a testing accuracy of 51.27% which is not great but in the expected range. The alphabet generations were just ordered continutations of the alphabet whereas the war and peace generations were suposed to be sentences of words but they are mostly nonsense even when the words are spelled right.

3. Explain impact of changing temperature
  Increasing the temperature resulted in more gibberish/random characters being chosen. In the case of the alphabet this meant having out of order predictions and for war and peace it meant "words" were more likely to be nonsense.

| Dataset     | Initial Text | Temperature | # Chr to Gen | Output                         |
|-------------|--------------|-------------|--------------|--------------------------------|
| Alphabet    | hello        | 0           | 10           | pqrstuvwxy                     |
| Alphabet    | hello        | 1           | 10           | pqrstuvwxy                     |
| Alphabet    | hello        | 3           | 10           | pqrstnopqr                     |
| War&Peace   | hello        | 0           | 30           | w her father and the sound of  |
| War&Peace   | hello        | 1           | 30           | wdimanatilling the directly mo |
| War&Peace   | hello        | 3           | 30           | ki!yefr  , t esdat oluzp alw v |


4. Reflection
  One challenge was knowing where and when to tranpose to ensure matrix sizes allowed for multiplication, but this was somewhat easily resolved by tracking matrix sizes. Also understanding the given base code took me a bit but once I got going is was fine. With the sets of weights and hidden layer is it clear how this is a Neural Network, but I also see how it is specifically structured to analyze and learn sequences.

5. Terminal
![image1](./CS_383_A7_Terminal.png)
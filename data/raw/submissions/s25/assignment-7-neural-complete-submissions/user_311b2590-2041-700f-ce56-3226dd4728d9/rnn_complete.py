...
for epoch in range(num_epochs):
    ...
    for batch_inputs, batch_targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        loss = criterion(output.reshape(-1, output_size), batch_targets.reshape(-1)) #calculate loss for this batch
        optimizer.zero_grad()       # clear old gradients
        loss.backward()             # Backpropagate the loss to compute gradients
        optimizer.step()            # Update model parameters using gradients
        total_loss += loss.item()   # Record the loss value
...
print(f"Test Loss: {avg_test_loss:.4f}  |  Test Accuracy: {accuracy:.4%}")
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    input_indices = [char_to_idx.get(c, 0) for c in start_text]
    current_input = torch.tensor([input_indices], dtype=torch.long).to(device)
    ...
    with torch.no_grad():
        ...
        last_logit = logits[:, -1, :]
        next_idx = sample_from_output(last_logit, temperature = temperature)
        
        for _ in range(k):
            ch = idx_to_char[next_idx.item()]
            generated += ch
            
            current_input = next_idx.to(device)

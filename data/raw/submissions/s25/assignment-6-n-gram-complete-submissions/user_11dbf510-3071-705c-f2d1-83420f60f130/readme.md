...
Each frequency table is represented by its own dictionary, with the context characters as the keys. The values are another dictionary, whose keys are characters and whose values keep track of the number of times the particular character was preceeded by the context characters.
This is done by iterating through the input document multiple times, doing various string operations. 
...
1. ***What is your vocabulary in this case***
   - Vocabulary is the set of characters the document is made of, so it is \{a,b,c\}
...
- ***Put the intuition of your code here***
function computes the probability of seeing a character after a given sequence using n-gram frequency tables. It loops through each position in the combined sequence (sequence + char), and for each position, it looks back up to n characters to form a context. Using this context, it retrieves the frequency of the next character (the "numerator") and the total number of times any character followed that context (the "denominator"). It multiplies these conditional probabilities together to get the full joint probability of the sequence. If a context was never seen in the training data, it returns 0.
...
- ***Put the intuition of your code here***
  Iteratres through every character in the vocabulary and then calls calculate_probability on it, storing the character with the maximum result as the next one to be placed in the generation.
...
I found that having a corpus that is larger tends to do better than a smaller corpus. Also, a corpus that contains enough diversity of words to cover the entire alphabet, while not skewing the frequencies to favor any rare characters (for example, a corpus with tons of words with "z" in it will likely not be as helpful as a more balanced corpus), seems to do better. I found that text files upwards of 32275740 (10 times the size of war and peace) began to become quite tedious, though with some patience you could go higher.

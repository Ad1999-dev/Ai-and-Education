...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        self.W_in   = nn.Parameter(torch.randn(embedding_dim, hidden_size) * 0.01)
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            x_t = x_embed[t]
            h_t = torch.tanh(x_t @ self.W_in + h_t @ self.W_hh + self.b_h)
            output.append(h_t)
        output = torch.stack(output) # [l, b, h]
        output = output.transpose(0, 1) # [b, l, h]

        # TODO set these values after completing the loop above
        final_hidden = h_t.clone().detach() # [b, h]
        logits = self.W_out(output) # [b, l, vocab_size=v]
...
# sequence = "abcdefghijklmnopqrstuvwxyz" * 100 # Commented out for Milestone 3
...
idx_to_char = {i: ch for ch, i in char_to_idx.items()} # TODO: Create the reverse mapping
data = [char_to_idx[char] for char in sequence]

# TODO Understand and adjust the hyperparameters once the code is running
# Hyperparameters for Milestone 1 (Alphabet)
# sequence_length = 26
# stride = 1
# embedding_dim = 32
# hidden_size = 64
...
# Hyperparameters for Milestone 3 (War and Peace)
sequence_length = 128   # Length of each input sequence
stride = 16             # Stride for creating sequences
...
model = CharRNN(input_size, hidden_size, output_size, embedding_dim=embedding_dim).to(device) # Pass embedding_dim
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        for t_inputs, t_targets in test_loader:
            ...
            logits, _ = model(t_inputs, None)
            tloss = criterion(logits.reshape(-1,vocab_size), t_targets.reshape(-1))
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    hidden = None
    for _ in range(k):
        input_seq = indices[-sequence_length:] if len(indices) >= sequence_length else indices
        inp = torch.tensor([input_seq], dtype=torch.long).to(device)

        with torch.no_grad():
            logits, hidden = model(inp, hidden)

        next_idx = sample_from_output(logits[0, -1, :].unsqueeze(0), temperature)
        indices.append(next_idx.item())

    generated = ''.join(idx_to_char[i] for i in indices)
    return generated

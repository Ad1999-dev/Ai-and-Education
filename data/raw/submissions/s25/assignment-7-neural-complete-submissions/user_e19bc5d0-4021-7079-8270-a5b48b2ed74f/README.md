...
Using the sequence "abcdefghijklmnopqrstuvwxyz" \* 100 had an accuracy of 1.0 and only took a few seconds to run. Using War and Peace had an accuracy of 0.5629 and took about 15 minutes to run. For the alphabet sequence, in my experiments using the default temperature of 1, the letters generated were always just the letters alphabetical order following the last letter in the input phrase. In order to get decent results for War and Peace, I needed much more complex parameters than the alphabet sequence. The sequence_length, embedding_dim, hidden_size, batch_size were much greater. For War and Peace, I tested with a bunch of common starts to sentences including "I am", "He", "You are", "The", etc. The generated results resembled English but often did not make too much sense. Some generated words were properly formed while others were close to being real words but weren't.

2. Analysis on final train and test loss for both datasets

The sequence "abcdefghijklmnopqrstuvwxyz" \* 100 had a final train loss of 0.0004 and test loss of 0.0003 while War and Peace had a final train loss was 1.4606 and a test loss was 1.4878. The final train and test loss for the alphabet sequence is very similar. Also, the final train and test loss for war and peace is very similar. The alphabet sequence has a much lower loss than war and peace because the text is very simple and has the same patterns which is easy for the RNN to memorize. In contrast, war and peace is very complex which makes it harder to predict.

3. Explain impact of changing temperature

For the alphabet sequence, the input text "cde" produced "cdefghij" with a temperature of 1 and "cdeccrhc" with a temperature of 10. For War and Peace, the input text "he" produced "hed which is" with a temperature of 1, "he hiswhh, g" with a temperature of 2, and "hed. she was" with a temperature of 0.5. Lower temperatures like 0.5 and 1 produced more predictable results that made more sense and better follow the patterns of the text. Higher temperatures are more random, variable, and incoherent.

4. Reflection

The biggest challenge I faced was finding good hyperparameters for the different texts. This was especially hard for War and Peace because each run testing new hyperparameters took a long time. Another challenge was debugging when the dimensions of the matrices were mismatched. During implementation, I first prioritized making sure the model worked on the alphabet sequence since using that text is a lot more predictable and easier to judge the correctness. The biggest insights I gained about RNNs and sequence modeling is how the hidden state propagates the context and how much of a difference the hyperparameters make in having a good model.

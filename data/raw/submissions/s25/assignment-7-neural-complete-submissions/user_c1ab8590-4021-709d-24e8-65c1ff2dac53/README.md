...
I trained with 2 datasets warandpeace and "abcdefghijklmnopqrstuvwxyz” × 100, for abc... I used Hyperparameters: sequence_length=50, stride=1, embedding_dim=10, hidden_size=32, lr=0.01, batch_size=64, epochs=1
and I got train loss = 1.0934, test loss = 0.0354, observations:The model learned the simple repeating pattern almost perfectly. At T=1.0 it continued “cde…” with the next five letters; at T=0.5 it was even more deterministic; at T=2.0 it produced random gibberish.

For warandpeace:  Hyperparameters: sequence_length=100, stride=5, embedding_dim=64, hidden_size=128, lr=0.002, batch_size=128, epochs=1 Results: train loss ≈ 1.6209, test loss = 1.6209 and Observations: The model captured some English word structure but made frequent odd word choices. At mid‐temperature it produced fairly coherent but imperfect prose; at lower temperature the text became overly repetitive; at high temperature it fell into nonsense



2. Analysis on final train and test loss for both datasets

| Dataset| Train Loss | Test Loss |
|----|---|---|
| Alphabet × 100          | 1.0934     | 0.0354    |
| War and Peace (only 1 epoch) | 1.6909     | 1.6209    |

The extremely low losses on the toy alphabet task (train 1.09 → test 0.04) show that the RNN essentially memorized the simple repeating pattern and generalized perfectly to held‑out sequences. In contrast, on War and Peace both train and test loss settled around 1.62, indicating that after one epoch the model had learned some character‐level structure but was still far from “solving” natural language prediction. The fact that train and test losses are nearly identical in both cases tells us there’s little overfitting—the model’s capacity and training schedule were well matched to each dataset’s complexity. However, the higher absolute loss on War and Peace reflects the increased difficulty of modeling long‐range dependencies and rich character interactions in real text.

3. Explain impact of changing temperature
Low (0.5): very conservative, highly repetitive output (e.g. “cdefgh…” or “…and a passed the countess were alone…”).
Medium (1.0): balanced—largely coherent next characters/words with occasional novel surprises. which I found to be most usable among them, but of course depends on what you use it for.
High (2.0): very random, often nonsensical (“cdedestu” in the toy example; garbled non‑words in War and Peace).

4. Reflection
Tuning hyperparameters—especially learning rate and sequence length—was critical, I didn't know how learning rate is so crucial. The toy alphabet confirmed the RNN’s ability to memorize simple periodic patterns, while War and Peace highlighted its struggles with long‐range dependencies in real text. Playing with temperature underscored the trade‑off between safe, repetitive output and creative but error‑prone sequences. Overall, this project deepened my understanding of RNN dynamics, sampling strategies, and the challenges of scaling from synthetic to natural language data. Also, learned the importance of parameters.

5. IMPORTANT: Include screenshot of output for generate_text() function: 
First one is for alphabet second is war and peace
<img src="Screenshot 2025-05-03 at 4.11.55 PM.png" alt="Alphabet generation outputs" width="600"/>
<img src="Screenshot 2025-05-03 at 4.17.43 PM.png" alt="War and Peace generation outputs" width="600"/>
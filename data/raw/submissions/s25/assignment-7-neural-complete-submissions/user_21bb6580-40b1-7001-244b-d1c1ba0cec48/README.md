...
Training the character-level RNN involved two distinct phases. First, the model was trained on a repetitive alphabet sequence (`"abcdefghijklmnopqrstuvwxyz" * 100`). This served as a sanity check to ensure the RNN architecture and training loop were correctly implemented. The goal was to achieve near-perfect prediction, given the highly predictable nature of the input. Second, the model was trained on the `warandpeace.txt` corpus. This presented a significantly more complex task, involving learning the statistical patterns of English text, including vocabulary, grammar, and common character sequences.

Hyperparameter tuning was crucial for both datasets. For the alphabet sequence, relatively smaller `hidden_size` (e.g., 128) and `embedding_dim` (e.g., 50) along with a moderate `learning_rate` (e.g., 0.001) and sufficient `num_epochs` (e.g., 10) allowed the model to quickly converge. For `warandpeace.txt`, a larger model capacity (`hidden_size=256`, `embedding_dim=100`) was necessary. The `learning_rate` (0.002), `sequence_length` (100), and `batch_size` (256) were adjusted based on experimentation, aiming for significant loss reduction within a single epoch as suggested in the assignment guide. The preprocessing step, which converted text to lowercase and removed non-essential characters, helped simplify the learning task.

**Analysis on final train and test loss for both datasets**

*   **Alphabet Sequence:** As expected, the model performed exceptionally well. The final training loss approached zero (e.g., < 0.1), and the test accuracy was very high (e.g., > 98%). This indicated that the model successfully memorized and predicted the simple repeating pattern.
*   **War and Peace:** Training on the text corpus resulted in significantly higher final loss and lower accuracy compared to the alphabet sequence. A typical final training loss after one epoch was around 1.82, with a **test loss of approximately 1.57 and test accuracy around 53.7%**. This is understandable because predicting the next character in natural language is inherently more complex and ambiguous than predicting the next letter in the alphabet. Achieving perfect prediction is impossible, and the model learns statistical likelihoods rather than deterministic rules. The generated text, while containing English-like words and structures, lacked true coherence.

**Explain impact of changing temperature**

The `temperature` parameter in the `sample_from_output` function controls the randomness of the generated text. 

*   **Low Temperature (e.g., < 1.0, closer to 0):** Makes the output more deterministic and predictable. The model tends to choose the characters with the highest predicted probability. This often results in repetitive or conservative text, sometimes getting stuck in loops.
*   **Temperature = 1.0:** Samples directly from the probability distribution predicted by the model. This provides a balance between coherence and creativity.
*   **High Temperature (e.g., > 1.0):** Increases randomness by scaling the logits, making the probability distribution flatter. This leads to more surprising, diverse, and sometimes nonsensical output, as less likely characters have a higher chance of being selected.

For example, generating from the `warandpeace` model with a low temperature might produce common words or repetitive sequences like "the the the", while a high temperature might produce more jumbled or novel (but often meaningless) character combinations.

**Reflection**

This assignment provided valuable hands-on experience with implementing and training RNNs in PyTorch for sequence modeling. Key challenges included understanding the flow of hidden states through time and batches, debugging the tensor dimensions within the RNN forward pass, and the iterative process of hyperparameter tuning. Tuning for the `warandpeace.txt` dataset was particularly insightful, demonstrating the trade-offs between model capacity, training time, and performance on complex, real-world data. Observing the effect of the temperature parameter highlighted the probabilistic nature of language models and the control we have over the generation process. It solidified the understanding that RNNs learn sequential patterns but generating truly meaningful, long-form text requires more sophisticated architectures and vast amounts of data.

**IMPORTANT: Include screenshot of output for generate_text() function:**

![RNN Output Screenshot](Screenshot%202025-05-02%20at%203.08.56%E2%80%AFAM.png)

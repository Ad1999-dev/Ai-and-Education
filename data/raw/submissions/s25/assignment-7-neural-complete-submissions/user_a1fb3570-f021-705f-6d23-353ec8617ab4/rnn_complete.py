...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        self.Wxh = nn.Parameter(torch.randn(embedding_dim, hidden_size) * 0.01)  # input to hidden
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            h_t = torch.tanh(x_t @ self.Wxh + h_t_minus_1 @ self.Whh + self.bh)  # [b, h]
            output.append(h_t)
            h_t_minus_1 = h_t
        ...
        logits = output @ self.Why + self.by # [b, l, vocab_size=v] 
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    for _ in range(k):
        logits, hidden = model(input_tensor, hidden)  # logits: [1, seq_len, vocab_size]
        last_logits = logits[0, -1]  # get the logits of the last character
        sampled_idx = sample_from_output(last_logits.unsqueeze(0), temperature)  # [1, 1]
        next_char = idx_to_char[sampled_idx.item()]
        
        generated += next_char

        # Prepare the next input by appending sampled index and keeping last n tokens
        input_indices.append(sampled_idx.item())
        input_tensor = torch.tensor(input_indices[-n:], dtype=torch.long).unsqueeze(0).to(device)

    return {
    "input": start_text,
    "generated_only": generated[n:],  # just the new part
    "full": generated  # original input + new chars
}
...
while True:
    ...
    print(f"Generated text: {completed_text['full']}")
    print(f"Only generated: {completed_text['generated_only']}")  
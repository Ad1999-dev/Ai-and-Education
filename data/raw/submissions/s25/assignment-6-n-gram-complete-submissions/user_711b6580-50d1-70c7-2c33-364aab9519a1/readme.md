...
The function builds n different tables that store the frequency counts of character sequences of different lengths, from 1 to n. For each table, you iterate through each doc and extract all possible n-grams of the corresponding length. For unigrams where i = 1, you count how many each individual character appears. When i > 1, you use a nested dictionary where the prefix is the outer key and the suffix is the inner key. The core idea of an n-gram model is to predict the next item in a sequence based on the preceding items. The frequency fount if how many times that specific suffix follows that specific prefix. 
...
P(X₁=x₁, X₂=x₂, ..., Xₙ=xₙ) = P(x₁) · P(x₂|x₁) · P(x₃|x₂) · ... · P(xₙ|xₙ₋₁)

In terms of frequency tables, this is computed as:

P(X₁=x₁, X₂=x₂, ..., Xₙ=xₙ) = f(x₁)/size(C) · f(x₁,x₂)/f(x₁) · f(x₂,x₃)/f(x₂) · ... · f(xₙ₋₁,xₙ)/f(xₙ₋₁)

### Code analysis

- ***Put the intuition of your code here***
The function uses the formula to look up the corresponding frequency counts in the tables to deal with differnt scenarios.
For empty sequences, it calcualtes the unigram probability of the character.
For a sequence longer than the model's context window, it only uses the last n-1 characters
For existing n-grams, it calculates the condition probability from the tables
For unseen n-grams, it applies smoothing to avoid zero probabilities
If a context isn't found, it backs off to a lower-order model by recursively calling the function with a shorter context
### Your Calculations
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - *Show your work*
   P(X₁=a, X₂=a, X₃=a) = P(a) · P(a|a) · P(a|aa)
  = (11/20) · (4/11) · (1/4)
  = (11/20) · (4/11) · (1/4)
  = 0.55 · 0.364 · 0.25
  = 0.05
2. $P(X_1=a, X_2=a, X_3=b)$
   - *Show your work*
   P(X₁=a, X₂=a, X₃=b) = P(a) · P(a|a) · P(b|aa)
  = (11/20) · (4/11) · (2/4)
  = 0.55 · 0.364 · 0.5
  = 0.1
...
3. $P(X_1=a, X_2=a, X_3=c)$
   - *Show your work* 
    P(X₁=a, X₂=a, X₃=c) = P(a) · P(a|a) · P(c|aa)
  = (11/20) · (4/11) · (1/4)
  = 0.55 · 0.364 · 0.25
  = 0.05
...
- ***Put the intuition of your code here***
For each character in the vocabulary, you calculate the probability that it appears after the given sequence using the calculate_probability function. You keep track of which character has the highest probability, and return that character as the prediction. 
...
The next character in the sequence "aa" should be 'b' because it has the highest probability (0.1) compared to 'a' and 'c' which both have probabilities of 0.05.
...
War and Peace produced better predictions because it is longer but took a longer time. When n=3, there was a good balance between time efficiency and prediction accuracy. When n=4 and n=5, time spent went up greatly but predictions improved as well. Higher than n=5 takes too much time to process and is impractical. 
<hr>

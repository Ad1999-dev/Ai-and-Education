...
class CharRNN(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            x_t = x_embed[t]  # [batch_size, embedding_dim]
            h_t = torch.tanh(x_t @ self.W_xh + h_t_minus_1 @ self.W_hh + self.b_h)  # [batch_size, hidden_size]
            output.append(h_t)
            h_t_minus_1 = h_t
        ...
        logits = output @ self.W_hy + self.b_y  # [batch_size, seq_len, output_size]
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    """
    model.eval()
    input_seq = [char_to_idx.get(c, 0) for c in start_text]
    ...
    for _ in range(k):
        ...
        next_idx = sample_from_output(next_logits.unsqueeze(0), temperature)
        next_char = idx_to_char[next_idx.item()]
        generated += next_char
        next_idx = next_idx.view(1, 1)
        input_tensor = torch.cat([input_tensor[:, 1:], next_idx], dim=1)

...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        # RNN parameters
        self.W_ih = nn.Parameter(torch.randn(embedding_dim, hidden_size) * 0.01)  # input to hidden
        self.W_hh = nn.Parameter(torch.randn(hidden_size, hidden_size) * 0.01)    # hidden to hidden
        self.b_h = nn.Parameter(torch.zeros(hidden_size))                         # bias for hidden
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            x_t = x_embed[t]  # [b, e]
            h_t = torch.tanh(x_t @ self.W_ih + h_t_minus_1 @ self.W_hh + self.b_h)  # [b, h]
            output.append(h_t)
            h_t_minus_1 = h_t
        ...
        logits = output @ self.W_ho + self.b_o  # [b, l, v] 
...
char_to_idx = {ch: idx for idx, ch in enumerate(vocab)} # TODO: Create a mapping from characters to indices
...
for epoch in range(num_epochs):
    ...
    for batch_inputs, batch_targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        logits = output.view(-1, output_size)  # [batch * sequence_length, vocab_size]
        targets = batch_targets.view(-1)       # Flatten targets
    ...
    epoch_accuracy = total_correct / total_preds
    print(f"Epoch {epoch+1}, Loss: {total_loss/len(train_loader):.4f}, Accuracy: {epoch_accuracy:.4f}")
...
with torch.no_grad():
    for batch_inputs, batch_targets in test_loader:
        ...
        output, hidden = model(batch_inputs, hidden)
        hidden = hidden.detach()
        logits = output.view(-1, output_size)
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    for _ in range(k):
        with torch.no_grad():
            output, hidden = model(input_tensor, hidden)  # pass hidden

        last_logits = output[0, -1, :].unsqueeze(0)
        next_idx = sample_from_output(last_logits, temperature=temperature).item()
        next_char = idx_to_char[next_idx]
        generated_text += next_char

        input_indices.append(next_idx)
        input_indices = input_indices[1:]  # maintain fixed input length
        input_tensor = torch.tensor([input_indices], dtype=torch.long).to(device)

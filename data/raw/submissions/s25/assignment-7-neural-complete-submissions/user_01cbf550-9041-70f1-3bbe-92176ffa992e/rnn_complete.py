...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        self.embed_to_hidden = nn.Linear(embedding_dim, hidden_size)
    ...
    def forward(self, x, hidden):
        ...
        if hidden is None:
            h_t_minus_1 = self.init_hidden(b).to(x.device)
        else:
            h_t_minus_1 = hidden  # Shape [b, h]
        ...
        for t in range(l):
            ...
            h_t = self.activation(
                self.hidden_to_hidden(h_t_minus_1) + self.embed_to_hidden(x_t)
            )  # Shape [b, h]
            outputs.append(h_t)
            h_t_minus_1 = h_t

        output = torch.stack(outputs)  # [l, b, e]
        output = output.transpose(0, 1)  # [b, l, e]

        # TODO set these values after completing the loop above
        final_hidden = h_t.clone()  # [b, h]
        logits = self.hidden_to_output(output)  # [b, l, vocab_size=v]
...
sequence_length = 40  # Length of each input sequence, prev: 25, 30
stride = 1  # Stride for creating sequences
embedding_dim = 64  # Dimension of character embeddings, prev: 32
hidden_size = 256  # Number of features in the hidden state of the RNN, prev: 64, 128
learning_rate = 0.001  # Learning rate for the optimizer, prev: 0.002
num_epochs = 1  # Number of epochs to train
batch_size = 128  # Batch size for training, prev: 64
vocab_size = len(vocab)
input_size = len(vocab)
output_size = len(vocab)

model = CharRNN(input_size, hidden_size, output_size, embedding_dim=embedding_dim).to(
    device
)
...
for epoch in range(num_epochs):
    ...
    for batch_inputs, batch_targets in tqdm(
        ...
        _, predicted_indices = torch.max(
            output, dim=2
        )  # output shape: [batch_size, seq_len, vocab_size]
        total_train_samples += (
            batch_targets.numel()
        )  # numel gives total number of elements
        total_correct_train += (predicted_indices == batch_targets).sum().item()

    epoch_train_accuracy = (
        (100 * total_correct_train / total_train_samples)
        if total_train_samples > 0
        else 0
    )
    print(
        f"Epoch {epoch + 1}, Loss: {total_loss / len(train_loader):.4f}, Training Accuracy: {epoch_train_accuracy:.2f}%"
    )
...
with torch.no_grad():
    for batch_inputs, batch_targets in test_loader:
        output_logits, _ = model(batch_inputs, None)
        loss = criterion(
            output_logits.reshape(-1, vocab_size), batch_targets.reshape(-1)
        )
        total_loss += loss.item()
        _, predicted_indices = torch.max(output_logits, dim=2)
        correct += (predicted_indices == batch_targets).sum().item()
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    with torch.no_grad():
        ...
        last_char_idx_tensor = input_tensor[:, -1].unsqueeze(-1)  # Shape [1, 1]

        for _ in range(k):
            output_logits, hidden = model(last_char_idx_tensor, hidden)
            # output_logits shape [1, 1, vocab_size]
            # hidden shape [1, hidden_size]

            # Shape becomes [1, vocab_size]
            logits_for_sampling = output_logits.squeeze(1)

            next_char_idx_tensor = sample_from_output(logits_for_sampling, temperature)
            # next_char_idx_tensor shape [1, 1]
            ...
            last_char_idx_tensor = next_char_idx_tensor  # Already has shape [1, 1]

...
This RNN model was trained on 2 data sets. First, the abc... * 100, and second, the warandpeace.txt file, which is much (much) bigger. The alphabet one, since much smaller, was able to train much faster compared to warandpeace, which more epochs. I was able to used around 7-10 epochs for the alphabet, which took maybe 10 seconds to finish training, however with warandpeace, just 1 epoch took over 10 minutes to train. Training was significantly slower due to the size and complexity of the dataset, and the generated text was less structured but showed English-like patterns and pseudo-words. This result was achieved with the parameters of:
- sequence_length: 100
- stride = 3
- embedding_dim: 32
- hidden_size: 128
...
I noticed that decreasing sequence length and increasing stride does cause the model to learn faster, which makes sense because a higher stride cause much faster learning due to more skipping, since there are fewer samples. However, the psuedo-english is not as good as with the first mentioned parameters, since we get strings such as "iressof if more he gan, hoging thee her off woul laded.", which does have some English but not as much as the first one. The rest of the report will be on the first mentioned parameters.

2. Analysis on final train and test loss for both datasets

For the alphabet set, we were able to acheive a very low training loss, which always reached about 0.001%, and the test loss was the same. For warandpeace, training loss was a little higher, at around 1.6, which is still pretty good. The test loss was the same, around 1.6. Even with a higher loss, the model produced text that showed structure and partially formed words, indicating it learned character-level patterns.

3. Explain impact of changing temperature

The temperature parameter significantly affected text generation. At a temperature of 1.0, the model produced coherent pseudo-English phrases like:

"the ring the as had whe sat cometreasent the a comen spor to one his on decled of to righture to to so"

We see that this psuedo English is returned, showing that some words are correctly predicted (in this case correctly meaning they are English words), but overall the sentence doesn't make sense gramatically, and there are some words that are not enlgish (or could be half english half gibberish). Lowering the temperature (e.g., to 0.5) resulted in more repetitive and conservative output, while increasing it (e.g., to 1.5) led to more creative but incoherent text, even adding punctuation such as question marks. This showed how temperature adjusts the randomness in sampling and reflects the model's confidence.

4. Reflection

This project helped me understand how RNNs handle sequential data by propagating hidden state across time. I learned how the difficulty of a task impacts training time and required tuning — simple sequences train easily, but real language needs careful hyperparameter adjustment. I also saw how generation can be fine-tuned by temperature to balance creativity and coherence. Overall, this was a hands-on experience in sequence modeling and training neural networks on textual data.

5. IMPORTANT: Include screenshot of output for generate_text() function: 

Here's a screenshot of the generated text using the warandpeace.txt file. (some lines are duplicated but didn't want to restart the training since it would have taken a long time)

"Generated text" is the text with initial text
"Only Generated" is just the predicted text, without initial text

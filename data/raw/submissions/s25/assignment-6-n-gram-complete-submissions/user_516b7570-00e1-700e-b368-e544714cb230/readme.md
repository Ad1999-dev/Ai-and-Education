...
## 383GPT
Did you use 383GPT at all for this assignment: No
...
- ***or create_frequency_tables: Created a dict, and used a sliding window approach to add sequences to the dictionary, or add 1 to
 the frequency if the sequence is already in dictionary. I also iterated for sliding window lengths from 1 to n. I also added a final total_len variable in the dictionary to call on when I want to get the total length.***
...
- ***For calculate_probability: I applied the N‑gram Bayesian algorithm to each character and its preceding context, calculating the conditional probability that the character will follow the current sequence.
...
P(X1 = a, X2 = a, X3 = a) = P(a) * P(a | a) * P(a | aa) = 0.55 * 0.4545 * 0.20 = 0.05

2. $P(X_1=a, X_2=a, X_3=b)$
P(X1 = a, X2 = a, X3 = b) = P(a) * P(a | a) * P(b | aa) = 0.55 * 0.4545 * 0.40 = 0.10

3. $P(X_1=a, X_2=a, X_3=c)$
P(X1 = a, X2 = a, X3 = c) = P(a) * P(a | a) * P(c | aa) = 0.55 * 0.4545 * 0.20 = 0.05
...
- ***  
For predict_next_char: Goes through every character in a loop, calling the calculate_probability of each character, keeping values for the highest probability and the character with that highest probability. At the end, we return the char with the highest probability
 ***
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - *Your answer* 
  The next character should be b after "aa", since it has the highest probability distribution
...
When testing the corpus files and n values in N-gram language models, smaller files are often more effective due to their speed in generating frequency tables. Although larger files give richer character sequences, they would significantly extend processing times as n increases. For instance, a small corpus, such as a short text or novel, can handle n values of 3 or 4 efficiently. However, for n values of 5 or higher, the computational burden grows, particularly when building frequency tables for large datasets. As n rises, the exponential growth in possible character combinations slows down table calculations. When running on WarandPeace, I was able to get to 8 before I quit out of impatience. 

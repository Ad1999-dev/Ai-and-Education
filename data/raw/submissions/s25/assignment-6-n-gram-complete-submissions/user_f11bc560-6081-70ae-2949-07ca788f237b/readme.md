...
- for n iterations, a new dictionary is created. The document is iterated over, and the appropriate length of the gram is found and added to the dictionary, with the gram string as the key and the frequency as the value incremented. The dictionary is then sorted and put into another defaultdictionary, so that any n-grams that were not found in the document will have a default value of 0. The sorted dictionary is then appended to the list, so that its index is n-1.
...
### Formula
$P(X_i | X_{i-1}, X_{i-2}, \ldots, X_{i-n+1}) = \frac{f(X_{i-n+1}, \ldots, X_i)}{f(X_{i-n+1}, \ldots, X_{i-1})}$

### Code analysis

Joint probability is initialized to 1, and the first term of the formula: f(x_1)/size is found and multiplied by joint probability. Then, the loop is entered, and the formula is carried out, with the numerator being initialized to the appropriate sequence of characters of length n (or the first few characters if we cannot get a substring of that size yet) and denominator as the numerator except the last character. Joint probability is updated for every term.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   = 11/20 x 5/11 x 1/5 = 1/20
2. $P(X_1=a, X_2=a, X_3=b)$
   = 11/20 x 5/11 x 1/5 = 2/20
...
For each char in the vocabulary, the probability of the sequence with that character appended is found. The highest probability and corresponding char are kept and returned.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  The next character should be c
...
I think that the Alice's Adventures in Wonderland corpus consistently gives better results, as in words that appear more often in the actual text itself, than the warandpeace corpus. This might be because the first book has some more simpler language as compared to warandpeace. After doing some testing, n can get quite high. I got up to 100 to still run smoothly and in a reasonable amount of time, but something like 1000 was far too time-consuming.

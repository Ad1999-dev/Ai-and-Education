...
### Code analysis
The goal of the create_frequency_tables(document, n) function is to build a series of frequency tables that count how often different n-grams appear in the document. It starts with unigrams (1-character sequences), then bigrams (2-character sequences), up to n-grams. Each table adds context: unigrams tell you how often a character appears, bigrams show how likely one character follows another, and so on. These tables are useful for building language models that predict the next character based on previous ones.
...
1. ***What is your vocabulary in this case***
   - Write it here 
   From the string above, the vocabulary is:
   {'a', 'b', 'c'}
...
|$P(\odot)$|	Probability value|
| ------ | ------------------|
|$P(b \mid a, a)$|	$\frac{2}{3}$|
|$P(c \mid a, a)$|	$\frac{1}{3}$|
|$P(a \mid a, b)$|	$\frac{1}{1}$|
|$P(b \mid b, a)$|	$\frac{1}{2}$|
|$P(a \mid b, a)$|	$\frac{1}{2}$|
|$P(c \mid a, b)$|	$\frac{1}{1}$|
|$P(a \mid b, c)$|	$\frac{2}{2}$|
|$P(c \mid c, a)$|	$\frac{1}{2}$|
|$P(a \mid c, a)$|	$\frac{1}{2}$|
...
$$
P(x_{t+1} \mid x_1,\ x_2,\ \ldots,\ x_t) = \frac{f(x_1,\ x_2,\ \ldots,\ x_t,\ x_{t+1})}{f(x_1,\ x_2,\ \ldots,\ x_t)}
$$

For example, in a trigram model:

$$
P(x_3 = c \mid x_1 = a,\ x_2 = a) = \frac{f(a,\ a,\ c)}{f(a,\ a)}
$$

### Code analysis

- ***Put the intuition of your code here***
The function calculate_probability(sequence, char, tables) calculates the probability of a given character appearing after a specific sequence of characters. It uses the n-gram frequency tables generated earlier to find the longest matching context (like a bigram or trigram) and computes the conditional probability based on that. If the longest context isn’t available, it automatically falls back to a shorter one (e.g., from trigram to bigram, or bigram to unigram). This approach allows the model to make accurate next-character predictions based on the most relevant context found in the training data.

Let me know if you want this even shorter or more casual!
...
1. $P(X_1 = a,\ X_2 = a,\ X_3 = a)$  
   - $P(a) = \\frac{11}{20}$  
   - $P(a \\mid a) = \\frac{1}{6}$  
   - $P(a \\mid a,\ a) = \\frac{0}{3}$  
   - $P(a,\ a,\ a) = \\frac{11}{20} \\cdot \\frac{1}{6} \\cdot 0 = 0$

2. $P(X_1 = a,\ X_2 = a,\ X_3 = b)$  
   - $P(a) = \\frac{11}{20}$  
   - $P(a \\mid a) = \\frac{1}{6}$  
   - $P(b \\mid a,\ a) = \\frac{2}{3}$  
   - $P(a,\ a,\ b) = \\frac{11}{20} \\cdot \\frac{1}{6} \\cdot \\frac{2}{3} = \\frac{11}{180}$

3. $P(X_1 = a,\ X_2 = a,\ X_3 = c)$  
   - $P(a) = \\frac{11}{20}$  
   - $P(a \\mid a) = \\frac{1}{6}$  
   - $P(c \\mid a,\ a) = \\frac{1}{3}$  
   - $P(a,\ a,\ c) = \\frac{11}{20} \\cdot \\frac{1}{6} \\cdot \\frac{1}{3} = \\frac{11}{360}$
...
- ***Put the intuition of your code here***
The predict_next_char(sequence, tables, vocabulary) function is designed to return the most probable next character given a preceding sequence, using an n-gram language model. Internally, it iterates through every possible character in the vocabulary and appends each candidate to the current sequence. For each such extended sequence, it calls calculate_probability() – a function that computes the probability of that sequence based on the frequency tables of the trained model. As it loops through the options, predict_next_char keeps track of which character yields the highest probability. It typically initializes the “best” character as 'a' as a default fallback (in case no character has a higher score or if all probabilities are equal/very low). By the end of the loop, the character with the highest computed probability is stored as the best candidate. The function then returns this best-scoring character as the prediction. In summary, predict_next_char systematically evaluates each possible next character using the model’s probabilities and selects the one with the maximum likelihood, defaulting to 'a' only if none surpass it.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - *Your answer*
 The next character should be b, because from the trigram table, P(b | a, a) = 2/3 is the highest among all possible next characters. This makes b the most likely continuation of the sequence aa.
## Experiment
...
We carried out experiments by training n-gram models with different values of n (for example, n = 2, 3, 4, 5) on various corpus sizes, and observed how these changes affect the model’s performance. The results demonstrate a clear trade-off. With a smaller training corpus, lower-order n-grams (like bigrams, n=2) tend to work better because they require fewer observations to estimate probabilities reliably. In a tiny dataset, a bigram model can still make reasonable predictions since most 2-character combinations might appear enough times. In contrast, a higher-order model such as a 5-gram (n=5) on the same small corpus would suffer from extreme data sparsity – many specific 5-character sequences never occur in training, leading to zero or unreliable probabilities for those unseen sequences. Thus, on limited data, simpler models (lower n) generalize better and are less likely to encounter unknown contexts. On the other hand, if we increase the corpus size, higher-order n-grams begin to shine. With a large enough dataset, a trigram or 5-gram model can capture more context nuances for each prediction, often improving accuracy because it considers a longer history. In a very large corpus, the model has likely seen most of the relevant 5-length contexts, so an n=5 model can leverage that detailed context to make more informed choices than a bigram model, which might be too simplistic. In summary, lower n works better on small data, whereas higher n works better if you have more data – the choice of n should match the amount of training data available. There are also practical computational considerations. Increasing n makes training slower and the tables larger. This is because a higher-order model must count and store a vastly greater number of n-gram combinations. For example, a trigram model (n=3) stores frequencies for every 3-character sequence observed, whereas a 5-gram model stores frequencies for 5-character sequences – potentially an enormous list if the corpus or vocabulary is sizable. As n grows, the memory required for the tables (which hold probability distributions for each (n−1)-length context) grows exponentially, and collecting these counts from the corpus involves more computation. Training a 5-gram model may take significantly longer than training a bigram model on the same data due to this overhead. In practice, one must balance the desire for a more expressive model (higher n) with the available data and resources. Too high an n for a given corpus can lead not only to overfitting or sparsity issues but also to inefficient training and storage. Therefore, choosing n is about finding the sweet spot: a model that is complex enough to capture meaningful context but simple enough to be supported by the dataset (and computationally feasible).

...
- For each number from 1 to n, split the document into i-grams and put them into a set, then for each i-gram in the set, find it's frequency in the document, and put them in a hashmap with the key being the gram, and the value being the count. After you've finished with the hashmap, place it in the frequency_tables array.
...
2. ***Write down your probability table 3***:
    ...
    | $P(a \mid b \mid b)$ | $\frac{0}{18}$ |
    | $P(a \mid b \mid c)$ | $\frac{2}{18}$ |
    | $P(a \mid c \mid a)$ | $\frac{0}{18}$ |
    | $P(a \mid c \mid b)$ | $\frac{1}{18}$ |
    | $P(a \mid c \mid c)$ | $\frac{1}{18}$ |
    | $P(b \mid a \mid a)$ | $\frac{1}{18}$ |
    | $P(b \mid a \mid b)$ | $\frac{1}{18}$ |
    | $P(b \mid a \mid c)$ | $\frac{0}{18}$ |
    | $P(b \mid b \mid a)$ | $\frac{0}{18}$ |
    | $P(b \mid b \mid b)$ | $\frac{0}{18}$ |
    | $P(b \mid b \mid c)$ | $\frac{0}{18}$ |
    | $P(b \mid c \mid a)$ | $\frac{2}{18}$ |
    | $P(b \mid c \mid b)$ | $\frac{0}{18}$ |
    | $P(b \mid c \mid c)$ | $\frac{0}{18}$ |
    | $P(c \mid a \mid a)$ | $\frac{2}{18}$ |
    | $P(c \mid a \mid b)$ | $\frac{0}{18}$ |
    | $P(c \mid a \mid c)$ | $\frac{1}{18}$ |
    | $P(c \mid b \mid a)$ | $\frac{1}{18}$ |
    | $P(c \mid b \mid b)$ | $\frac{0}{18}$ |
    | $P(c \mid b \mid c)$ | $\frac{0}{18}$ |
    | $P(c \mid c \mid a)$ | $\frac{1}{18}$ |
    | $P(c \mid c \mid b)$ | $\frac{0}{18}$ |
    | $P(c \mid c \mid c)$ | $\frac{0}{18}$ |
...
### Formula
- $P(X_1 = a, X_2 = a, X_3 = x_3) = P(a) \cdot P(a \mid a) \cdot P(x_3 \mid a) = \frac{f(a)}{20} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(a,x_3)}{f(a)}$

### Code analysis

- Find a table in the tables array with grams the length of sequence plus 1 (for the new character), add up the frequency of every gram in that table, and divide the frequency of the sequence plus the new character by the total frequency for every gram in the table
...
1. $P(X_1=a, X_2=a, X_3=a)$
   ...
   - $P(a) \cdot P(a \mid a) \cdot P(a \mid a)$

   - $\frac{f(a)}{20} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(a,a)}{f(a)}$

   - $\frac{11}{20} \cdot \frac{5}{11} \cdot \frac{5}{11}$

   - $0.1136363636$
2. $P(X_1=a, X_2=a, X_3=b)$
    - $P(a) \cdot P(a \mid a) \cdot P(a \mid b)$

   - $\frac{f(a)}{20} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(a,b)}{f(a)}$

   - $\frac{11}{20} \cdot \frac{5}{11} \cdot \frac{3}{11}$

   - $0.0681818182$
3. $P(X_1=a, X_2=a, X_3=c)$
    - $P(a) \cdot P(a \mid a) \cdot P(a \mid c)$

   - $\frac{f(a)}{20} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(a,c)}{f(a)}$

   - $\frac{11}{20} \cdot \frac{5}{11} \cdot \frac{2}{11}$
...
- for each character char in the vocab, if char has a greater probability of occurring than current character with the greatest probability of occurring, then char is the new character with the greatest probability of occurring
...
Alice in Wonderland worked way faster than warandpeace (perhaps because it's shorter). After n = 2 warandpeace became a little too time consuming, but even at n = 7 Alice in Wonderland was still faster. But for some reason at n = 8, when I used the sequence, "It was the", the predictions just stopped at the letter m. I tried a couple other phrases like "Alice was ", and "There was ", with the first phrase stopping after v and the next phrase stopping after no. I tried increasing n to 9 and 10 to similar results, the predictions would either stop short, or not even start at all.
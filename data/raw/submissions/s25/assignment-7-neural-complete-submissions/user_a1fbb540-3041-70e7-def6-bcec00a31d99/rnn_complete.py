...
class CharRNN(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            # RNN step
            e_t = x_embed[t]  # [b, e]
            h_t = torch.tanh(e_t @ self.W_e + h_t_minus_1 @ self.W_h + self.b_h)
            output_t = h_t @ self.W_o + self.b_o  # [b, output_size]
            
            output.append(output_t)
            h_t_minus_1 = h_t
            
        output = torch.stack(output)  # [l, b, output_size]
        output = output.transpose(0, 1)  # [b, l, output_size]
        
        final_hidden = h_t_minus_1  # [b, h]
        logits = output  # [b, l, output_size]
...
hidden_size = 25        # Number of features in the hidden state of the RNN
learning_rate = .01    # Learning rate for the optimizer
...
for epoch in range(num_epochs):
    ...
    for batch_inputs, batch_targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        loss = criterion(output.reshape(-1, output_size), batch_targets.reshape(-1))
        loss.backward()
        optimizer.step()
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    with torch.no_grad():
        ...
        start_text = start_text.lower()
        input_seq = [char_to_idx[char] for char in start_text[-n:]]  # Use last n characters if input is longer
        input_tensor = torch.tensor(input_seq, dtype=torch.long).unsqueeze(0).to(device)  # [1, n]
        ...
        for _ in range(k):
            ...
            input_seq = input_seq[1:] + [sampled_idx.item()]

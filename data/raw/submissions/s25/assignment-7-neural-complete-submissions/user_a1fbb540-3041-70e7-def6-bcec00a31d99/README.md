...
I experimented with the hyperparameters using my prior knowledge of how each would work. I immediately limited the batch size to 20-50 samples for most of my training loops. This was very effective in increasing accuracy for the model trained on war and peace. The other model doesn't quite require the additional computation because of its simplicity. Adjusting sequence length had pretty intense effects on training time. I didn't find the increase in training time to be worth it, and I was able to get consistently decent results wih sequence lengths of 20-40. Adjusting stride was most useful around the 5-10 mark, I suggest because these are fairly typical word lengths. Anything larger will create risk of removing too much context from each sequence and smaller isn't worth the additional computation. I aimed for stride to be in that 5-10 range mainly because it is an average word length. Learning rates of 0.01 - 0.05 were the most effective for me. embedding_dim and hidden_size were some of the most important hyperparameters. The larger these values are, the more complex context each embedding can hold. This was much less important for the smaller model. For the larger one, these values were kept around 25-50.

2. Analysis on final train and test loss for both datasets

I achieved a loss of 0.005 for the first, smaller dataset. Most likely because it was so simple. As for the one trained on war and peace, I was only able to get loss down to around 2.0. This makes sense, as interpreting the english language is a much more difficult task.

3. Explain impact of changing temperature

Increasing temperature increases the randomness of the generated text sequences. In these examples, because the models already could not generate text with high accuracy, increasing temperatue often resulted in illegible blobs of text.

4. Reflection

Through this experiment, I gained practical insights into how hyperparameter choices directly impact model performance. The alphabet sequence demonstrated the RNN’s ability to learn simple, deterministic patterns with near-perfect accuracy, while War and Peace highlighted the challenges of natural language requiring deeper architectures or more sophisticated approaches to achieve lower loss. Temperature tuning further emphasized the trade-off between predictability and creativity in text generation. Overall, the project reinforced the importance of balancing computational efficiency with model capacity, especially for complex tasks.
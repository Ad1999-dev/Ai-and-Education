...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    if not input_seq:
        return "Error: start_text must contain at least one valid character."
    ...
    for _ in range(k):
        ...
        input_tensor = torch.cat([input_tensor, next_char_idx], dim=1)  # [1, n+1]

    output_seq = input_tensor[0].tolist()

...
I noticed that sequence length had some of the biggest impacts onto training times, as a sequence length of 500 led to an hour spent on training for 5 epochs. meanwhile 100 epochs and it was done in roughly 10-15 minutes. decreasing sequence length also didn't seem to have a major impact on the loss as i fine-tuned other hyperparameters so I ended up using 100 for my sequence length for most of my experimentation. I also noticed that even though training loss would go down consistently, often the test loss would be somewhere inbetween the initial training loss and the last training loss. 


Experiments and observations for the War and peace dataset: 

Test #1

| sequence_length | stride | embedding_dim | hidden_size | num_layers | learning_rate | num_epochs | batch_size |
|---|---|---|---|---|---|---|---|
| 500 | 5 | 2 | 32 | 1 | 0.01 | 5 | 64 |

Training loss was consistenly high and it didnt decrease greatly over the epochs. epoch one had an avg loss of 1.967 and epoch 5 had an avg loss of 1.910. Then, the test loss increased back up to 1.947 with an accuracy of 42.35%. with a accuracy so low it was inevitable that it outputted gibberish. For the initial text of 

"when" and a temperature of 1 I got an output of "when outter war. saminf now offiseath, puupurate the resithout you dityriar whithostiot. i stlath, that" 

so mostly gibberish with some proper words scattered in there, only three letter words which is odd. 

Test #2

| sequence_length | stride | embedding_dim | hidden_size | num_layers | learning_rate | num_epochs | batch_size |
|---|---|---|---|---|---|---|---|
| 100 | 5 | 15 | 64 | 1 | 0.01 | 5 | 64 |

Training loss here started at 1.833 and ended at 1.775 so marginally better and it was also significantly faster to run so based off of this I'm going to assume that a lower sequence_length is ok to use. I also increased the hidden_size and batch_size and will try even higher next run. With a slightly higher accuracy of 46.17% the output this time around from the initial text of "when" was: 

"when and in twr could nightaky to moscow domgthk. unattryed to hig his rostv in the looke pame thyords, w". 

Slightly more real words on average, and the "could" indicates that it can generate longer words than the three letter words from eariler.

I realized after this run that my embedding dim has been way too low so for the next test I went back to a embedding_dim of 15 and also increased hidden_size, decreased learning_rate and increased batch_size. 
...
| sequence_length | stride | embedding_dim | hidden_size | num_layers | learning_rate | num_epochs | batch_size |
|---|---|---|---|---|---|---|---|
| 100 | 5 | 10 | 32 | 1 | 0.01 | 5 | 32 |

training loss was extremely low as expected, going from 2.45 avg loss in the first epoch all the way down to 0.0320 avg     in the last epoch. Then the test accuracy came out to be 100% with an average test loss of 0.0012, so 0.0005 lower than the lowest of the training losses. 

The output generate also does look good, consistently generating the alphabet correctly and looping back through the alphabet after generating a z. 


War and peace dataset final test:
Test #3

| sequence_length | stride | embedding_dim | hidden_size | num_layers | learning_rate | num_epochs | batch_size |
|---|---|---|---|---|---|---|---|
| 100 | 5 | 15 | 128 | 1 | 0.001 | 5 | 128 |

Training loss this time around started at 1.6848 and ended at 1.4258 with the testing loss at 1.4926 and 55.28% accuracy. the resulting words are: 

"when from the equamal pindspamty gun im seemed an armession. but its advance, prince., ordered something" 

Once again increasing the number of proper words and also more incorrect words that seem like they could almost seem like words. 
   
3. Explain impact of changing temperature

Since that last test had a bit of a higher accuracy I also decided to start trying different temperatures when I had been defaulting to 1. A temperature of 0.7 granted this output: 

"when about the kellow and was to have nicholas and the battles were bend and smile, or the german in a w" 

then, a temperature of 0.5 granted this output: 

"when with the same to the emperor. when recess and and three not see the order to seen and there was all"

As temperature decreases the more the model takes its most confident output so as I decreased the temperature the more I got proper words and less of the amalgamations of similar words. 

4. Reflection

One mistake I made while trying to fine tune the parameters was not properly reading up on what each of the hyperparameters did, because I probably could have had a much better initial test and much faster training time if I realized what each hyperparameter actually did. It was also interesting comparing this with our nlp and pytorch assignments as this was sort of an combination of both. Especially comparing it to the nlp project was interesting since it looks like the two would be comparable from an outside point of view but really the way they work is still quite different. 

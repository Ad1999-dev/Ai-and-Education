...
## 383GPT
Did you use 383GPT at all for this assignment (yes/no)? Yes to write tests
...
My approach for the task to build multiple tables that track how often characters appear in the document, both individually and in sequences was: I start with the simplest case - counting individual characters in the unigram table. Then I build increasingly complex tables for longer sequences.
For each n-gram size (from 2 to n), I slide a window of that length through the document and increment the count for each sequence I find. I use tuples as dictionary keys since they're immutable, which makes them perfect for storing character sequences. This sliding window approach ensures I capture all possible sequences of the right length from the document.
The result is a list of tables where each table provides frequency information at different levels of context - from single characters all the way to sequences of length n.
...
### Formula
P(char|seq) = count(seq+char) / count(seq)
### Code analysis

I first find how much of the input sequence I can use based on the n-gram tables. If the sequence is too long, I take the last k characters where k is the maximum context size of model.
For the unigram case of no preceeding sequence, I simply lookup how often the character appears divided by the total character count.
For longer sequences:
find how often the full sequence+character appears 
find how often the prefix sequence appears followed by any character 

then loop through the appropriate n-gram table and sum up counts for all entries that start with our prefix. This gives  the total occurrences of the prefix followed by any character neede for conditional probability.
The result is P(char|sequence)
### Your Calculations
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - to find is P(a|aa)
   - aaa appears once
   - seq with aa and any character after is 4
   - therefore 1/4 = P(a|aa) = 0.25
...
3. $P(X_1=a, X_2=a, X_3=c)$
   - to find P(c|aa)
   - aac appears once
   - seq with aa and any char after is 4
   - 1/4 = P(c|aa) = 0.25
...
This function builds on the previous onces and evaluates the maximum probability to predict the next character of the sequence. Calculate the probability for each possible next character in our vocabulary and then pick the one with the highest probability.
Use a dictionary to store all the calculated probabilities, then use Python's max() function to find the character with the highest probability value. 
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  ...
  - Since b has highest probability of 0.5 > 0.25. Therefore, should be b next
...
For small corpus files like test examples, calculation is efficient even with higher n values (n=5, n=6). However, with larger corpus files, the memory space required increases exponentially as n increases. n=3 or n=4 provides good prediction accuracy without excessive computational. Beyond n=4, the tables became very sparse with many sequences that dont appear in the data and computation time keeps increasing.
The corpuses with more repetitive patterns work best with smaller n values (n=2) since the patterns are simple and often. complex corpus files with natural language text need higher n to capture true and natural flow of sequence of text. This requires great computation and isn't possible from such a simple model if you are aiming for high accuracy.
You can try running up till n=5 .
<hr>

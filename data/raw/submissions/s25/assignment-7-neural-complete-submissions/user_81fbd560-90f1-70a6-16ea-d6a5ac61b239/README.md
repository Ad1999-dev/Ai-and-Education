...
Please submit to Assignment 7 Neural Complete on Gradescope.
...
In this experiment, we implemented a character-level RNN to generate text based on sequential character predictions. The model was trained on a repeating sequence of characters over five epochs, using a sequence length of 100 and a stride of 10 to create overlapping input-output pairs. Throughout training, we observed a steady decrease in loss, indicating effective learning of character relationships. After training, the model was tested to evaluate its generalization ability, with test loss being monitored to check for potential over-fitting.

2. Analysis on final train and test loss for both datasets:
The final training loss decreased significantly, suggesting strong performance on the training set. In contrast, the test loss was higher, indicating that the model may not generalize effectively to previously unseen data. The gap between the training and test losses implies potential over-fitting, where the model memorizes the training data instead of learning generalizable patterns. Ideally, training and test losses should be similar to demonstrate effective learning.

3. Explain impact of changing temperature:
The "temperature" parameter affects the randomness of character selection during text generation. A low temperature (< 1.0) sharpens the output probability distribution, making the model favor more likely characters, resulting in more repetitive and deterministic text. Conversely, a high temperature (> 1.0) flattens the distribution, introducing greater randomness and variability in the generated text, which can yield more creative and diverse outputs but may also lead to inconsistency.

4. Reflection:
Overall, this experiment provided valuable insights into the capabilities and limitations of character-level RNNs for text generation. The observed trends in training and test losses highlighted the importance of balancing model complexity and generalization. Future experiments could involve tuning hyper-parameters further, exploring larger datasets, or implementing techniques like dropout to reduce over-fitting. Additionally, experimenting with various temperature settings during text generation showcased how model behavior can be tailored, underscoring the flexibility of RNNs in creative applications.
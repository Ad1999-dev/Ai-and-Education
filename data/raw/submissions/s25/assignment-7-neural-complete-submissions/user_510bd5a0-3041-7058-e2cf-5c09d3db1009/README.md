...
To start, I trained the character-level RNN on a simple repeating alphabet sequence ("abcdefghijklmnopqrstuvwxyz" * 100). Since the sequence is only 26 characters long, I used a sequence_length of 25 to allow the model to see nearly a full cycle in each input. The model learned this pattern extremely quickly. Loss dropped to near-zero in just a few epochs, and the generated text showed perfect alphabet continuations, like "def" → "defghi".

After verifying the model on the simple dataset, I moved to training on warandpeace.txt. To reduce training time, I used only the first 100,000 characters. I updated hyperparameters to handle the increased complexity, including a longer sequence_length, higher hidden_size, and larger embedding dimension. Training was slower, but after a single epoch the model began generating English-like outputs. These included common word patterns and plausible phrases, demonstrating the RNN’s ability to model real language to a reasonable degree.

2. Analysis on final train and test loss for both datasets
The final training and test loss values clearly reflected the complexity of each dataset. For the alphabet dataset, the model achieved a training loss of approximately 0.0004 and a test loss around 0.0003. This outcome confirmed that the model had fully learned the repeating sequence and generalized it perfectly. In contrast, the War and Peace dataset, even limited to just 100,000 characters, was significantly more challenging. After one epoch of training, the model reached a training loss of about 2.31 and a test loss of roughly 2.08. The relatively small difference between these values indicated that the model was learning meaningful structure rather than simply memorizing the training data, which is a promising sign given the complexity of English text. On the full dataset, performance improved significantly with a training loss of 1.52 and a test loss of 1.48, indicating that the model was successfully learning language structure beyond memorization.

3. Explain impact of changing temperature
During text generation, I observed a clear impact from changing the temperature parameter. At lower temperatures, such as 0.5, the model produced more repetitive and stable outputs, generally favoring the most probable next characters. For example, the prompt “war” generated the continuation “warkess one a,” which, while not meaningful, resembled plausible English structure and word formation. At a temperature of 1.0, the model produced more creative and varied outputs. For example, the prompt “fig” generated “fighe vet ad,” which included less predictable character sequences and a more diverse vocabulary. These observations demonstrated how adjusting temperature allows the model to trade off between coherence and creativity, with higher values introducing more randomness into the sampling process.

4. Reflection
This assignment provided a hands-on opportunity to deeply understand how recurrent neural networks model sequences at the character level. By implementing the RNN logic, including embedding layers and recurrence equations, I developed a much stronger grasp of how hidden states evolve and how temporal dependencies are captured in a model. Training on both a synthetic dataset and real English text highlighted the difference between learning memorized patterns and modeling natural variability. I also saw firsthand how hyperparameters and sampling techniques like temperature influence both training efficiency and output diversity. Overall, this project helped solidify both my theoretical and practical understanding of sequence modeling with RNNs.

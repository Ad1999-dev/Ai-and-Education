...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        self.W_e = nn.Parameter(torch.randn(hidden_size, embedding_dim) * 0.01)  # input weights
        self.W_h = nn.Parameter(torch.randn(hidden_size, hidden_size) * 0.01)  # recurrent weights
        self.W_o = nn.Parameter(torch.randn(output_size, hidden_size) * 0.01)  # output weight
        
        # Initialize biases
        self.b_h = nn.Parameter(torch.zeros(hidden_size))  # bias for the hidden
        self.b_o = nn.Parameter(torch.zeros(output_size))  # bias for the output
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            x_t = x_embed[t]  # [b, e] now
            # rnn recurrence step using lecture formula - f(W_h*h_t-1 + W_e*e_t), 
            # note that we just included a bias term, b_h, and the function we are using is tanh
            h_t = torch.tanh(h_t_minus_1 @ self.W_h.T + x_t @ self.W_e.T + self.b_h)  
        ...
        logits = output @ self.W_o.T + self.b_o # [b, l, vocab_size=v] 
...
sequence_length = 50   # Length of each input sequence , use 120 for warandpeace
stride = 10            # Stride for creating sequences, use 5
embedding_dim = 32     # Dimension of character embeddings, use 32
hidden_size = 64       # Number of features in the hidden state of the RNN, use 128
learning_rate = 0.1    # Learning rate for the optimizer, use 0.005
num_epochs = 10        # Number of epochs to train, use 2
batch_size = 64        # Batch size for training, use 64
...
with torch.no_grad():  
    for batch_inputs, batch_targets in test_loader:
        ...
        logits, final_hidden = model(batch_inputs, None) 
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    for _ in range(k):
        ...
        with torch.no_grad():
            ...
            # get next char index
            sampled_char_idx = sample_from_output(last_logits, temperature=temperature).item()
            sampled_char = idx_to_char[sampled_char_idx]
            generated += sampled_char

            # update the input_tensor since our context window is n
            input_indices.append(sampled_char_idx)
            input_tensor = torch.tensor(input_indices[-n:], dtype=torch.long).unsqueeze(0).to(device)

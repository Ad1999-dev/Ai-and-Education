...
Going about training the RNN model for both the sequence "abcdefghijklmnopqrstuvwxyz" * 100 and the text from warandpeace.txt, I found the latter to be very time consuming. For the former, I trained the model with the hyperparameters of a sequence length of 100, stride of 10, embedding dimension of 30, 50 epochs, hidden size of 64, and learning rate of 0.01. As for the latter, I used similar hyperparameters, except with the number of epochs in this case changing to be 1. On the alphabet sequence, the model performed very well, the training loss reaching value of 0.0005 and testing loss value of 0.0002. The generated text from these tests were also what were to be expected from the training dataset. With the warandpeace.txt dataset, the results differed as the training loss reached values of 1.788 and test loss of 1.787. As for the generated text for these two, the alphabet sequence outputted whatever were the next letters in the alphabet from the last letter inputted. This is to be expected based on the sequence it was trained on. On the other hand, warandpeace trained model outputted phrases and words which were less predictable based on the large dataset it was trained on. The differences of the models using the different datasets highlights how RNN's work with varying complexities and sizes of training data. The warandpeace.txt being a very large file with many different words, structures, and complexities whereas the alphabet sequence being much shorter and simpler, it is to be expected that the training process will be much faster and predictable with the smaller alphabet dataset. This can explain the varying results of the training and testing loss for both datasets.
As for how changing impacts the temperature impacts results, we can see how decreasing the temperature increases the predictability of the results. Suppose a temperature of 7 versus 0.5, the 0.5 would output words and phrases from the text whereas the 1.5 temperature would output characters in seemingly random orders. After running the input sequence of "the" for a length of 5 additional characters, I received the outputs of "the hand" and "the:uszk" for temperature values of 0.5 and 7, respectively. This makes sense as in the code, the logits is computed by dividing by the temperature, a higher temperature value decreasing the logits value, which also adjusts the probability values and promoting the chances of less likely sequences. 
Over the training process, key issues I faced was dealing with errors with wrong tensor sizes, toggling learning rates and other hyperparameters to allow for efficient training. With initial hyperparameters, the training process would take unnecessarily long and implausible to train. This, despite taking time, helped myself to gain insight on the complexities of building RNN models and how intricate these systems are. 
Alphabet Sequence Trained - 
![Screenshot 2025-05-05 160233](https://github.com/user-attachments/assets/b8620203-462b-4c74-91f4-2d7dfa0757c0)
warandpeace.txt Trained - 
![Screenshot 2025-05-05 161345](https://github.com/user-attachments/assets/9e7aba42-e14f-4dac-b1b4-8ce7ef14f6fc)

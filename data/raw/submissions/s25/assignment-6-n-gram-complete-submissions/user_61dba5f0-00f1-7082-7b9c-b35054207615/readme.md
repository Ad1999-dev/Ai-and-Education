...
- First I created a unigram table since it is the easiest and all I had to do was loop through every character in the document and update the frequency. For larger grams, I used a sliding window method where I loop through the document with a window of how many grams I want the table to be and record the frequency of each segment.
...
- There are three cases that might be possible, the first being that there is no given sequence to work with, so the probability would be the probability of each character from the unigram table based on its frequency. The second case is that after concatenating the sequence and the given char, the entire sequence exists in the table and we can then use the above formula to calculate the probability. The third case is if after concatenating, the sequence does not exist in the table, if this happens, I recursively use a smaller n-gram table until the sequence exists to find the probability.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $P(a,a,a)$ = $P(a \cap a \cap a)$
   - $P(a,a,a)$ = $P((a \cap a) \cap a)$
   - $P(a,a,a)$ = $P(a \mid a \cap a) P(a \cap a)$
   - $P(a,a,a)$ = $P(a \mid a \cap a) P(a \mid a)P(a)$
   - $P(a,a,a)$ = $(\frac{1}{3})$ $(\frac{5}{11})$ $(\frac{11}{20})$
   - $P(a,a,a)$ = $\frac{1}{12}$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $P(a,a,a)$ = $P(a \cap a \cap b)$
   - $P(a,a,a)$ = $P((a \cap a) \cap b)$
   - $P(a,a,a)$ = $P(b \mid a \cap a) P(a \cap a)$
   - $P(a,a,a)$ = $P(b \mid a \cap a) P(a \mid a)P(a)$
   - $P(a,a,a)$ = $(\frac{2}{5})$ $(\frac{5}{11})$ $(\frac{11}{20})$
   - $P(a,a,a)$ = $\frac{1}{10}$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $P(a,a,a)$ = $P(a \cap a \cap c)$
   - $P(a,a,a)$ = $P((a \cap a) \cap c)$
   - $P(a,a,a)$ = $P(c \mid a \cap a) P(a \cap a)$
   - $P(a,a,a)$ = $P(c \mid a \cap a) P(a \mid a)P(a)$
   - $P(a,a,a)$ = $(\frac{1}{5})$ $(\frac{5}{11})$ $(\frac{11}{20})$
   - $P(a,a,a)$ = $\frac{1}{20}$
...
- I just looped through each character, and put it into the calculate_probability function I made earlier and found the one with the highest probability and returned that character.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - b would be the most likely next character since it has the highest probability
...
I found that when using the warandpeace.txt, I get the following timings for varying values of n.

    - when n = 5 - 5 seconds
    - when n = 10  - 14 seconds
    - when n = 15  - 24 sec
    - when n = 20  - 40 sec
    - when n = 25  - 48 sec

I found that the difference in predictions from an n value of 10 vs 20 is barely noticable yet the amount of time it takes is more than double. I found that with an n value of 5, if the length of the words that it begins to predict is too long, like "disposition" for example, it starts to not make any sense really quickly, so with this corpus, where there are large amounts of longer words, I believe that an n value of 8 or above is necessary for predictions to be coherent.

When using the Alice Adventures in Wonderland corpus, I found that it runs significantly faster than the war and peace corpus since it is much shorter. And so I can increase the number of grams I use for the same amount of run time, however, as I mentioned above, using an n value of more than 10 did not significantly improve the quality of the predictions.

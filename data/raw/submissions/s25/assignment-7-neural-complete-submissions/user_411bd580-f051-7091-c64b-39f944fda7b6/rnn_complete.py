...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        k = 1.0 / (hidden_size ** 0.5)              # small uniform init range
        self.W_xh = nn.Parameter(
            torch.empty(embedding_dim, hidden_size).uniform_(-k, k)
        )                                           # input → hidden
        self.W_hh = nn.Parameter(
            torch.empty(hidden_size, hidden_size).uniform_(-k, k)
        )                                           # hidden → hidden
        self.b_h  = nn.Parameter(torch.zeros(hidden_size))          # hidden bias

        self.W_hy = nn.Parameter(
            torch.empty(hidden_size, output_size).uniform_(-k, k)
        )                                           # hidden → vocab
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            x_t = x_embed[t]                                           # [b, e]
            h_t = torch.tanh(                                          # tanh activation
                x_t @ self.W_xh + h_t_minus_1 @ self.W_hh + self.b_h
            )
            output.append(h_t)                                 
            h_t_minus_1 = h_t  
        ...
        logits = torch.matmul(output, self.W_hy) + self.b_y # [b, l, vocab_size=v] 
...
sequence_length = 80 # Length of each input sequence
...
learning_rate = .08    # Learning rate for the optimizer
...
with torch.no_grad():
    for batch_inputs, batch_targets in test_loader:
        batch_inputs, batch_targets = batch_inputs.to(device), batch_targets.to(device)
        if hidden is not None and hidden.size(0) != batch_inputs.size(0):
            hidden = None
        output, hidden = model(batch_inputs, hidden)
        hidden = hidden.detach()
        loss = criterion(output.view(-1, output_size),
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    generated = start_text  # Final output string

    # Convert input text to indices
    input_indices = [char_to_idx[c] for c in start_text]
    input_tensor = torch.tensor(input_indices).unsqueeze(0)  # shape: [1, n]
    ...
    for _ in range(k):
        ...
        last_logits = output[:, -1, :]  # shape: [1, vocab_size]

        # Sample next character index using temperature
        next_idx = sample_from_output(last_logits, temperature)
        next_char = idx_to_char[next_idx.item()]

        # Append the new character to the generated string
        generated += next_char

        # Prepare input_tensor for next step
        input_tensor = torch.tensor([[next_idx.item()]])

...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        self.W_xh = nn.Parameter(torch.randn(embedding_dim, hidden_size) * 0.01) # [e, h]
        self.W_hh = nn.Parameter(torch.randn(hidden_size, hidden_size) * 0.01) # [h, h]
        self.b_h = nn.Parameter(torch.zeros(hidden_size)) # [h]

        self.W_hy = nn.Parameter(torch.randn(hidden_size, output_size) * 0.01) # [h, v]
        self.b_y = nn.Parameter(torch.zeros(output_size)) # [v]
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            output.append(h_t)
            h_t_minus_1 = h_t # Update the hidden state for the next timestamp
...
sequence = "abcdefghijklmnopqrstuvwxyz" * 500
...
train_data = data_tensor[:train_size] # 90% for training
test_data = data_tensor[train_size:] # 10% for testing
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    input_seq = torch.tensor([char_to_idx[ch] for ch in start_text], device=device).unsqueeze(0)
    hidden = None
    ...
    with torch.no_grad():
        for _ in range(k):
            ...
            next_idx = sample_from_output(logits_last, temperature).item()

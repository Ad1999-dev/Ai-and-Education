...
It uses a loop over the document, extracting (k-1)-length context tuples and their following characters to build nested dictionaries with counts. The function builds n frequency tables, with n = 1 being unigram, n = 2 being bigram, n = 3 being trigram.
...
P(x_(t+1) | x_(t-n+2), .... , x_t) = (f(x_(t-n+2), x_t, x_(t+1)))/f(x_(t-n+2), ..., x_t)
### Code analysis

- ***Put the intuition of your code here***
Extracts the relevant (n-1)-length context from the sequence. Looks up frequency of the sequence + candidate character in the nth table. Divides by total frequency of the context in the same table. Returns the normalized probability or 0.0 if the context is unseen.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - *Show your work*
   P(a,a,a) = 11/20 * 4/11 * 1/2 = 0.1
   Thus, P(a |aa) = 0.5
2. $P(X_1=a, X_2=a, X_3=b)$
   - *Show your work*
   P(a,a,b) = 11/20 * 4/11 * 1/2 = 0.1
   Thus, P(b|aa) = 0.5
3. $P(X_1=a, X_2=a, X_3=c)$
   - *Show your work* 
   P(a,a,c) = 11/20 * 4/11 * 0 = 0
   Thus, P(c|aa) = 0

From Unigram: P(a) = 11/20, P(b) = 5/20, P(c) = 4/20
From Bigram: P(a|a) = 4/11, P(b|a) = 4/11
From Trigram: P(a|a,a) = 1/2, P(b|a,a) = 1/2, P(c|a,a) = 0
...
- ***Put the intuition of your code here***
Iterates over each character in the vocabulary. Calls calculate_probability() to score each possible next character. Selects and returns the character with the highest probability.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - *Your answer*
 It is a tie between 'a' and 'b' since they both have the probability 0.5, 'c' cannot be next in the sequence with a probability of 0.
## Experiment
...
Short corpus like "aababcaccaaacbaabcaa" is great for debugging and manual verification. Larger corpus such as warandpeace.txt works well but takes longer to build tables. With n = 1 (unigrams), the predictions were purely based on overall character frequency and lacked any contextual awareness, resulting in poor completions. At n = 2 (bigrams), the model began recognizing character-to-character transitions and could generate more coherent sequences. When increasing to n = 3 (trigrams), there was a noticeable improvement in predictive quality, as the model started capturing more meaningful local patterns, such as likely trigrams that appear within common words. However, increasing beyond n = 4 led to significant slowdowns during frequency table creation and a sharp increase in sparsity—many n-grams were never seen in training, resulting in zero probabilities and unstable predictions. Therefore, I found that n = 3 offered the best balance between performance and computational cost, especially on larger texts. For higher values of n, incorporating smoothing or backoff strategies would be necessary to maintain the same predictive quality.
<hr>

...
In this function, how I implemented it is by first initialising the amount of frequency tables using defaultdict data structure based on the input n. I then iterated through the document based on the length of the document and in an inner loop based on n, the amount of frequency tables. Then for each iteration, I firstly checked to make sure the character at that indice is still within the length of the document and then extracted that character and accordingly incremented the table for that n-gram or character. The whole document is done for that and after the defaultdicts structure is converted to normal dictionaries and all of these are returned. 
...
- ***Put the intuition of your code here***
In this function, what I do first is first taking the number of frequency tables as well as the length of the sequence inputted so far. Then we check for edge cases if the inputted sequence is length 0 or exceeds the number of tables then return 0. After confirming these are valid, we choose the corresponding frequency table associated with the length of the inputted sequence. We then extract the sequence we look for by concatenating the next character being checked for as well as the old sequence without that character. Then, using these two sequences, the previous and full sequences, find the frequency of the corresponding sequences using the correct tables. Then, using the formula, we divide the full sequence frequency by the previous sequence frequency to get the probability of that occurring and return that probability. 
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - *Show your work*
To calculate the probability of this, we follow the formula above. 
$$P(X_1=a, X_2=a, X_3=a) = P(a) \cdot P(a \mid a) \cdot P(a \mid aa)$$
- We find P(a) from table 1 = $\frac{11}{20}$ 
- Then, $P(a \mid a)$ from table 2 = $\frac{5}{10}$
- And, $P(a \mid aa)$ now from table 3 = $\frac{1}{4}$ 
- Then, multiplying we get $$\frac{11}{20} \cdot \frac{5}{10} \cdot \frac{1}{4} = \frac{11}{160}$$  
...
- We find P(a) from table 1 = $\frac{11}{20}$ 
- Then, $P(a \mid a)$ from table 2 = $\frac{5}{10}$
- And, $P(b \mid aa)$ now from table 3 = $\frac{2}{4}$ 
- Then, multiplying we get $$\frac{11}{20} \cdot \frac{5}{10} \cdot \frac{2}{4} = \frac{11}{80}$$  
...
- We find P(a) from table 1 = $\frac{11}{20}$ 
- Then, $P(a \mid a)$ from table 2 = $\frac{5}{10}$
- And, $P(c \mid aa)$ now from table 3 = $\frac{1}{4}$ 
- Then, multiplying we get $$\frac{11}{20} \cdot \frac{5}{10} \cdot \frac{1}{4} = \frac{11}{160}$$  
...
- ***Put the intuition of your code here***
For my predict_next_char function, how I implemented it is by doing the following steps. Firstly, initialising the best (highest probability) character and the maximum probability of that character to null and -1 values, respectively. I then, iterate through each character the vocabulary, which is a set of all possible characters, and calculate the respective probability of each character, based on the sequence and frequency tables. Then, I update the maximum probability and best character only if that specific character has a higher probability than the previous value. After iterating through the whole list of characters in the vocabulary, I then return that character so far with the highest probability.  
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - Based on the probability distribution obtained above, b would be the next character in sequence. 
...
After experimenting with the given corpus files being used for training as well as with varying values of n, several insights can be formed. For starters, the Alice's Adventures in Wonderland text file has proven to speed up the processing of the program, being able to output predictions for values of n above 20 in a reasonable time. On the other hand, for warandpeace text file, values above 10 have shown significantly longer processing times. In addition, when given irregular strings which are unlikely to show up in the training files, the outputs accordingly were also showing abnormal results.

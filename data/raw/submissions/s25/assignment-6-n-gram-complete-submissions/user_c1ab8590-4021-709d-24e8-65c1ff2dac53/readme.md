...
We scan through each position i in the training text. For each gram‐size k from 1 to n, we extract the preceding k−1 characters as context, and increment

tables[0] holds unigram counts (context = ""),

tables[1] holds bigram counts (context = last 1 char),

tables[2] holds trigram counts (context = last 2 chars)
...
1. ***Write down your probability table 2***:
   - as in your probability table should look like (wait a second, you should know what I'm talking about)
        ...
        | $P(\odot)$ | Probability value |  
        | ------ | ----------------- |
        | $P(a \mid a)$ | 	$\frac{5}{10} = 0.50$ |
        | $P(b\mid a)$	 | $\frac{3}{10} = 0.30$ |
        | $P(c\mid a)$	 | 	$\frac{2}{10} = 0.20$ |
        | $P(a\mid b)$	 | $\frac{2}{4} = 0.50$ |
        | $P(b\mid b)$ | 		$\frac{0}{4} = 0.00$ |
...
2. ***Write down your probability table 3***:
   - You got this!
        ...
        |$P(a\mid ab)$ | $\frac{1}{3} \approx 0.33$|
        |$P(b\mid ab)$ | $\frac{0}{3} = 0.00$|
        |$P(c\mid ab)$ | $\frac{2}{3} \approx 0.67$|
        |$P(a\mid ba)$ | $\frac{1}{2} = 0.50$|
        |$P(b\mid ba)$ | $\frac{1}{2} = 0.50$|
        |$P(c\mid ba)$ | $\frac{0}{2} = 0.00$|
        |$P(a\mid bc)$ | $\frac{2}{2} = 1.00$|
        |$P(b\mid bc)$ | $\frac{0}{2} = 0.00$|
        |$P(c\mid bc)$ | $\frac{0}{2} = 0.00$|
        |$P(a\mid ca)$ | $\frac{2}{3} \approx 0.67$|
        |$P(b\mid ca)$ | $\frac{0}{3} = 0.00$|
        |$P(c\mid ca)$ | $\frac{1}{3} \approx 0.33$|
        |$P(a\mid ac)$ | $\frac{0}{2} = 0.00$|
        |$P(b\mid ac)$ | $\frac{1}{2} = 0.50$|
        |$P(c\mid ac)$ | $\frac{1}{2} = 0.50$|
        |$P(a\mid cc)$ | $\frac{1}{1} = 1.00$|
        |$P(b\mid cc)$ | $\frac{0}{1} = 0.00$|
        |$P(c\mid cc)$ | $\frac{0}{1} = 0.00$|
        |$P(a\mid cb)$ | $\frac{1}{1} = 1.00$|
        |$P(b\mid cb)$ | $\frac{0}{1} = 0.00$|
        |$P(c\mid cb)$ | $\frac{0}{1} = 0.00$|
...
For a sequence $(x_1,\dots,x_t)$:
$[
P(x_1,\dots,x_t)
= P(x_1)\,\prod_{i=2}^t P\bigl(x_i\mid x_{i-n+1},\dots,x_{i-1}\bigr).
]$
In particular, for $(t=3)$ and $(n=3)$:
$[
P(X_1=x_1,X_2=x_2,X_3=x_3)
= P(x_1)\;P(x_2\mid x_1)\;P(x_3\mid x_1x_2).
]$


### Code analysis
- Take the last $(n-1)$ characters of `sequence` as the **context**.
- Look up how many times `(context → char)` occurred in the frequency tables.
- Divide by the total count of that context to get  
  $[
    P(\text{char}\mid \text{context})
    = \frac{f(\text{context}\;\text{→}\;\text{char})}
           {\sum_{c'} f(\text{context}\;\text{→}\;c')}.
  ]$
...
- Iterate over each character \(c\) in `vocabulary`.  
- Compute $(P(c\mid \text{last }n-1\text{ chars of sequence}))$ via `calculate_probability`.  
- Return the character with the highest probability (or an arbitrary one if all are zero).
...
From our trigram table for context `"aa"`:
- $(P(a\mid aa)=0.25)$  
- $(P(b\mid aa)=0.50)$  
- $(P(c\mid aa)=0.25)$  
so it should be
...
I experimented with 3‑gram through 8‑gram models on both Alice’s Adventures in Wonderland and War & Peace. On Wonderland, the 3‑ and 4‑gram tables built almost instantly, the 5‑gram was slower and exhibited high sparsity, and orders above 5 provided no real quality gain. On War & Peace, the 3‑gram remained snappy, the 4‑gram incurred a clear slowdown, the 5‑gram felt a little bit sluggish. In both cases, autocompletes were coherent at n=3, with only marginal improvements at n=4 and rapidly diminishing returns (plus resource costs) at n≥5. For instance 12-grams with 100 completion it took like 45 seconds which I believe is quite a lot.
Conclusion: a 3‑gram model delivers the best trade‑off of speed, memory, and suggestion quality—shorter corpora can tolerate n=4, but for larger texts you should stick to n=3 or 4.

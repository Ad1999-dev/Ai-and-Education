...
| Dataset | Seq-len / Stride | Embed / Hidden | Epochs | LR | Final **Train Loss** | Final **Test Loss** |

|---------|-----------------|----------------|--------|----|----------------------|---------------------|

| Alphabet (26 chars × 100) | 25 / 1 | 32 / 128 | 10 | 0.005 | **0.0003** | **0.0001** |

| *War & Peace* | 100 / 15 | 32 / 128 | 2 | 0.05 | **2.1488** | **5.2646** |
...
**Alphabet.**  
With a simple, perfectly-predictable sequence the RNN converged in < 3 epochs; after 10 epochs both train and test loss were essentially zero, and any 3-letter prompt rolled deterministically forward through the alphabet (e.g., *“abc” → “abcdefghijklm”*). Lowering temperature below 1 had no visible effect because the soft-max distribution was already a delta spike.  

**Literary corpus.**  
On *War & Peace* the same cell size (128 hidden) captured coarse character statistics but not long-range syntax. The network stalled around 2.1–2.2 train loss; the dev loss ballooned to ~5 when the windowed validation slices contained unseen trigrams. Even so, the sampler produced plausible fragments like *“love the do so”* at *T = 0.8*, and gibberish such as *“cat[[[[[[[[[[”* once the temperature exceeded 1.1, reflecting the much flatter output distribution.


3. Explain impact of changing temperature

| Prompt | Temp | Sampled Text |
|--------|------|--------------|
| `abc`, alphabet model | 0.5 | **abcdefghijklm** |
| `abc`, alphabet model | 2.0 | **bcdefghijklmnopqrstuvwx** (still correct due to peaked probs) |
| `abc`, W&P model | 0.7 | *“abct, conderow.”* |
| `abc`, W&P model | 1.0 | *“abct, cendeod row.jug h”* |
| `abc`, W&P model | 1.5 | *“abcmwq;fl%prk”* (degenerates into noise) |

Lower temperatures sharpen logits and force the model to choose the modal character—useful when quality matters. Raising `T` injects entropy, revealing the network’s uncertainty. On the tiny alphabet task this does nothing (distribution already degenerate), but on *War & Peace* it trades grammaticality for creativity until the output devolves into symbol soup beyond *T ≈ 1.3*.


4. Reflection

Implementing the vanilla RNN from raw weight matrices highlighted **hidden-state carry-over** and why truncated BPTT plus `hidden.detach()` are indispensable for stability. The alphabet sanity-check was an important control-experiment: it confirmed the forward/backward math **before** wasting minutes on the massive novel.  

Finally, temperature sampling illustrated that a half-trained language model can still generate human-readable morphemes, yet lacks global coherence—underscoring why deeper architectures (LSTM/GRU, Transformers) and more data are standard in modern NLP.


5. IMPORTANT: Include screenshot of output for generate_text() function:

<img width="984" alt="ss1" src="https://github.com/user-attachments/assets/7354ade4-3a05-4e53-8bdb-f9b80bfdfcbe" />
<img width="1094" alt="ss2" src="https://github.com/user-attachments/assets/829da9a5-413f-4118-a4c0-81834614806f" />

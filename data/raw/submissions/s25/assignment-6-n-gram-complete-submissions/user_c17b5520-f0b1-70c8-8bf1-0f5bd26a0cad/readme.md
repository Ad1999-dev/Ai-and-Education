...
The create_frequency_tables function constructs a series of frequency tables for n-grams from a given text document, where n-grams are sequences of characters of varying lengths up to n. For each n-gram size from 1 to n, the function slides a window across the document to extract every possible sequence of that length and counts how many times each occurs. These counts are stored in dictionaries, with each dictionary corresponding to a specific n-gram length. The result is a list of frequency tables that capture the statistical patterns and dependencies between characters in the document, useful for building character-based language models.
...
- This function estimates how likely a character is to follow a given sequence based on patterns learned from training data. It does this by looking up how often the full sequence (prev sequence + next character) appears, and dividing that by how often the context alone appears — a direct application of conditional probability. If the context has never been seen before, it safely returns a probability of 0 to avoid division errors.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - P(a) * P(a | a) * P(a | aa) = 11/20 * 5/11 * 1/5 = 0.55 * 0.4545 * 0.2 = 0.05
2. $P(X_1=a, X_2=a, X_3=b)$
   - P(a) * P(a | a) * P(b | aa) = 11/20 * 5/11 * 1/5 = 0.55 * 0.4545 * 0.4 = 0.099
3. $P(X_1=a, X_2=a, X_3=c)$
   - P(a) * P(a | a) * P(c | aa) = 11/20 * 5/11 * 2/5 = 0.55 * 0.4545 * 0.2 = 0.05

## `predict_next_char(sequence, tables, vocabulary)`

### Code analysis

- The intuition behind this code is to use the given sequence of characters to predict the most likely next character by calculating the conditional probability for each possible next character in the vocabulary. The function iterates over all characters in the vocabulary, computes the probability for each one, and keeps track of the character with the highest probability. The character with the highest probability is returned as the predicted next character, which is essentially the character most likely to follow the given sequence based on historical data.
...
When experimenting with both the War and Peace and Alice in Wonderland corpora using varying values of n, I found that Alice in Wonderland worked better for smaller n-gram values (e.g., n = 2 or n = 3) because it is a smaller, more consistent text with simpler sentence structures and vocabulary. In contrast, War and Peace, being a much longer and more complex novel, benefited from slightly higher values of n (e.g., n = 4 or n = 5) where the increased context helped capture more meaningful character patterns. However, beyond n = 5, frequency tables began to grow too large, resulting in slower computation times and sparse data, especially for rarer sequences. In practice, the tradeoff between model complexity and runtime made n = 3 or n = 4 the sweet spot for balancing performance and efficiency across both corpora.

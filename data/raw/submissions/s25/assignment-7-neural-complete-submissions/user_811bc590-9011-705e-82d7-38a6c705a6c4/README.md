...
1. Describe your experiments and observations & 2. Analysis on final train and test loss for both datasets
- When training the model with the first dataset: the sequence "abcdefghijklmnopqrstuvwxyz" * 100, the model converged very quickly receiving a train loss of less than 0.1 after just the second epoch and by 10 epochs around 0.001 and achieving a 100% test accuracy and around a 0.001 test loss. My experimentation with this first sequence was very straightforward and easy I did not need to tweak the hyper parameters very much to get a low loss and high accuracy. These were the values I had:
```python
sequence_length = 100
stride = 1
embedding_dim = 16
...
batch_size = 64
```
The embedding dimension, hidden_size, and batch_size did not need to be to high to get a good model because just analyzing a sequence is very easy for the model to understand. When testing the autocomplete text generation for this dataset it was easily able to suggest the correct next letters in the alphabet showing it was easy for it to pick up on their direct relationships. It 100% of the time in my testing suggested the following characters in the alphabet when I gave it an initial sequence.

- For the second dataset, `warandpeace.txt`, the training process was a lot less simple. The hyperparameter tuning step took much longer as well as the actually training phase since working with such a large dataset the model took a long time to train on the whole thing. After multiple attempts of training the model and tweaks of the the hyper parameters, making them more complex but trying to keep the epoch from taking to long, I settled on these:
```python
sequence_length = 100
stride = 2
embedding_dim = 128
...
batch_size = 64
```
With these hyper parameters I was able to achieve a training loss of 1.4871 after the one epoch I ran and an accuracy of 56.49% and a test loss of 1.4899. When I tested the text generation for this model it would often give some real words or nonsense/"kind of" words separated by spaces but most of the time not forming coherent sentences and sometimes  list of words that might often appear near each other. I also found that when asking it to generate more characters it would start to just repeat words about halfway through or near the end of its output. The text generated with this dataset was much more random and different every time even with the same parameters compared the alphabet dataset version which was the same each time and the output was much more predictable and made more sense.
Example outputs from `warandpeace.txt` data:
| Initial text | # characters to generate | Temp | Output |
|----------------|--------------|------------|-------------------|
| she | 10 | 1 | sheavoluta, p |
| dog | 15 | 1 | dogethe whicher di |
| dog | 20 | 2 | dog ate a scorck? biysides, |
| dog | 15 | .000001 | doge was prive her |
| she | 20 | .001 | she and the were the wa |
| he is | 30 | .00001 | he is a soldiers and she said the s |
| he is a tall man with | 50 | .0000001 | he is a tall man with a smile of the same time to the same sound of the |
| he is a tall man with | 50 | 1 | he is a tall man with diness forward behind that side? when he would, s |
| they walked together to the | 75 | .000001 | they walked together to the same sound of the same time to the same sound of the same time to the same |
| she smiled while | 75 | .000001 | she smiled while the sound of the same time the same sound of the same time the same sound |

3. Explain impact of changing temperature

The temperature parameter made a major impact in modifying the randomness of the text generation. A lower temperature such as less than 0.01 led to more deterministic outputs with less variability, often resulting in repetitive phrases or common word patterns. While a higher temperature like 1 or 2 caused much more randomness in the outputs and allowed the generation of more unique sequences but also increasing the chances of producing nonsense words. As shown in the above table of outputs from the model trained on the `warandpeace.txt` when the temperature was >= 1 random terms like: `sheavoluta`, `dogethe whicher`, and `scorck? biysides`, were generated. On the other hand when the temperature was very small <= 0.001 it more consistently generated all real words and often had repetition of words in the output like: `the same sound of the same time to the same sound of the same time to the same` and `the sound of the same time the same sound of the same time the same sound`, each of which was generated in an entirely different prompt.

4. Reflection

While creating the model, tuning the hyper parameters, testing, and analyzing the generated text I faced many challenges but also had many learning moments. It was a challenge to tune the hyper parameters effectively for the larger dataset to achieve a low loss and high accuracy and made me observe that the model's performance depended heavily on its architecture and training settings. During this process I learned that RNNs excel at capturing short‐term, deterministic patterns like the alphabet, but struggle more with large dependencies and the complex structure of natural language.

5. Screen shot of running model and testing generate_text:
`warandpeace.txt`:
![alt text](image.png)
alphabet sequence:

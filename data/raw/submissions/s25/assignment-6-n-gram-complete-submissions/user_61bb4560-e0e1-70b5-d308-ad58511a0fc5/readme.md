...
The `create_frequency_tables(document, n)` function loops through every character position in the text and generates all n-grams from size 1 to n. It uses a list of `defaultdict(int)` to count how many times each n-gram appears. Each table corresponds to a different n-gram size — the first is for unigrams, the second for bigrams, and so on.
...
| $P(\odot)$       | Probability value |
|------------------|-------------------|
| $P(a \mid a)$    | $\frac{5}{11}$ ≈ 0.4545 |
| $P(b \mid a)$    | $\frac{3}{11}$ ≈ 0.2727 |
| $P(c \mid a)$    | $\frac{2}{11}$ ≈ 0.1818 |

4. ***Write down your probability table 3***:
   | $P(\odot)$         | Probability value |
|--------------------|-------------------|
| $P(a \mid aa)$     | $\frac{1}{5}$ = 0.2 |
...
This function uses backoff from trigram to bigram to unigram probabilities. It tries to find the most specific context available. If there's no match in a higher-order n-gram table, it falls back to a lower-order one. The logic uses suffix slicing and frequency lookups.
...
1. **\( P(X_1=a, X_2=a, X_3=a) \)**  
   \[
   = P(a) \cdot P(a \mid a) \cdot P(a \mid aa) = 0.55 \cdot 0.4545 \cdot 0.2 ≈ 0.050
   \]

2. **\( P(X_1=a, X_2=a, X_3=b) \)**  
   \[
   = P(a) \cdot P(a \mid a) \cdot P(b \mid aa) = 0.55 \cdot 0.4545 \cdot 0.4 ≈ 0.100
   \]

3. **\( P(X_1=a, X_2=a, X_3=c) \)**  
   \[
   = P(a) \cdot P(a \mid a) \cdot P(c \mid aa) = 0.55 \cdot 0.4545 \cdot 0.2 ≈ 0.050
   \]
...
This function iterates through all characters in the vocabulary and uses `calculate_probability()` to compute the probability of that character coming next. It keeps track of the character with the highest probability and returns it.
...
- **"b"** (it had the highest probability of 0.1)
 
## Experiment
Using `warandpeace.txt` and `Alice's Adventures in Wonderland.txt`, I tested the model with `n = 3`, the input `"the"`, and predicted 15 characters.
...
The model trained on **War and Peace** produced more diverse results with meaningful English phrases like `"and the"`. In contrast, **Alice** led to over-repetition, likely due to a smaller corpus size or less stylistic variability. Increasing `n` improves prediction quality up to a point but significantly slows down table construction. A good balance seems to be `n = 3` or `4` before performance drops.

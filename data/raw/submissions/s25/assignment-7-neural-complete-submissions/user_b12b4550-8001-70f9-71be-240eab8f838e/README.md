...
Alphabet Dataset
For the synthetic dataset, the task was relatively simple: the alphabet follows a strict and repetitive pattern. The model learned this quickly and accurately. Within one epoch, the RNN achieved a test loss of 0.0365, nearly perfect. Generated outputs from prompts like "abc" or "lmn" reliably followed the correct alphabetical sequence.

This showed that the RNN was capable of learning and memorizing short-range dependencies very effectively.

War and Peace
When trained on War and Peace, the challenge increased significantly. Natural language contains varied spelling, punctuation, and long-range context that a vanilla RNN struggles to learn. After 5 epochs, the model stabilized at a test loss of 1.6122 — higher but expected.

Generated text at temperature 1.0 had some character coherence and pseudo-English word formations, but lacked sentence-level structure. As I increased temperature (e.g. to 2 or 3), the model became more creative, often generating gibberish or character noise, which confirmed the temperature’s impact on output randomness.


3. Explain impact of changing temperature

RNNs perform well on simple, repetitive sequences but are limited when handling complex, real-world text without attention or memory mechanisms (e.g., LSTM, GRU).
The temperature parameter plays a critical role in controlling output randomness:
Low temperature → safer, repetitive output
High temperature → creative, but less meaningful text
Manual implementation of the RNN cell deepened my understanding of hidden state updates and the flow of information through time steps.

4. Reflection

This project helped me understand the capabilities and limitations of vanilla RNNs. They work well on simple patterns but struggle with long-range dependencies and natural language irregularities. I learned how to preprocess text, prepare sequence data using strides, implement the RNN cell manually, and tune hyperparameters to control text generation quality. Using temperature to manipulate creativity in generation was particularly insightful.

5. IMPORTANT: Include screenshot of output for generate_text() function: 
War and Peace 
![War and Peace](War_and_Peace.png)

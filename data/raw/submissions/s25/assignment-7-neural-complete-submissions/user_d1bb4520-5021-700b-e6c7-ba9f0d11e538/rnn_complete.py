...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    input_seq = [char_to_idx[ch] for ch in start_text[-n:]]
    ...
    for _ in range(k):
        ...
        next_idx = sample_from_output(next_logits.unsqueeze(0), temperature).item()
        input_seq.append(next_idx)
        input_tensor = torch.tensor(input_seq[-n:], dtype=torch.long).unsqueeze(0).to(device)

    generated = ''.join([idx_to_char[i] for i in input_seq])
    return generated

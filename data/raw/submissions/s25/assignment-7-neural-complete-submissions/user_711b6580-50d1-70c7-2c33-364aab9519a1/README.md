...
# Neural Complete (Assignment 7) Final Report

## Late Days Used
3 late days

## 1. Describe your experiments and observations
I used the War and Peace text and the alphabet sequence provided. For the alphabet sequence, the model converged quickly and the loss was near zero in 2 epochs. In using "abc" or "xyz", the model produced the exact alphabet sequence. The training was fast as seen by each epoch completing in a short amount of time, less than a second.

In the War and Peace text, an epoch took about a minute so training took a lot longer than the previous dataset. The loss decreased but stabilized around 2.0-1.9, never approaching zero. It generated text with word-like structures but it wasn't coherent nor readable.

## 2. Analysis on final train and test loss for both datasets

### Alphabet Sequence Results
- **Final Training Loss**: 0.0001
- **Final Test Loss**: 0.0001

The near-identical train and test losses indicate no overfitting. The simple, repetitive pattern was learned perfectly, demonstrating the RNN's ability to capture deterministic sequences.

### War and Peace Results
- **Final Training Loss**: 2.03
- **Final Test Loss**: 1.91

The slightly lower test loss suggests potential regularization effect. The relatively high loss values reflect the complexity of natural language modeling, where perfect prediction is mathematically impossible due to the inherent ambiguity and context-dependence in real text.

## 3. Explain impact of changing temperature
Temperature controls the randomness in text generation by scaling the logits before sampling:

- **Temperature < 1 (e.g., 0.1)**: Makes predictions more deterministic
  - For alphabet: Still produces perfect sequences
  - For War and Peace: Would produce more repetitive, conservative text

- **Temperature = 1.0 (default)**: Balanced randomness
  - Alphabet: Perfect prediction maintained
  - War and Peace: Mixed coherence with some creativity

- **Temperature > 1 (e.g., 2.0)**: Increases randomness
  - Alphabet: Might introduce errors in previously perfect sequences
  - War and Peace: More creative but less grammatically correct output

## 4. Reflection
I found that simple patterns converge to zero loss, while complex language learning plateaus at higher loss values. The model generalized perfectly on alphabet (test loss = train loss) but showed slight overfitting on War and Peace. I faced challenges managing training time for large text datasets and adjusting the hyperparameters for the different datasets. 

## 5. Text Generation Output

### Alphabet Sequence Generation:
![Alphabet Sequence Generation](abc.png)

### War and Peace Generation:
![War and Peace Generation](war&peace.png)

...
I started with the simple dataset composed of repeated sequences of the alphabet. Because I made it predictable, the model was able to quickly learn the character transitions and minimize training loss.
During the training, the loss decreased steadily at the beginning, which means that the model was learning effectively from the training data. But then the loss evened out and wouldnt keep learning. I then experimented with different values of the hidden size, embedding dimensions, learning rates, and batch sizes. Increasing embedding dimensions often led to better representations but it slowed the training. 
2. Analysis on final train and test loss for both datasets
A low training loss combined with a high validation loss indicates overfitting, and both low values suggest good generalization.
The test loss was higher than the training loss, which means that that while the model learned patterns in training data well, it didn't generalize perfectly to unseen data. 
3. Explain the impact of changing temperature
The temperature parameter controls the randomness of predictions during text generation. A temperature of 1.0 represents a balance where the model generates text according to the probabilities from the softmax output. As temperature increases, the model samples from a distribution that allows for more randomness, leading to diverse outputs but less coherence.
A temperature below 1.0 (e.g., 0.5) yields more deterministic outcomes, thus producing text that is more coherent and closely aligns with training data but might limit creativity and novelty. A temperature above 1.0 (e.g., 1.5) produces more diverse, surprising outputs including nonsensical patterns; useful for creative applications, but risks the generation of less coherent text.
4. Reflection
I gained experience in RNNs for text generation. I worked with the effects of specific hyperparameters and how different configurations influenced learning and output, which was fun to play around with. 
One of the main problems I was running into with tuning the model would be how drastic some changes would be. Sometimes you would be improving by increasing one parameter and then you increase it some more and the loss would increase. 

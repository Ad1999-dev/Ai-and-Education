...
This code iterates from 1 to n, creating a corresponding list of n frequency tables for n-grams. In each iteration of the outer loop, it initializes a new frequency table (a dictionary) to count occurrences of n-grams of the current length. It then processes all substrings of that length from the document and updates their frequencies in the table, which is subsequently added to the overall list of frequency tables.
...
P(x_1^T) \;=\;
\prod_{i=1}^T
\frac{f\bigl(x_{\max(1,i-n+1)} \dots x_{i-1}\,x_i\bigr)}
     {f\bigl(x_{\max(1,i-n+1)} \dots x_{i-1}\bigr)}.
$$

### Code analysis

This code calculates the probability of a specified character occurring after a given sequence of characters, using precomputed frequency tables for n-grams. It identifies the relevant n-gram based on the length of the input sequence, retrieves the count of that n-gram as the numerator, and finds the count of the preceding context as the denominator to compute the conditional probability. If the context is empty, it uses the total count of characters for normalization. The function returns the probability value or 0.0 if the denominator is zero, indicating that the sequence is not observed in the training data.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   = $\frac{f(a)}{20} \times \frac{f(aa)}{f(a)} \times \frac{f(aaa)}{f(aa)}$
...
This code predicts the most likely next character following a given input sequence by evaluating the probabilities of all characters in a predefined vocabulary. For each character in the vocabulary, it uses the `calculate_probability()` function to compute the likelihood of that character occurring after the sequence. It keeps track of the character with the highest probability and returns it as the predicted next character. If no characters have been assessed yet, the code initializes the best probability to zero and updates it as higher probabilities are found.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - The next character in the sequence would be "b" since the above probability distribution shows the string "aab" as having the highest probability.
...
The first finding I noticed was that when using just 1 gram for both War and Peace and Alice's Adventures in Wonderland, the autocomplete will give you many spaces, even up to length of completion 20. This makes sense since a space is probably the most common unigram in both texts and in all texts for that matter. When doing runs up to n=4, it seemed to take between 2-3s to run. When incresing it to 5 and 6 it took around 4-6s to run and increasin n to 10 made it take 10s. When considering an autocomplete system, even runs of 5-6s might seem unreasonable since we want suggestions instantly, even more so for 10s. It seems like the Alice's Adventures in Wonderland is easier to work with compared to War and Peace. With n=4 Alice's Adventures in Wonderland took 2s for a completion lenth of 5 while war and peace took 7s. This is likely because Alice's Adventures in Wonderland is a smaller corpus.

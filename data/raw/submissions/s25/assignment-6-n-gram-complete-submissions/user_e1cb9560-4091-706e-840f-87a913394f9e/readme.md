...
- ***`create_frequency_tables`, calls my helper function `createFrequencyTable` (that takes in a document and a gram number and returns a table represented as a dictionary) in a loop for \( n \) iterations, each iteration using an increasing gram from 1 to \( n \) and appending the result to a list. The list is then returned at the end as an output. My helper function essentially creates a sliding window of size gram that passes through the document. If the text in the window exists in the table, it increments the frequency. If not, it adds the text to the table with a frequency of 1.***
...
1. ***What is your vocabulary in this case***
   - The vocabulary is a set of (a, b, c) 
...
Let $m$ be the number of gram and $n$ be the number of character in the text that we want to find the probability of.

$$
\begin{align*}
P(x_1, x_2,...,x_n) &= p(x_1)p(x_2|x_1)...p(x_m|x_1,...,x_{m-1})...p(x_n|x_{n-m},...,x_{n-1}) \\
&=\frac{f(x_1)}{size(C)} \cdot \frac{f(x_1, x_2)}{f(x_1)} \cdot ... \cdot \frac{f(x_1,...,x_m)}{f(x_1,...,x_{m-1})} \cdot ... \cdot \frac{f(x_{n-m},...,x_{n})}{f(x_{n-m},...,x_{n-1})}
\end{align*}
$$

### Code analysis

- ***I created one helper for this function called calculateConditionalProb, which is to calculate conditional probability for each term (e.g., p(x_3|x_1, x_2)). For each term, it looks up the frequency from the table and returns the numerator (f(x_1, x_2, x_3) in this case) divided by the denominator (f(x_1, x_2) in this case). In calculate_probability, it calls calculateConditionalProb n times, where n is the length of the sequence. In each iteration, the argument is the target character (like x_3 in the previous example) and at most the previous m characters as a conditional part. It returns accumulated product in each iteration.***
...
- ***In this function, I loop through every possible character in the vocabulary and calculate its probability of coming after the given sequence. Throughout the loop, I keep track of the character that has the highest probability and return it at the end. If there are ties, it will keep the first one.***
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - *Base on the probability above, b would be the next letter as it has the highest probability.*
...
Answer: Since time consuming is quite vague and may depends on the application, let's assume any runtime above 30 seconds is too long. The number of n that exceed 30 seconds runtime is 31 for warandpeace.txt and 550 for Alice's Adventures in Wonderland.txt. I believe this is mainly because the former text is much longer thus it take longer for the program to go through it. In general, I believe the performance of the former program (warandpeace) is better for a larger gram because it give out more coherence text as it has more data to train on. On the other hand, I believe the second program (Alice's Adventures in Wonderland) outperform the first program because the first program keep alternate between two words.

Here is the result from each of my experiment

training text = Alice's Adventures in Wonderland

n = 5, initial text = "because", k = 40
output = because to the project gutenberg™ electronic wo

n = 10, initial text = "because", k = 40
output = because i'm on the second time 'round, she came

traning text = warandpeace

n = 5, initial text = "because", k = 40
output = because of the said the said the said the said

n = 10, initial text = "because", k = 40
output = because he was always the case with his hand to

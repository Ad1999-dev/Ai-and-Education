...
- ```increment_dict_value```: helper function that increments the value of a key in a dict if it exists, or adds it to the dict with value 1 if not
- The ```create_frequency_tables``` function itself goes through the entire document with a sliding window of size $i$ and adds each substring to a dictionary, for each $i$ from 1 to $n$. So it returns a list with a frequency table of 1-character strings, 2-character strings, etc up to n-character strings.
...
1. ***What is your vocabulary in this case***
   - {'a','b','c'}
   - IMPORTANT: Note that I am using the paradigm where end-of-file grams also count. For example, if the entire document is "aba", then P(b | a) = 1/2 because "b" follows "a" once, and the end of file follows "a" once. 
...
In an n-gram model where we are predicting $x_m$ based on $x_1,x_2,...,x_{m-1}$, we need to find the following probability for all $x_m$ in our vocabulary:

$P(x_m | x_1, x_2, ..., x_{m-1}) = \frac{P(x_1,x_2,...,x_{m-1},x_m)}{P(x_1,x_2,...,x_{m-1})}$.

We could attempt to calculate both the numerator and denominator in their entirety, but since we are only looking at n-grams, only the last $n-1$ terms will matter, so this is equal to 

$P(x_m | x_{m-n+1}, x_{m-n+2}, ..., x_{m-1}) = \frac{P(x_{m-n+1},x_{m-n+2},...,x_{m-1},x_m)}{P(x_{m-n+1},x_{m-n+2},...,x_{m-1})}$. 

But we don't actually have to calculate the numerator and denominator separately: Note that after removing grams which we cannot access in an n-gram model, the remaining terms are part of a fully joint model, meaning Section 2's formula for conditional probability applies:

$P(x_m | x_{m-n+1}, x_{m-n+2}, ..., x_{m-1}) = \frac{f(x_{m-n+1},x_{m-n+2},...,x_{m-1},x_m)}{f(x_{m-n+1},x_{m-n+2},...,x_{m-1})}$. 

In the special case of the 1-gram model, $P(x_1 | \text{nothing})$ doesn't make much sense, so we instead actually just find $P(x_1)$ which is $f(x_1)/\text{size}(D)$ where size(D) is the size of the document.

### Code analysis

- First, if the sequence is longer than n - 1 (where n is number of grams we are considering), we only look at the last n-1 grams of the sequence (to predict the nth one). 
- If this number of grams considered is just 1, then we use the special case formula from above.
- Then, we look up the frequencies in the formula written above in the frequency table found in the first part (and if we don't find it in the lookup we assume it to be 0).
- Finally, we calculate the probability and return (if the fraction is 0/0 we return 0 as a default).
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - = $P(X_3=a | X_1=a,X_2=a)\cdot P(X_2=a|X_1=a)\cdot P(X_1=a) = \frac{1}{5} \cdot \frac{5}{11}\cdot \frac{11}{20} = \frac{1}{20}$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $P(X_3=b | X_1=a,X_2=a)\cdot P(X_2=a|X_1=a)\cdot P(X_1=a) = \frac{2}{5} \cdot \frac{5}{11}\cdot \frac{11}{20} = \frac{1}{10}$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $P(X_3=c | X_1=a,X_2=a)\cdot P(X_2=a|X_1=a)\cdot P(X_1=a) = \frac{1}{5} \cdot \frac{5}{11}\cdot \frac{11}{20} = \frac{1}{20}$
...
- If we have sequence = $x_1,x_2,...,x_m$, this function finds all the $P(x_m | x_1,x_2,...x_{m-1})$ using `calculate_probability`, for all $x_m$ in the vocabulary and stores it in a list.
- Then it returns the character $x_m$ which maximizes that probability.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - 'b' would be the next in the sequence because it has the highest probability
...
With $n=1$, the model runs very quickly and always just outputs spaces. This is probably because space is the most common character in the whole of War and Peace, and with 1 gram the model has no extra context, so it only predicts the most common character.

With $n=2$, the model still runs very quickly, and seems to output common words. For example, if your input is 't', 'h', 'e', or ' ', the output (with high length of completion value) just keeps repeating ' the the the' etc. If the input is something else, the immediate output makes sense for the context, but then as soon as it predicts a space, it goes back to 'the the the the '.

With $n=5$, the model takes a few seconds to run, and outputs 'said the said the said the' after the first few suggestions which actually make sense.

With $n=10$, the model takes about 10 seconds to run on my machine, and it feels almost like what happens when you spam the next word suggestions on an iPhone. It starts out having a reasonable autocorrect, but eventually gets stuck in a loop. Still, $n=10$ seems fairly reasonable.

With $n=20$ the model takes approaching 30 seconds to run, and if you have the length of completion be a large number, it outputs what look to be entire passages from war and peace, but are actually just parts of passages stitched together like Frankenstein's Monster.

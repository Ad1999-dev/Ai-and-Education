...
- My code starts with creating a list named tables which will have pythin dictionaries as its elements and will be the mian output of the function. Since we need n dictionaries the loop makes n default dictionaries of int type values and have default value as 0 when an elements is added without a specific value. The second loop slices the string document to add keys based on the table number ( 1 for unigram, 2 for bigram) and then adds 1 to the value to keep track of the frequency of the characters. Thwn this default dictionary is converted to a normal dictionary and then appended to tables and outputed.
...
### Formula
- P(X_n+1 | X1,X2,..,Xn)= f(X1,X2,...,X_n+1)/f(X1,X2,..,Xn)

### Code analysis

- calculates the conditional probability of a character following a given sequence based on n-gram probability tables. It first initializes a variable, which is derived from the length of the input sequence. A check is performed to ensure that this index does not exceed the number of available tables. If it does, the `sequence` is truncated to prevent out-of-bounds errors when accessing the tables. The code then retrieves a denominator count from the appropriate n-gram table, representing the total occurrences of the `sequence`. A new n-gram word, `to_find_word`, is created by appending a character `char` to the original sequence, and its corresponding index is checked similarly. The numerator count for this new n-gram is then obtained from the relevant table. The probability is computed as the ratio of the numerator to the denominator, with checks in place to handle cases where the denominator is zero, thereby returning a probability of zero to avoid division by zero. Finally, the calculated probability is returned. To enhance the code's robustness, suggestions include adding input validation to ensure proper types and values, improving efficiency through the use of data structures like defaultdict for frequency counting, implementing error handling for clearer signaling of issues, and providing a docstring to clearly document the function's purpose, parameters, and return type.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $P(X_1=a)* P(X_2=a \mid X_1=a) * P(X_3=a \mid X_1=a,X_2=a)$
   - $11/160$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $= P(X_1=a) * P(X_2 = a \mid X_1 = a) * P(X_3 = b \mid X_1 = a, X_2 = a)$
   - =2/4
3. $P(X_1=a, X_2=a, X_3=c)$
   -  $P(X_1=a, X_2=a, X_3=c) = P(X_1=a) ⋅ P(X_2 = a \mid X_1 = a) ⋅ P(X_3 = c \mid X_1 = a, X_2 = a)$
   -  = 1/5
...
-It initializes two variables, `max_probability` and `predicted_char`, to track the highest probability found and the corresponding character. The function first checks the length of the input sequence against the number of available n-gram tables, truncating the sequence if necessary to ensure compatibility. It then iterates through each character in the specified vocabulary, constructing a new sequence by appending the current character and calculating the probability of this new sequence using the `calculate_probability` function. If the calculated probability is `None`, it defaults to `0` to avoid issues during comparisons. The function keeps track of the character with the highest probability, updating `max_probability` and `predicted_char` accordingly. If no character has a valid predicted probability, the function defaults to returning a space character. Finally, it returns the character with the maximum probability, allowing for the prediction of the next character based on previously computed n-gram frequencies. Potential improvements include input validation, adding logging for debugging, handling tie cases for maximum probability, and ensuring robust documentation of expected output.
...
after using different values of n and the different corpus given, I found out that when n has smaller values the code runs faster than when it is bigger, as it needed less computations on making frequency tables and predicting next character. I also noticed that smaller values of n gave less acurate answers as it has insufficient context but with bigger n's the dependencies between characters better. the time to make frequency tables became very demanding on larger corpus. So we need a balance between the two, with big enough n but not too big.

n = 10 showed slow results but n = 2 was good
<hr>

...
### Code analysis
This function builds a list of frequency tables for all n-grams from length 1 up to n. It iterates over each character in the document and collects substrings (n-grams) of increasing length starting at each position. These are stored in separate tables:

Table 0 for unigrams (length 1),

Table 1 for bigrams (length 2),

… up to n-grams (length n).

This provides a full view of how often each pattern appears, which is later used to calculate probabilities.

Example:
If n = 3 and document = "aabc", then:

Table 0 (1-grams) → {'a':2, 'b':1, 'c':1}

Table 1 (2-grams) → {'aa':1, 'ab':1, 'bc':1}

Table 2 (3-grams) → {'aab':1, 'abc':1}

This prepares the raw counts needed for calculating conditional probabilities like:
P(x₃ | x₁x₂) = f(x₁x₂x₃) / f(x₁x₂)
...
### Formula
P(x1, x2, x3, x4) = P(x1) * P(x2|x1) * P(x3|x2) * P(x4|x3) = f(x1)/N * f(x1, x2)/f(x1) * f(x2, x3)/f(x2) * f(x3, x4)/f(x3)


### Code analysis

This project uses N-gram language models to compute the probability of character sequences. The idea is to apply the chain rule of probability, which breaks a sequence into conditional probabilities: P(x1, x2, x3, ..., xn) = P(x1) * P(x2 | x1) * P(x3 | x2) * ... * P(xn | xn-1)


1-gram: P(x1) = frequency(x1) / total_characters
2-gram: P(xi | xi-1) = frequency(xi-1, xi) / frequency(xi-1)
N-gram: P(xi | xi-k, ..., xi-1) = frequency(xi-k, ..., xi) / frequency(xi-k, ..., xi-1)

Code Explanation (calculate_probability() function)
tables[0][ngram] / sum(tables[0].values())
→ computes P(x1) (unconditional probability).

tables[k][ngram] / tables[k-1].get(context, 1)
→ computes P(xi | context) for higher-order N-grams.

prob *= ...
→ multiplies the probabilities step-by-step across the sequence.

This estimates how likely a character is to follow a given context, based on frequency counts from the training data.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   $$
P(a, a, a) = P(a) \cdot P(a \mid a) \cdot P(a \mid aa)
$$

$$
= 0.55 \cdot 0.4545 \cdot 0.20
$$
...
2. $P(X_1=a, X_2=a, X_3=b)$
   $$
P(a, a, b) = P(a) \cdot P(a \mid a) \cdot P(b \mid aa)
$$

$$
= 0.55 \cdot 0.4545 \cdot 0.40
$$
...
3. $P(X_1=a, X_2=a, X_3=c)$
  $$
P(a, a, c) = P(a) \cdot P(a \mid a) \cdot P(c \mid aa)
$$
...
It loops through every character in the vocabulary.

For each character, it calculates the joint probability of the sequence followed by that character.

It keeps track of the character that gives the highest probability.

Finally, it returns that character as the predicted next character.
...
When experimenting with different corpus files and varying values of n for N-gram language models, smaller corpus files tend to work better because they require less memory and computational time to calculate the frequency tables. Larger corpora, while providing more diverse character sequences, lead to a significant increase in processing time as n grows. For example, with a small corpus like a short text or a novel, values of n up to 3 or 4 can be run relatively quickly. However, for higher n values (5 and above), the computational cost can become quite expensive, especially when constructing frequency tables for large corpora. As n increases, the number of possible combinations of previous characters grows exponentially, which makes the table calculations more time-consuming. Generally, for practical purposes, it's common to limit n to 4 or 5 for most language modeling tasks. Beyond this, the efficiency and speed of table calculations degrade, especially for larger datasets.

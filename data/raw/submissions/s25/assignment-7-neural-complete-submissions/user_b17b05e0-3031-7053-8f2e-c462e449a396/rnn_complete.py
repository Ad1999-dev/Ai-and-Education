...
idx_to_char = {i: char for char, i in char_to_idx.items()} # TODO: Create the reverse mapping
...
for epoch in range(num_epochs):
    ...
    for batch_inputs, batch_targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        b, l, v = output.size()
        output = output.reshape(b * l, v)
        targets = batch_targets.reshape(b * l)
...
with torch.no_grad():
    for batch_inputs, batch_targets in tqdm(test_loader, desc="Testing"):
        ...
        outputs, hidden = model(batch_inputs, None)
        b, l, v = outputs.size()
        loss = criterion(outputs.view(b * l, v), batch_targets.view(-1))
        total_loss += loss.item()
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    input_tensor = torch.tensor([input_idxs], dtype=torch.long, device=device)
    hidden = None
    ...
    with torch.no_grad():
        ...
        for _ in range(k):
            next_idx = sample_from_output(last_logits, temperature=temperature) 
            idx_int = next_idx.item()
            next_char = idx_to_char[idx_int]
            generated.append(next_char)

            input_tensor = next_idx.view(1, 1)
            logits, hidden = model(input_tensor, hidden)
            last_logits = logits[:, -1, :]

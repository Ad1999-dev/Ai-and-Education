...
-   **_Put the intuition of your code here_**
    The intuition was to initialize a list of empty dictionaries to store each table, and then loop through n-gram sizes and extract each n gram by slicing the document. Later, updating the frequency counts and returning the able.
...
-   **_Write the formula for sequence likelihood as described in section 2_**
    $$ P(x*1, x_2, \dots, x_t) \approx \prod*{i=1}^{t} P(x*i | x*{i-n+1}, \dots, x\_{i-1}) $$

### Code analysis

-   **_Put the intuition of your code here_**
    The code queries the tables safely for the n_gram and the sequence frequency counts using the length of the n_gram and sequence and extracting it with a dictionary method, and then creating the probability by putting the n_gram count over the sequence count as a fraction.
...
1. $P(X_1=a, X_2=a, X_3=a)$
    - $P(a,a,a) \approx P(a) \cdot P(a | a) \cdot P(a | aa)$
    - $P(a,a,a) \approx (\frac{11}{20}) \cdot (\frac{5}{11}) \cdot (\frac{1}{5})$
    - $P(a,a,a) \approx (\frac{1}{20}) \cdot (5) \cdot (\frac{1}{5})$
    - $P(a,a,a) \approx (\frac{5}{20}) \cdot (\frac{1}{5})$
    - $P(a,a,a) \approx (\frac{1}{4}) \cdot (\frac{1}{5}) = \frac{1}{20}$
2. $P(X_1=a, X_2=a, X_3=b)$

    - $P(a,a,b) \approx P(a) \cdot P(a | a) \cdot P(b | aa)$
    - $P(a,a,b) \approx (\frac{11}{20}) \cdot (\frac{5}{11}) \cdot (\frac{2}{5})$
    - $P(a,a,b) \approx (\frac{1}{20}) \cdot (5) \cdot (\frac{2}{5})$
    - $P(a,a,b) \approx (\frac{5}{20}) \cdot (\frac{2}{5})$
    - $P(a,a,b) \approx (\frac{1}{4}) \cdot (\frac{2}{5}) = \frac{2}{20} = \frac{1}{10}$

3. $P(X_1=a, X_2=a, X_3=c)$

    - $P(a,a,c) \approx P(a) \cdot P(a | a) \cdot P(c | aa)$
    - $P(a,a,c) \approx (\frac{11}{20}) \cdot (\frac{5}{11}) \cdot (\frac{1}{5})$
    - $P(a,a,c) \approx (\frac{1}{20}) \cdot (5) \cdot (\frac{1}{5})$
    - $P(a,a,c) \approx (\frac{5}{20}) \cdot (\frac{1}{5})$
    - $P(a,a,c) \approx (\frac{1}{4}) \cdot (\frac{1}{5}) = \frac{1}{20}$
...
-   **_Put the intuition of your code here_**
    Initialized a max probability to a negative number and a predicted character to None. Iterated through the set of possible characters, calculating the probability based on the sequence, said character, and the tables. If there is a higher probability for a character, it replaces the previous probability, and in the end returns the predicted character.
...
-   **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
    -   The next character would be b as it has the highest probability of 2/5.
...
The Alice in wonderland corpus worked a lot faster than the other corpus, war and peace. It ran things quickly, and even with 100 as the n value it ran in less than a couple of seconds. It was when I made the n=500 it took a while to complete, but still completed accurately. When the corpus was warandpeace, it took a couple of seconds even just for an n value of 20, while alice in wonderland did it instantly.
<hr>

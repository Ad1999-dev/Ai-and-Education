...
I first trained an RNN on a repeated alphabet sequence. With a small model and learning rate, it quickly achieved near-zero loss and accurately predicted the next letters in the sequence. I then trained on War and Peace using a larger model and reduced learning rate. After one epoch, the model reached a training loss of 1.6864 and began generating real English words and coherent phrases, showing it had learned meaningful patterns in the data.


2. Analysis on final train and test loss for both datasets

For the alphabet dataset, the model achieved a final train loss of approximately 0.0001 and test loss of 0.0002, reflecting perfect learning of the simple, repetitive pattern. In contrast, for the War and Peace dataset, the model reached a train loss of 1.3742 and a test loss of 1.4590 after three epochs. This higher loss reflects the complexity and variability of natural language. Despite the difference, the RNN successfully captured meaningful structure and produced coherent English-like output.

3. Explain impact of changing temperature

Changing the temperature affects the randomness of text generation.
At low temperatures (e.g., 0.5), the model produces more predictable and repetitive text.
At moderate values (1.0), the output is balanced and more creative.
At high temperatures (1.5+), the model generates surprising, but often nonsensical, text.
This allows for tuning between coherence and creativity.

4. Reflection

One challenge I faced was debugging tensor shape mismatches, especially during text generation. I had to carefully manage input dimensions and hidden states to ensure the model worked across both training and inference. Tuning hyperparameters was also tricky, particularly finding the right learning rate for the more complex War and Peace dataset. Through this process, I learned how RNNs maintain context over sequences and how temperature controls generation diversity. Overall, this assignment deepened my understanding of sequence modeling and the importance of thoughtful model design.

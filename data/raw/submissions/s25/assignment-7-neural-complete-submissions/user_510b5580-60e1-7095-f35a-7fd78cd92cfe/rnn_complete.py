...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        self.W_e = nn.Parameter(self.std * torch.randn(hidden_size, embedding_dim))
        self.W_h = nn.Parameter(self.std * torch.randn(hidden_size, hidden_size))
        self.b_h = nn.Parameter(self.std * torch.randn(hidden_size))

        self.W_o = nn.Parameter(self.std * torch.randn(output_size, hidden_size))
        self.b_o = nn.Parameter(self.std * torch.randn(output_size))
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            new_hidden = torch.tanh(torch.mm(x_embed[t], self.W_e.t()) +  # [b, e] * [emb_dim, hidden_size]
                                     torch.mm(h_t_minus_1, self.W_h.t()) +  # [b, hidden_size] * [hidden_size, hidden_size]
                                     self.b_h)
            output.append(new_hidden)
            h_t_minus_1 = new_hidden
        
        output = torch.stack(output)  # [l, b, hidden_size]
        # Transpose to [b, l, hidden_size]
        output = output.transpose(0, 1)  # [b, l, hidden_size]
        
        # TODO set these values after completing the loop above
        final_hidden = torch.clone(new_hidden) # [b, h]
        
        logits = torch.matmul(output, self.W_o.t()) + self.b_o  # [b, l, output_size]
...
char_to_idx = {char: index for index, char in enumerate(vocab)} # TODO: Create a mapping from characters to indices
idx_to_char = {index: char for index, char in enumerate(vocab)} # TODO: Create the reverse mapping
data = [char_to_idx[char] for char in sequence]

# TODO Understand and adjust the hyperparameters once the code is running
sequence_length =200 # 1000   # Length of each input sequence
stride = 10                   # Stride for creating sequences
embedding_dim = 32 #2         # Dimension of character embeddings
hidden_size = 256 #10         # Number of features in the hidden state of the RNN
learning_rate = 0.001 #0.15   # Learning rate for the optimizer
num_epochs = 2 #10            # Number of epochs to train
batch_size = 128 #16          # Batch size for training
...
test_dataset = CharDataset(test_data, int(sequence_length * 0.1), stride, output_size)
...
# TODO: Iterate through test_loader and perform a forward pass to compute predictions
    ...
    for batch_inputs, batch_targets in tqdm(test_loader):
        ...
        outputs, hidden = model(batch_inputs, hidden)  # Pass inputs through the model

        predictions = torch.argmax(outputs, dim=-1)

        correct += (predictions == batch_targets).sum().item()

        total += batch_inputs.size(0) * batch_targets.size(1)
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    for i in range(k):
        inputs = torch.tensor([[char_to_idx[char] for char in start_text]] * batch_size, dtype=int).to(device)

        outputs, hidden = model(inputs, hidden)  # Pass inputs through the model
        next_idx = sample_from_output(outputs[0], temperature)
        next_char  = idx_to_char[next_idx[m-1][0].item()]

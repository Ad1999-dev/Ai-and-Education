...
- We build n amount of tables with our outer table. We also need to run for the entire length of our corpus. From there we need to figure out how to get the a char and its previous context. So, we simply get the subtring from j to(not including) j - i + 1 which gives us a substring eqaul to the n gram value for that table. From there we can simply see the next for this context is at j - i + 1. So we know have a context and the next char. This can then just be plotted into the frequency table where for that context we incrmeent the amount of times the following character comes after to get a frequncy for that specic char. Simply put, getting a context and coutning the aomunt of times a char follows that context. This gives a proper table. 
...
$P(w_1, w_2, \ldots, w_m)$ =
$\prod_{i=1}^m
\frac{f\bigl(w_{i-n+1}, \ldots, w_{i-1}, w_i\bigr)}
     {f\bigl(w_{i-n+1}, \ldots, w_{i-1}\bigr)}$

### Code analysis

- ***Put the intuition of your code here***
We have already made our tables for a certain amount of frequncies based of some sorta of history length, like 0,1,2 excetera. This being n gram models 1,2,3. We then need to determine how much hisotry we can acutally use based off our tables and given sequnce. Like, given we only have one table, but have a sequence of 'aa'. It is not possible to calculate a hisotry greater than what we have. We then just have to choose what table to use, a specail case for unigram since it just the frequncy of the char over the size of our corpus. We also make sure to see if we can use our full seuqnuce by performing a cut on it, this helps to get the right contexts to look for. Then we can simply get that context from table. We then find the number of times char follwed that context and the total times any char followed that context. Then the prob is simply division between these two factors. 
### Your Calculations
...
= $\frac{11}{20}\,\times\,\frac{5}{10}\,\times\,\frac{1}{4}$
= $\frac{55}{800}$
$\approx 0.06875$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $P(a,a,b)$
= $P(a)\,\times\,P(a\mid a)\,\times\,P(b\mid aa)$
= $\frac{11}{20}\,\times\,\frac{5}{10}\,\times\,\frac{2}{4}$
= $\frac{110}{800}$
= $0.1375$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $P(a,a,c)$
= $P(a)\,\times\,P(a\mid a)\,\times\,P(c\mid aa)$
= $\frac{11}{20}\,\times\,\frac{5}{10}\,\times\,\frac{1}{4}$
= $\frac{55}{800}$
$\approx 0.06875$
...
- The code is just doing a next step prediction given a certain sequnce to see what the next char in our vocab is after a given sequnce. We already get the probablity for a given char given a seqeunce from the previous function. So all this code is doing ist just comparing all the chars in our vocab to see which one has the ighest prob given the sequnce from our calc prob func. 
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - The next char is b given seqeunce aa with prob 0.1375
...
Running a table with 8 grams takes a considerable amount of time, deifintly into the minutes. I found that going up from 5, to 8, and then 11 was deifnlty a exponetial increase in the amount of time it took to run the algoriihm. It took actaul ages. Running I love as my sequnece and a high 8 gram for the war and peace text reuslts in this, I love ////// a nonsenical answer where in the 5 n gram I got I love the. I also did a test with 15 n gram to see if my computer could handle it. I get a super long wait time. 

Running a simple message like I space with a n of 3 for both corpuses. It seems to me that the Alice and Wonderland corpus works better creating an actaul word statment like so I come(k=5) where as in the warandpeace corpus it results in something that does not even look like an english word I çell. 
<hr>

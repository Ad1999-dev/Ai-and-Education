...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        self.W_h = nn.Parameter(torch.randn(hidden_size, hidden_size) * 0.01)    # Hidden weights
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            x_t = x_embed[t]
            h_t = torch.tanh(
                torch.matmul(x_t, self.W_e.t()) +  # Input contribution
                torch.matmul(h_t_minus_1, self.W_h.t()) +  # Hidden contribution
                self.b_h  # Bias contribution
            )  # [b, hidden_size]
            
            # Append the current hidden state to the output list
            output.append(h_t)
            
            # Update h_t_minus_1 for the next timestep
            h_t_minus_1 = h_t
        ...
        final_hidden = h_t_minus_1.clone()  # Detach from graph to avoid tracking gradients
        logits = self.fc(output)  # Shape: [b, l, output_size]
...
total_test_loss = 0
hidden = None
with torch.no_grad():  # Disable gradient calculation for the test loop
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    # Hint: you will call sample_from_output() to sample a character from the logits
     # Prepare input sequence by converting characters to their respective indices
    input_sequence = [char_to_idx[char] for char in start_text]
    
    # Initialize the hidden state as None for the first input
    hidden = None
    # Generate k characters
    for _ in range(k):
        # Convert input_sequence to a tensor and ensure it's the right shape for the model
        input_tensor = torch.tensor(input_sequence[-sequence_length:], dtype=torch.long).unsqueeze(0).to(device)

        # Get the model's predictions for the next character
        logits, hidden = model(input_tensor, hidden)
        
        # Take the logits of the last character in the sequence
        last_char_logits = logits[:, -1, :]
        
        # Sample the next character index using the sampling function
        next_char_idx = sample_from_output(last_char_logits, temperature).item()
        
        # Append the predicted character index to the input sequence
        input_sequence.append(next_char_idx)
    
    # Convert generated indices back to characters
    generated_text = ''.join(idx_to_char[idx] for idx in input_sequence)

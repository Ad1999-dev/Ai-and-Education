...
- I experimented with various hyperparameter configurations across both datasets. For the alphabet sequence, training was very efficient and the model learned the pattern quickly with little tuning. I started with a small hidden size (32) and found it sufficient for this simple pattern. For War and Peace, I needed to increase model capacity significantly (hidden_size=256, embedding_dim=100) to capture the pattern of the text. One noticeable thing is about sequence length. For longer sequences (1000 characters for War and Peace) provided better context for prediction and the model did output a lower loss but it requires much more computational resources and time to train. Balancing stride (50) and batch size (64) was also crucial and optimal for effective training. Initially, I set the learning rate too high (0.1), causing unstable training, but reducing it to 0.01 yielded stable convergence. Since it took lots of time to run 1 epoch, and after testing it out, I feel like 1 epoch is enough to yield the satisfactory result.

2. Analysis on final train and test loss for both datasets
- For the alphabet sequence: The model achieved near-perfect performance with final training loss of approximately 0.0672 or 98.63% accuracy and test loss of 0.0016 or 99.95% accuracy. This demonstrates that the RNN can effectively memorize simple, deterministic patterns with minimal complexity.
- For War and Peace: The model achieved a training loss of approximately 1.5206 or 55.11% accuracy and a test loss of 1.4683 or 56.70% accuracy. This one has significantly higher loss compared to the alphabet sequence which reflects the complexity and unpredictability of natural language. Despite higher loss values, the model still learned meaningful character-level patterns that enabled it to generate text with recognizable word-like structures.

3. Explain impact of changing temperature
- I realized the higher the temperature parameter, the more diverse and unpredictable the generated text becomes. At small value below 1, the model produces highly deterministic, repetitive text that follows the most probable character sequences. With temperature=1.0 (the default), the model generates more varied but still coherent text reflecting the training distribution. At temperature=2 or more, the text becomes random and less coherent, with unusual character combinations and unconventional words appearing more frequently. This parameter helps controlling the trade-off between predictability and creativity in the generated output.

4. Reflection
- Implementing this character-level RNN teach me a lot about sequence modeling. The most challenging aspect was correctly implementing proper hidden state propagation through time steps. I feel that the choice of hyperparameters significantly affects both training efficiency and model performance, with the hidden size being very important for capturing complexity in natural language. In the beginning, to keep my model simple, I set the value too small and model not learnt very well so I adjusted it to a more appropriate number. The contrast between training on the alphabet versus War and Peace tell us about how sequence complexity affects model architecture and training time significantly. I found it fascinating how even a relatively simple RNN architecture could capture enough patterns to generate text with recognizable structure, despite having no explicit understanding of language rules. If I were to extend this project, I would experiment with more sophisticated architectures like LSTMs or Transformers to see how they might improve its generative power over longer sequences.

...
- 
We start by making a list of n empty dictionaries, one for each n-gram size.
We loop through 1 to n+1 and then grab every substring of that length.  We then count how many times each n-gram (substring) appears and store that count as the value in the dictionary. 
...
1. ***Write down your probability table 2***:
   - as in your probability table should look like (wait a second, you should know what I'm talking about)
        ...
        | $P(\odot)$ | Probability value |  
        | ------ | ----------------- |
        | $P(a \mid a)$ | $\dfrac{5}{11}\approx0.4545$ |
        | $P(b \mid a)$ | $\dfrac{3}{11}\approx0.2727$|
        | $P(c \mid a)$ | $\dfrac{2}{11}\approx0.1818$|

2. ***Write down your probability table 3***:
   -    | $P(\odot)$ | Probability value |  
        | ------ | ----------------- |
        | $P(a \mid aa)$ | $\dfrac{1}{5}\approx0.20$ |
        | $P(b \mid aa)$ | $\dfrac{2}{5}\approx0.40$|
        | $P(c \mid aa)$ | $\dfrac{1}{5}\approx0.20$|
...
Where:
- $$\( f(x_1, x_2, \dots, x_t, x_{t+1} \)$$ is the frequency of the full sequence including the next character.
- $$\( f(x_1, x_2, \dots, x_t) \)$$ is the frequency of the current sequence without the next character.
### Code analysis

- ***Put the intuition of your code here***
Find the size of the n-gram model and the length of the input sequence. Trim the sequence to keep onlt the n-1 characters. Build up the full n-gram by combining the sequence with the character you're checking. Look up the count of that full n-gram (sequence + char) in the frequency tables. Look up the count of the sequence and then return the probability as (frequency of full n-gram) ÷ (frequency of sequence), (count_ngram / denom). 
...
1. $P(X_1=a, X_2=a, X_3=a)$
   $$P(aaa) = P(a) \cdot P(a \mid a) \cdot P(a \mid aa)
= \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{1}{5}
= \frac{1}{20} = 0.05$$

2. $P(X_1=a, X_2=a, X_3=b)$
   $$P(aab) = P(a) \cdot P(a \mid a) \cdot P(b \mid aa)
= \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{2}{5}
= \frac{2}{20} = 0.10$$


3. $P(X_1=a, X_2=a, X_3=c)$
   $$P(aac) = P(a) \cdot P(a \mid a) \cdot P(c \mid aa)
= \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{2}{5}
= \frac{2}{20} = 0.10$$
...
- ***Put the intuition of your code here***
Start with the best probability at -1 and then loop through each character in the vocabulary ('a','b','c'). For each character calculate how likely it is to follow the sequence using Calculate probability. Track the most likely character by updating best_prob, return the best_char after checking all characters. 
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  ...
  -  P(aac) = 0.10 
  -  P(aaa) = 0.05
  Thus, either b or c could be the next character.  
...
I ran the autocomplete model on the "War and Peace" text using different values of 'n'. With a high n-gram value like 'n=15' the model was able to compute the sequence  `"battlefield"` into `"battlefield and returned to hi..."` This felt pretty strong contextual wise as it seemed to almost have a fluent output. WHen I raised the n-value the model took a lot longer to compute the sequences.    When I used 'n=9' I had the model complete `"the world"` as `"the world and let h..."`, this result also seemed pretty strong but I believe that if I had made the length longer with only n it would have been less accurate compared to a higher n-value (as their would be less data on the character probabilities). Personally, I found the range of n=12-15 with a k of 10-15 to be the most efficient in terms of results and speed.  

...
- When experimenting with the "abcde.." dataset, I was able to find the sweet spot in one epoch after bringing down the learning rate reasonably to 0.05. Having the sequence_length be the same as the number of unique vocab, stride 1, hidden_size brought up to 32. 
Similarly, when I changed the sequence_length to the vocab size for the warandpeace dataset, it had also worked better with other parameters kept the same. When the sequence_length was >= 100, the accuracy was lower.

- When the embedding dimension is also kept the same length as the vocab size, the accuracy is higher for abcde... but as for the warandpeace dataset, the accuracy is higher if it was kept much lower then the vocab size.

- The smaller the stride, the better the outcome. 

- If the hidden size was larger then the sequence length, e.g 64 as opposed to 32, the accuracy was smaller. 

- The learning rate is more reasonable to be kept around 0.01, 0.05, 0.001, 0.005. The best learning rate for the model so far has been 0.01. It has worked better with different hidden sizes and embedding dimensions. 

### 2. Analysis on final train and test loss for both datasets

Similarities 

- The amount of loss was similar around 2-4
- Both trained only on one epoch

Differences
- The test accuracy was much better for "abcde..." dataset and it makes sense because there was a lot of repetition. 
- Although the loss was smaller for both the datasets, the accuracy was much lower for warandpeace. And that is most likely because of the high variance in the data. There are different words and sentences and not much consistency. 

### 3. Explain impact of changing temperature

The more random it becomes, the things that gets generated are less like words. With a temperature of 1, there is still some logical order where things produced are okay to pronounce. 
A good mix of consonents and vowels. The more random it becomes, it becomes less okay to pronounce. 

### 4. Reflection

Challenges Faced: 

I had a lot of challenges with getting the right size for the tensors. First, I had trouble with the tensor dimensions when running them through the loss function. Then I had dimension errors with the target size in the testing loop and then with the tanh equation in the forward() of the model. 
383GPT was able to help me out with suggesting the unsqueeze(), and view() functions in a few places, but I had to use print functions to figure out where would be the best place to put the unsqueeze() function. 

Thought Process: 
I followed the TODO list for the char to idx mapping. 
While building the model, I knew that I had to have the tanh function to calcuate h(t-1) from the lecture. 
I had trouble with the logit as well. I was using a softmax function there, and I think it caused a few errors. When I later saw it being used in the sample output() function, and after reading the slides again, I realized I should be multiplied and used torch.matmul after getting dimension errors with torch.mm. 
When buidling the model, I first implemented the same structure as we had done for assignment 5: computing loss, then backpropagating. 
For the testing loop, I also integrated a few of the formatting we had in assignment5 and the training loop set up where hidden is set to None and also detached after getting the output from the model. 
In the generate_text() function, I set up the model to be on evaluation mode, and also initialized hidden as None, following how it was done in the testing loop. I then converted the input strings to integers using the mapping, turned the integers into a tensor. To predict the next char, I only selected the last character of the input (current_input) to be fed to the model. Then appended the new char to the input string after converting idx to char. 


### 5. Screenshots
![abcde training,testing,prediction best result](abcde_data.png)

![warandpeace training,testing,prediction best result](warandpeace_data.png)

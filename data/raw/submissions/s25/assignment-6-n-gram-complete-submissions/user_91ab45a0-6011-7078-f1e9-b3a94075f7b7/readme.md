...
- the output variable will be a list of n dictionaries.
  - Each dictionary consists of len(alphabet) dictionaries, where the outer dictionary's keys is the char we are finding the probability of, and the inner dictionary's keys are the sequences given.
  - each outer dictionary will have the same number of keys (len(x) keys), but the inner dictionary key length will increase exponentially for each table (for each $i$ in n, len(x)$^{i}$ to be exact)
- First, a list of unique characters is created, called alphabet
- Then the frequency table is created with all of the keys but all of the values (frequency numbers) are set to zero
- The frequency numbers are filled out by scanning through the document with 1-n+1 letters and adding to the frequency by one accordingly
...
- First, find Size(C) which is just adding together all the values in the first table
- Then calculate the first multiplicative term, $\frac{f(x_1)}{size(C)}$
- Then calculate the second multiplicative term $\frac{f(x_1,x_2)}{f(x_1)}$ and multiply it by the first term
- These two line will always happen as long as the sequence is at least one character. If the sequence is more than one character, the same pattern of the second term needs to be multiplied to the probability
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $P(X_1=a, X_2=a, X_3=a) = P(a) \cdot P(a \mid a) \cdot P(a \mid a) = \frac{f(a)}{size(C)} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(a,a)}{f(a)}$
   - = $\frac{11}{20} \cdot \frac{4}{11} \cdot \frac{4}{11}$
   - = $\frac{16}{220} = \frac{4}{55}$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $P(X_1=a, X_2=a, X_3=b) = P(a) \cdot P(a \mid a) \cdot P(b \mid a) = \frac{f(a)}{size(C)} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(b,a)}{f(a)}$
   - = $\frac{11}{20} \cdot \frac{4}{11} \cdot \frac{3}{11}$
   - = $\frac{12}{220} = \frac{3}{55}$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $P(X_1=a, X_2=a, X_3=c) = P(a) \cdot P(a \mid a) \cdot P(c \mid a) = \frac{f(a)}{size(C)} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(c,a)}{f(a)}$
   - = $\frac{11}{20} \cdot \frac{4}{11} \cdot \frac{2}{11}$
...
- Calculate the probabilities of each char in the vocabulary given the sequence
- Keep track of the maximum probability and its corresponding char
- After all the chars in vocabulary has its probability calculated and compared to the max, the char with the max probability gets returned
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - Since a has the highest probability given aa, a would most likely be next in the sequence.
...
No matter what number n you choose I think the time it takes to is run not fast at all, but any n that is 4 or greater is truly unbearable. For both texts. I think that Alice and Wonderland is faster for when n=2 and n=3 though.

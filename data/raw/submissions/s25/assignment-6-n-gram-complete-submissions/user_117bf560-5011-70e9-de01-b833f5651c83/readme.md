...
## Late Days
How many late days are you using for this assignment? 1 late day

## `create_frequency_tables(document, n)`

### Code analysis

My code has two helper methods. 

-The first is generate_strings, which takes in a vocabulary and the length n and creates a list of all possible strings of length n that can be made by the characters. 

-The second is string_to_freq, which takes in a string and turns it into a frequency string. For example if we were to have a string `"abb"`, then the frequency string would be `"f(b| a,b)"`. 

The create_frequency_tables begins by finding the vocabulary of the document, creating a set of characters based on the document and sorting it.

It then creates n frequency tables using a loop. Each iteration (except for the first), it uses generate_strings to find all possible strings of length i, and then uses string_to_frequency to convert all these strings into frequency strings. Each frequency string is appended to the dictionary with a value of 0. 

We then iterate over all substrings of length i in the document, increasing the corresponding frequency each time. 

In the case of a 1 character string, we create the frequency string without generate_strings (f(char)), and then increment it each time we run into the character in the document. 

After each loop is finished we append the i-frequency table into the list of frequency tables. After the loop is completed, we return the list of frequency tables.
...
$$P(X_1=x_1, X_2=x_2, X_3=x_3,) = P(x_1) \cdot P(x_2 \mid x_1) \cdot P(x_3 \mid x_1, x_2) = \frac{f(x_1)}{size(C)} \cdot \frac{f(x_1,x_2)}{f(x_1)} \cdot \frac{f(x_1, x_2,x_3)}{f(x_1, x_2)}$$

### Code analysis


The code finds the complete Gram by adding the character to the end of the sequence. For each character in the complete Gram, the code calculates the probability and multiplies it by the overall probability (initialized to 1).

There are four cases.

1. Probability of the first char, in this case we get the frequency of the first character in the overall document, and divide it by the total amount of characters in the document. The probability is multiplied by this value. We also use this case if n == 1.

2. Probability of the second char, in this case we get the frequency of the second character with respect to the first character, and divide it by the frequency of the first character. The probability is multiplied by this value.

3. If the index of the current char is greater than the number of frequency tables (n). Then we get the frequency of next char with respect to n previous chars and divide it by the frequency of the previous char with respect to the n-1 previous chars of it. The probability is multiplied by this value

4. In all other cases, we find the probability of the next char in the sequence with respect to the previous characters, and divide it by the probability of the previous char with respect to its previous characters. The probability is multiplied by this value

The overall probability is returned after looping through all characters in the combined sequence.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $P(X_1=a, X_2=a, X_3=a)$ = $P(X_1=a) * P(X_2=a| X_1=a) * P(X_3=a | X_1=a, X_2=a) = 
     \frac{11}{20} * \frac{5}{11} * \frac{1}{5} =  \frac{1}{20}$
2. $P(X_1=a, X_2=a, X_3=b)$

   - $P(X_1=a, X_2=a, X_3=b)$ = $P(X_1=a) * P(X_2=a| X_1=a) * P(X_3=b | X_1=a, X_2=a) = 
     \frac{11}{20} * \frac{5}{11} * \frac{2}{5} =  \frac{2}{20} =  \frac{1}{10}$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $P(X_1=a, X_2=a, X_3=c)$ = $P(X_1=a) * P(X_2=a| X_1=a) * P(X_3=c | X_1=a, X_2=a) = 
     \frac{11}{20} * \frac{5}{11} * \frac{1}{5} =  \frac{1}{20}$
...
-- The code makes a dictionary of probabilities for every character in the vocabulary. It then returns the character with the maximum probability from that dictionary

### So what should be the next character in the sequence?

-Based on the probability distribution obtained above, the next character in the sequence would be 'b'
...
I first tested out the war and peace text file. I was able to test up to tables of length n = 4, which took roughly a minute of time to continue. When I tried to test with 5 grams, the process was killed for taking too long. I next tested it with Alice's Adventure in Wonderland, which was able to generate tables up to lenth n  =  5 in a reasonable amount of time. I then tested it on a corpus with very few characters, the string "abcabacaba". This one was able to make tables up to n = 16 in length in reasonable time. From these tests I have concluded that the number of table entries grows exponentially with respect to the amount of unique characters in a corpus. The shorter a corpus, the bigger the tables I can have. But even more importantly, the less unique characters I have, the bigger a table I can have. This is because the amount of entries in a table increases by v times from tables of n size to n + 1 size, where v is the amount of characters in the vocabulary.

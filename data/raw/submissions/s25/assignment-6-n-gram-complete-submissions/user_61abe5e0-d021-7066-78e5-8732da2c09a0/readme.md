...
- This function builds frequency tables for an n-gram model using a given text document. It processes n-grams of sizes ranging from 1 to `n`, counting how often each character follows each n-gram. The function uses a nested dictionary to track these counts, and after processing, it converts these dictionaries into standard dictionaries. Finally, it returns a list of these frequency tables, illustrating character relationships at different levels of dependency.
...
This function generates a list of frequency tables for an n-gram model based on a given text document. It processes the document to create n-grams of size ranging from 1 to n. For each n-gram, it records the frequency of the character that follows it in a nested dictionary structure. The function iterates through the document, collecting these frequencies and storing them in separate dictionaries for each n-gram size. Finally, it returns a list of these frequency tables, which can be used to analyze character dependencies in the text.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $P(X_1=a, X_2=a, X_3=a) = P(a) \cdot P(a \mid a) \cdot P(a \mid a) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{5}{11} = \frac{5}{44}$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $P(X_1=a, X_2=a, X_3=b) = P(a) \cdot P(a \mid a) \cdot P(b \mid a) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{3}{11} = \frac{3}{44}$
   - *Show your work*
3. $P(X_1=a, X_2=a, X_3=c)$
   - $P(X_1=a, X_2=a, X_3=c) = P(a) \cdot P(a \mid a) \cdot P(c \mid a) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{2}{11} = \frac{1}{22}$
...
- This function predicts the next character in a sequence by evaluating the probability of each character in a given vocabulary following the input sequence. It initializes variables to track the maximum probability and the predicted character. For each character, it calculates the probability using the `calculate_probability()` function and updates the prediction if a higher probability is found. Finally, it returns the character with the highest probability as the predicted next character.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - The next character in the sequence would be a since it has the highest probability.
...
<hr>
In terms of n, I found that smaller values such as n = 2 or n = 3 resulted in the model being more efficient and running faster, but in turn lacking in predictions accuracy due to having less context.  With larger values such as n=6, the time and memory usage increased, but it resulted in more accurate predicions.  Additionally, larger corpus files performed better since they are more diverse and capture a greater number of patterns, while smaller corpus files led to less accurate results do to the lesser diversity provided.
<hr>

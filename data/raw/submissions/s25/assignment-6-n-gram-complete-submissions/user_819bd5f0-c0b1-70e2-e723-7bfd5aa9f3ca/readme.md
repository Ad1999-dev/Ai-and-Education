...
create_frequency_tables(document, n)
Intuition:
This function builds frequency tables for all n-gram sizes from 1 to n. Each table keeps track of how often a character appears after a specific context (which consists of up to n-1 previous characters). For example:

A unigram table counts individual character frequencies.

A bigram table counts how often each character follows a single preceding character.

A trigram table counts how often a character follows a two-character sequence, and so on.

These tables allow the model to condition its predictions on varying lengths of context, improving accuracy when more context is available.

calculate_probability(sequence, char, tables)
Intuition:
This function computes the probability of a given character char appearing after a context sequence. It:

Chooses the longest matching context available in the frequency tables.

Looks up how many times char follows that context.

Divides this count by the total number of characters that follow that context.

This gives the conditional probability P(char | context). If the context isn't found in any table, it returns 0.

predict_next_char(sequence, tables, vocabulary)
Intuition:
This function predicts the most likely next character based on the current context. It:

Iterates through all characters in the vocabulary.

Computes the probability of each character occurring next using calculate_probability.

Selects the character with the highest probability.

If all probabilities are zero (due to unseen context), it selects a random character from the vocabulary to avoid returning nothing.
...
| $P(\odot)$      | Probability value     |
|-----------------|-----------------------|
| $P(a \mid a)$   | $\frac{3}{9}$ = 0.333 |
| $P(b \mid a)$   | $\frac{3}{9}$ = 0.333 |
| $P(c \mid a)$   | $\frac{3}{9}$ = 0.333 |
| $P(a \mid b)$   | $\frac{3}{5}$ = 0.6   |
| $P(c \mid b)$   | $\frac{2}{5}$ = 0.4   |
| $P(a \mid c)$   | $\frac{4}{4}$ = 1.0   |

2. ***Write down your probability table 3***:

| $P(\odot)$      | Probability value     |
|-----------------|-----------------------|
| $P(a \mid aa)$  | $\frac{2}{4}$ = 0.5   |
| $P(b \mid aa)$  | $\frac{1}{4}$ = 0.25  |
| $P(c \mid aa)$  | $\frac{1}{4}$ = 0.25  |
...
- P(c∣s)= (count(s→c))/(∑v∈vocab count(s→v))
...
The function finds the longest possible matching context in the frequency tables. Then it:

Gets the frequency of the character after that context.

Divides it by the total frequency of all characters after that context. This gives the conditional probability of the character given the sequence.
...
### Using the Trigram Probabilities
...
$$
P(a) \cdot P(a \mid a) \cdot P(a \mid aa) = \frac{11}{20} \cdot \frac{3}{9} \cdot \frac{2}{4} = 0.55 \cdot 0.333 \cdot 0.5 = 0.0916
$$
...
$$
P(a) \cdot P(a \mid a) \cdot P(b \mid aa) = \frac{11}{20} \cdot \frac{3}{9} \cdot \frac{1}{4} = 0.55 \cdot 0.333 \cdot 0.25 = 0.0458
$$
...
$$
P(a) \cdot P(a \mid a) \cdot P(c \mid aa) = \frac{11}{20} \cdot \frac{3}{9} \cdot \frac{1}{4} = 0.55 \cdot 0.333 \cdot 0.25 = 0.0458
$$
...
Iterates through every character in the vocabulary.

Calculates the probability of each character following the given sequence using the calculate_probability function.

Returns the character with the highest probability.

If no context is found, it defaults to random.  
...
Answer: a (it has the highest probability) 
## Experiment
...
Smaller corpora work better with smaller values of n (e.g., n=2 or n=3). This is because higher values of n lead to data sparsity, where many n-gram sequences do not appear in the training data.

For larger corpora, we can go up to n=5 or n=6 without significant performance issues.

The main trade-off is accuracy vs. efficiency: higher n gives more accurate predictions due to more specific context, but it also increases memory usage and computational time significantly.

For the "aababcaccaaacbaabcaa" document, n=3 seems to provide a good balance—small enough to capture recurring patterns, but not so large that it suffers from sparsity.

...
- Similarly to what the readme says, changing the learning rate drastically changed the accuracy of the model. Changing the learning rate from 0.0001 to 0.001 increased the accuracy from 20% to 100%
- It was quite fast for the alphabet sequence, even when the number of epochs were quite high
- the sequence length can't be too high (can't be higher than ~250) or else there will be a divide by zero error
- if there were multiple epochs, the loss of each epoch would decrease
- Other than the learning rate, none of the other hyperparameters impacted the accuracy

#### War and Peace text:

- The same training and test loop as the alphabet sequence gave a matrix multiplication error. After lots of print statements and understanding the code, I realized that the matrix multiplication error during the war and peace text was that I didn't drop the remainder of the test set.
- The same hyperparameters that had 100% accuracy on the alphabet sequence had a 36% accuracy
- Learning rate also had the most impact on the accuracy here. Changing the other
- Increasing the stride makes the model train and test faster, but with less accuracy
- Increasing the batch size makes the model run faster because there are fewer batches to go through

2. Analysis on final train and test loss for both datasets

#### Alphabet Sequence:

- When the model predicted the next chars, if the sequence was only one letter the predictions were really bad. However, when the sequences were longer (3+ letters) the predictions were almost 100% accurate

#### War and Peace text:

- While the alphabet sequence just had a sequence of characters (with no spaces), this text predicts the characters in sensible words, but not sensible sentences. The predicted characters will cut off the last word though depending on what you inputted the number of characters the model should predict
- The loss is actually smaller than the loss of the alphabet sequence (almost half) at one epoch, but the accuracy is halved as well. The exact losses and accuracies are in the screenshots below

3. Explain impact of changing temperature

- the closer the temperature is to zero (like 0.000000000001), the more accurate the model is, even with shorter sequences and longer predictions
- as the temperature increases, the predictions become worse

| Sequence      | Temperature | Prediction                                        |
| ------------- | ----------- | ------------------------------------------------- |
| Alphabet      |             |                                                   |
| a             | 0.01        | bcdefghijklmnopqrstuvwxyzabcdefghijklmno          |
| a             | 0.1         | bcdefghijklmnopqrstuvwxyzabcdefghijklmno          |
| a             | 1           | bcdefghijklmnopqrstuvwxyzabcdefghijklmno          |
| a             | 5           | bbqjfghchevgeabcqylesxhrbqnscwklhaluhzwq          |
| ghi           | 0.1         | jklmnopqrstuvwxyzabcdefghijklmnopqrstuvw          |
| ghi           | 1           | jklmnopqrstuvwxyzabcdefghijklmnopqrstuvw          |
| ghi           | 5           | utveqhspugyijqrnbsiqdwzyirdessbbnqmbfmso          |
| War           | And         | Peace                                             |
| The           | 0.01        | conversation who was a man who was a man who      |
| The           | 0.1         | conversation who was a man who was a while her    |
| The           | 1           | aved mankher back, and which of spectacted in     |
| The           | 5           | sabuxieen;peo. efffoig cycb;oneenxvoauxrviv:b.    |
| The           | 10          | [o.y!p.iw?nldlvdunlunmeciyo!w!iyld;?malre,or:?od  |
| war and peace | 0.0001      | the followed the folds of the folds of the folds  |
| war and peace | 0.001       | the followed the folds of the folds of the folds  |
| war and peace | 0.01        | the followed the folds of the folds of the folds  |
| war and peace | 0.1         | the fold man of the fold man, said the followed t |
| war and peace | 1           | dicmorrally byett society a pvlovnas tosidely lik |

4. Reflection\
   I faced a lot of challenges. Even though I started as early as I can, I still faced many problems and had to go to office hours multiple times. I used the information gathered from lecture to implement the CharNN model, code from assignment 5 for the basis of the training and testing loop, as well as lots of 383gpt prompting to understand errors and the exact syntax of lines of code needed for implementation. A lot of the challenges I faced were multiplying matrices with incompatible dimensions, and going to office hours helped with debugging.\
   I also learned a lot about sequence modeling using RNNs with the fact that even though a model may have 100% accuracy, there are still lots of other factors that can make the model predict incorrectly like insufficient sequences and temperature rates.

5. Screenshots of datasets\
   ![Alphabet Model](/alphabetmodel.png "Alphabet Model")
   ![War and Peace Model](/warandpeacemodel.png "War and Peace Model")

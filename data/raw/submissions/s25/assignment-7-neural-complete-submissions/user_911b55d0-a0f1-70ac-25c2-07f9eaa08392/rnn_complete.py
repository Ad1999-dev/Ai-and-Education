...
class CharRNN(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        logits = output @ self.W_o + self.b_y # [b, l, vocab_size=v] 
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    start_indices = [char_to_idx[char] for char in start_text]
    start_tensor = torch.tensor(start_indices, dtype=torch.long).unsqueeze(0).to(device)
    hidden = None
    generated_text = start_text
    for _ in range(k):
        logits, hidden = model(start_tensor, hidden)
        last_logits = logits[0, -1].unsqueeze(0)     
        sampled_idx = sample_from_output(last_logits, temperature)   
        generated_text += idx_to_char[sampled_idx.item()]

        start_indices.append(sampled_idx.item())
        start_tensor = torch.tensor(start_indices[-n:], dtype=torch.long).unsqueeze(0).to(device)

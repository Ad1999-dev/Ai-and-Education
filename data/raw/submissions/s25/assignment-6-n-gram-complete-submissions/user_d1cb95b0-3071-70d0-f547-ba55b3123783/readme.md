...
- This function iterates through all characters in the input document and constructs frequency counts of character occurrences based on their preceding contexts. Specifically, it creates a list of dictionaries where each dictionary corresponds to a particular n-gram size (unigram, bigram, ..., n-gram). For each position in the document, the function updates how often each character follows each possible history of length up to n-1:
    - Firstly, initialize a list of n dictionaries, which represent our tables
    - Iterate over each character, while keeping track of its index, in the document.
    - For each character, it loops through possible n-gram sizes (1 to n):
        - Check if there are enough preceding characters to form the required history.
        - Slice the preceding characters to form the context (history).
        - Increment the count of occurrences for that character-history pair.
- Effectively, our data strucutre (a list of dicitonaries where each key's value is another dictionary), representes the frequency tables.
...
1. ***What is your vocabulary in this case***
    - {a, b, c}
        - Our vocabulary is the body of distinct characters used in the document.
...
- This function calculates the conditional probability using the frequency tables obtained from calculate_probaility(). It iterates over possible context lengths, beginning with the longest context available (up to n-1 characters) and gradually shortens it until a context with observed frequency is found.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $P(X_1=x_1, X_2=x_2, X_3=x_3) = P(x_1) \cdot P(x_2 \mid x_1) \cdot P(x_3 \mid x_1, x_2)$
   - $\Rightarrow P(X_1=a, X_2=a, X_3=a) = P(a) \cdot P(a \mid a) \cdot P(a \mid a, a)$
   - $\Rightarrow P(X_1=a, X_2=a, X_3=a) = \frac{11}{20} \cdot \frac{5}{10} \cdot \frac{1}{4} = \frac{11}{160}$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $P(X_1=a, X_2=a, X_3=b) = P(a) \cdot P(a \mid a) \cdot P(b \mid a, a)$
   - $\Rightarrow P(X_1=a, X_2=a, X_3=b) = \frac{11}{20} \cdot \frac{5}{10} \cdot \frac{1}{2} = \frac{11}{80}$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $P(X_1=a, X_2=a, X_3=c) = P(a) \cdot P(a \mid a) \cdot P(c \mid a, a)$
   - $\Rightarrow P(X_1=a, X_2=a, X_3=c) = \frac{11}{20} \cdot \frac{5}{10} \cdot \frac{1}{4} = \frac{11}{160}$
...
- This function predicts the most likely next character by evaluating the conditional probability for all characters. It iterates through the input vocabulary and computes the probability of each character following the provided sequence using calculate_probability. The character with the highest computed probability is chosen.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - Based on our sequence, the next character in the sequence would be the one with the highest conditional probability from our vocabulary. As obtained above, the next characer will be $\boxed{b}$. This is proven by the conditional probability: $P(b \mid a, a) = \frac{1}{2}$.
...
- For "War and Peace", the table calculation becomes too time consuming around n = 7. On the otherhand, "Alice's Adventures in Wonderland" table's calculation doesn't seem to become expensive regardless of how large n I use. This means it will most likely become expensive at an abnormally large n that won't be necessary to use (for this project atleast). After running some trials, it seems that "War and Peace" works better than the other corpus (notebally, it's a much larger document than "Alice's Adventures in Wonderland")

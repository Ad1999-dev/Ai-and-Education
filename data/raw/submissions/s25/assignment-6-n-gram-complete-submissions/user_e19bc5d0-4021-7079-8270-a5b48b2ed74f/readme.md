...
- Iterate from 1 to n, making a new table corresponding to the frequency for ngrams of that respective length n_i. To make the table, iterate over the entire document, getting each character sequence of length n_i - 1. If there is no sequence (n_i is 1), increase the frequency of that single character in the table by 1. Otherwise, if that sequence is not already in the table add it to the table as an empty dict, then for that corresponding sequence, increase the frequency of the character following that sequence by 1.
...
- $P(X_1, X_2, ..., X_m) = \Pi_{i=1}^m P(X_i \mid X_{i - n + 1}, ..., X_{i-1})$ where if $n = 1$ or $i = 1$ then $P(X_i \mid X_{i - n + 1}, ..., X_{i-1}) = P(X_i) = \frac{f(X_i)}{size(C)}$ otherwise $P(X_i \mid X_{i - n + 1}, ..., X_{i-1}) = \frac{f(X_{i - n + 1, ..., X_i})}{f(X_{i - n + 1}, ..., X_{i-1})}$

### Code analysis

- Get the maximum n that we can use based on how many tables we have. Conduct the length of the given sequence concatenated with the given character iterations. For each iteration, calculate n as the minimum of the max n and the number of characters in the sequence + char we have already iterated over, get the current character, and the prefix as the n - 1 characters before that character. If n is 1, there is no characters before our current character so just multiply our total probability by P(char) which is the frequency of that character divided by the total number of characters in the corpus if char is in the table and otherwise that character is not in the corpus at all so return 0. If n is not 1, if the prefix is in the table and the character is in the table for that prefix, calculate the conditional probability of having that character after the sequence meaning calculate the frequency of the sequence and char divided by the frequency of the sequence, otherwise return 0 because the corpus has not seen that combination of characters.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $=P(a) * P(a \mid a) * P(a \mid aa) = \frac{11}{20} * \frac{5}{11} * \frac{1}{5} = 0.05$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $=P(a) * P(a \mid a) * P(b \mid aa) = \frac{11}{20} * \frac{5}{11} * \frac{2}{5} = 0.1$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $=P(a) * P(a \mid a) * P(c \mid aa) = \frac{11}{20} * \frac{5}{11} * \frac{1}{5} = 0.05$

## `predict_next_char(sequence, tables, vocabulary)`

### Code analysis

- Iterates over all of the characters in the vocabulary and calculates the probability for the sequence char combination for each char, keeping track of the character that has the highest probability.
...
From experimentation, I have noticed if n = 1, it always just makes the next character the most frequent character in the corpus which is ' ' for both texts. With the warandpeace corpus, n = 25 takes about a minute so anything after n = 25 I would consider to be too time consuming. For the Alice's Adventures in Wonderland corpus, large n such as n = 400 runs relatively quickly but anything after that my VSCode crashes because I do not have enough RAM. To determine which corpus works better, I am testing each corpus with n = 10 on the autocompletion of common starts of sentences including "thank", "i have", "the", "my", and "you". In general, War and Peace as the corpus performed more consistently. Overall it seems to be better since the autocompleted sentences made more sense. I noticed that the completions from War and Peace tended to use very formal language where as the completions for Alice's Adventures in Wonderland were more creative and informal.

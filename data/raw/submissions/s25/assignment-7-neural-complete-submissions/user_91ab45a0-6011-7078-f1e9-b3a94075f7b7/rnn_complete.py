...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        self.W_h = nn.Parameter(torch.randn(hidden_size, hidden_size) * 0.01) # Hidden state weights
        self.W_e = nn.Parameter(torch.randn(embedding_dim, hidden_size) * 0.01) # Input to hidden layer weights
        self.W_y = nn.Parameter(torch.randn(hidden_size, output_size) * 0.01) # Hidden to output layer weights
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            x_t = x_embed[t]  # [b, e]
            h_t = torch.tanh(torch.matmul(x_t, self.W_e) + torch.matmul(h_t_minus_1, self.W_h))
            output.append(h_t)
            h_t_minus_1 = h_t
        output = torch.stack(output) # [l, b, e]
        # print(f"[l, b, e]: {output.shape}")
        output = output.transpose(0, 1) # [b, l, e]
        # print(f"[b, l, e]: {output.shape}")
        
        # TODO set these values after completing the loop above
        final_hidden = h_t.clone() # [b, h] 
        logits = torch.matmul(output, self.W_y)  # [b, l, vocab_size=v] 
...
# sequence = sequence[:int(len(sequence)/20)]
...
for epoch in range(num_epochs):
    ...
    for batch_inputs, batch_targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        loss = criterion(output.view(-1, output.shape[-1]), batch_targets.view(-1))  # Reshape output and targets
        loss.backward()  # Backpropagation
        optimizer.step()  # Update model parameters
        total_loss += loss.item()  # Accumulate the total loss for this batch
...
with torch.no_grad():
    ...
    for batch_inputs, batch_targets in test_loader:
        outputs, hidden = model(batch_inputs, hidden)  # Forward pass through the model
        hidden = hidden.detach()
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    # Prepare input tensor from the start_text
    input_sequence = torch.tensor([char_to_idx[c] for c in start_text], dtype=torch.long).unsqueeze(0)  # Shape: [1, n]
    hidden = None
    generated_text = ""

    for _ in range(k):  # Generate k additional characters
        # Get model predictions
        output, hidden = model(input_sequence.to(device), hidden)  # Forward pass through the model
        hidden = hidden.detach()  # Detach hidden state for next iteration
        
        # Apply temperature scaling and sample from the output distributions
        logits = output[:, -1, :]  # last timestep's predictions
        probabilities = F.softmax(logits / temperature, dim=-1)
        next_char_idx = torch.multinomial(probabilities.squeeze(0), 1)  # Sample the next character index
        
        next_char = idx_to_char[next_char_idx.item()]
        generated_text += next_char

        # Prepare input for the next iteration
        input_sequence = torch.cat((input_sequence, next_char_idx.unsqueeze(0)), dim=1)  # [1, n + current_character]

...
My experiments involved training the character-level RNN on two distinct datasets. First, I used a simple, repetitive sequence ("abcdefghijklmnopqrstuvwxyz" * 100) to verify the basic RNN implementation. For this dataset, using hyperparameters like hidden_size=64, lr=0.01, embedding_dim=20, and training for 5 epochs, I observed very rapid convergence. The loss decreased dramatically within the first epoch, and the model quickly achieved near-perfect accuracy, demonstrating its ability to learn simple, deterministic sequential patterns with minimal tuning.

The second experiment used the warandpeace.txt corpus. This presented a much more complex task, requiring significant adjustments. Initial hyperparameter settings from the alphabet task were insufficient. I iteratively tuned parameters, finding that increased model capacity and a smaller learning rate were necessary. I settled on hidden_size=256, embedding_dim=64, lr=0.003, and batch_size=128. Training even for a single epoch took noticeably longer (around 6-7 minutes on MPS) compared to the alphabet task. Convergence was slower, and the model achieved substantially lower final accuracy, reflecting the inherent complexity and statistical variety of natural language.

2. Analysis on final train and test loss for both datasets

The performance metrics clearly reflected the difference in dataset complexity:

Dataset:	Final Avg Training Loss |	Final Test Loss	| Final Test Accuracy
Alphabet Sequence:	0.0085 |	0.0092 |	0.9971
War and Peace:	1.6451 |	1.6883 |	0.5735

The alphabet sequence resulted in extremely low loss and near-perfect accuracy because the pattern is perfectly predictable after one cycle. The RNN could easily memorize and replicate the fixed sequence. In contrast, warandpeace.txt contains complex grammatical structures, a vast vocabulary, and subtle statistical dependencies between characters. While the model learned significant patterns (achieving ~57% accuracy is far better than random chance), predicting the single correct next character in such rich text is inherently difficult, leading to significantly higher loss and lower accuracy compared to the deterministic alphabet task.

3. Explain impact of changing temperature

The temperature parameter significantly impacts the randomness and perceived "creativity" of the generated text by scaling the logits before the softmax probability calculation. Lower temperatures sharpen the probability distribution, making the model favor the most likely next character (more predictable, potentially repetitive). Higher temperatures flatten the distribution, increasing the chance of sampling less probable characters (more surprising, potentially nonsensical).

Using the starting text "the countess" with the War and Peace model, I observed the following:

Temperature 0.5: the countess was the same the same the same the same the same the same the same the same the same the same the same the same the same the same the same the same the same the same the same the same the same the same the same the sam (Quickly fell into a repetitive loop, sticking to very high-probability sequences).
Temperature 1.0: the countess was a stranition of the conversation of the princess. the prince was standing in the drawing room when the conversation and the princess mary had not been talking to her face and the countess was not to say that h (Generated more varied text, forming plausible word fragments and common phrases like "the princess", "drawing room", showing a balance).
Temperature 1.5: the countess!why did not seem to him, buxlew; (of his hands of pierre's should her head?, wropked thiskly and she saw himselfful) her heard?, i may be alpatych, lawi; she smile of his shound have seen expression, and, and (Much more random, included misspellings like "wropked", non-words like "buxlew", less coherent structure, but occasionally generated surprising juxtapositions).
This demonstrates the clear trade-off: lower temperatures provide more coherent but potentially boring output, while higher temperatures offer more novelty at the cost of correctness and structure.

4. Reflection

Implementing the CharRNN provided valuable hands-on experience with sequence modeling. The most significant challenge was hyperparameter tuning for the warandpeace.txt dataset; finding the right balance for learning rate, hidden size, and embedding dimension to facilitate learning without excessive training time or instability required careful iteration. Debugging the forward pass, ensuring correct tensor shapes and hidden state propagation between timesteps and batches, was also critical. Following the assignment structure by validating on the alphabet task first proved essential for confirming the core RNN logic worked before tackling the complexity of natural language.

My key insight was a deeper appreciation for how the hidden state acts as a compressed summary of the sequence history, enabling the model to make context-aware predictions. The concept of character embeddings transforming discrete symbols into meaningful vector space representations became clearer. Experimenting with temperature vividly illustrated the probabilistic nature of generation and the control we have over the model's output style. While this simple RNN captured character-level statistics effectively, its limitations in handling long-range dependencies highlighted why more complex architectures like LSTMs or Transformers are often necessary for sophisticated language tasks.

5. IMPORTANT: Include screenshot of output for generate_text() function: 

[text](ssassignment7.pdf)

...
In the `CharDataset` class you will notice a concept of `stride` is used. When creating the training data for a character-level language model, we break long text intornn shorter overlapping sequences so the model can learn from many parts of the text.

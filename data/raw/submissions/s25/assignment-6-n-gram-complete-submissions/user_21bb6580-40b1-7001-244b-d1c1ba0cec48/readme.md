...
- My implementation creates n frequency tables for ngram models of lengths 1 to n. For each n-gram level, I iterate through the document to count how often each character appears given its preceding context. For unigrams (n=1), I count individual character frequencies. For higher n-grams, I split each sequence into its context (previous characters) and the target character, incrementing the count in a nested dictionary structure where the outer key is the character and the inner key is its context.
...
### Formula
- For an n-gram model, the formula for calculating the probability of a character given a sequence is:
$$P(X_{t+1}=x_{t+1} | X_{t-n+2}=x_{t-n+2},...,X_t=x_t) = \frac{f(x_{t-n+2},...,x_t,x_{t+1})}{f(x_{t-n+2},...,x_t)}$$
Where f() represents the frequency count from the n-gram tables.

### Code analysis

- My implementation:
Determines the appropriate context size based on the sequence length and n-gram order
Extracts the relevant context from the end of the sequence
Selects the right frequency table based on context size
Calculates the probability as the ratio of (count of sequence+char) to (count of sequence)
Handles edge cases where the character or context is not in our tables
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - Using the bigram model for the transition from X1 to X2: $P(X_2=a|X_1=a) = \frac{5}{11}$
    Using the trigram model for the transition from X2 to X3: $P(X_3=a|X_1=a,X_2=a) = \frac{2}{5}$
    Therefore: $P(X_1=a,X_2=a,X_3=a) = P(X_1=a) \cdot P(X_2=a|X_1=a) \cdot P(X_3=a|X_1=a,X_2=a) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{2}{5} = \frac{11}{100}$
2. $P(X_1=a, X_2=a, X_3=b)$
   - Using the same approach but with X3=b:
    Since $P(X_3=b|X_1=a,X_2=a) = 0$ (not observed in training data)
    Therefore: $P(X_1=a,X_2=a,X_3=b) = \frac{11}{20} \cdot \frac{5}{11} \cdot 0 = 0$
3. $P(X_1=a, X_2=a, X_3=c)$
   - Using the same approach but with X3=c:
    $P(X_3=c|X_1=a,X_2=a) = \frac{3}{5}$
    Therefore: $P(X_1=a,X_2=a,X_3=c) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{3}{5} = \frac{3}{20}$
...
- My implementation predicts the next character by:
Iterating through all characters in the vocabulary
Calculating the probability of each character appearing after the given sequence
Selecting the character with the highest probability
Handling the edge case where all probabilities are zero by returning a default character
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - Based on the probability calculations:
P(a|aa) = 0.4
P(b|aa) = 0
P(c|aa) = 0.6
The next character should be "c" since it has the highest probability of 0.6.
...
I experimented with different corpus files and n values. Small values of n (2-3) worked well across all corpora, providing a good balance between model accuracy and computational efficiency. For the War and Peace corpus, n=4 produced noticeably better completions but took significantly longer to compute. For n=5 and higher, the computation time increased exponentially while the quality improvements diminished. The Sherlock Holmes corpus performed well with n=3, generating more coherent completions than simpler texts. The model struggled with specialized vocabulary in technical documents, regardless of n value. In practice, n=3 seems to be the sweet spot for most general-purpose text, with n=4 being viable for smaller corpora where computational constraints are less significant.
<hr>

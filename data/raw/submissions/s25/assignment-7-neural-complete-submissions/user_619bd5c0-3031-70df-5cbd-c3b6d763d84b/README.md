...
For this assignment, I experimented with training a character-level RNN on two datasets: a synthetic one composed of repeated alphabet sequences, and a real-world dataset using the full text of War and Peace. The synthetic dataset allowed for quick iteration and debugging, and the model was able to learn its simple patterns quickly, resulting in low final train and test loss. However, when switching to War and Peace, a much larger and more complex dataset, I needed to significantly adjust the hyperparameters—such as increasing the hidden size, embedding dimensions—to handle the increased vocabulary diversity and longer-range dependencies. The final train and test loss were higher on the real dataset, reflecting the model's challenge in capturing its complexity. 

2. Analysis on final train and test loss for both datasets

synthetic data set: Train loss (20 epoch) = 0.0108, Test Loss = 0.0058

war and peace data set: Train loss (2 epoch) = 1.4243, Test loss = 1.4850

3. Explain impact of changing temperature

At low temperatures (e.g., 0.2), the output was repetitive and safe, often looping similar words or phrases. As I increased the temperature to around 0.7, the output became more varied and creative while still maintaining coherence. At high temperatures like 1.5, the text became erratic and often nonsensical, illustrating how temperature controls randomness in sampling. Overall, this project deepened my understanding of sequence modeling and hyperparameter tuning. It also showed the importance of dataset complexity in model behavior and how tuning the temperature can greatly influence the quality and creativity of generated text.

4. Reflection

This assignment helped me understand how character-level language models learn from sequences and the importance of tuning hyperparameters based on dataset complexity. The synthetic dataset was helpful for sanity checks, but the real challenge came with the full text of War and Peace, where I saw how crucial the sequence length, hidden size, and number of training epochs are to producing reasonable outputs.

The generate_text() function gave somewhat meaningful outputs at mid-range temperatures between 0 and 1, and I was able to observe how randomness is controlled in text generation through temperature. From this I learned that RNN's are very effective after extensive training however that training typically requires ample time and processing to be effective. 

5. IMPORTANT: Include screenshot of output for generate_text() function: 
![generated text output](<Screenshot 2025-05-02 194308.png>)
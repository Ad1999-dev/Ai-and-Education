...
## 383GPT
Did you use 383GPT at all for this assignment (yes/no)? <br> Yes

## Late Days
How many late days are you using for this assignment? <br> 1

## `create_frequency_tables(document, n)`

### Code analysis

- Since we want to create n frequency tables to represent tables for 1 up to n-grams, I want to create a dictionary for each of those tables. Therefore I basically go through the document and move my window (of size i, which goes up to n) through the document to record the frequency of each i-gram. 
...
**Note:** The probabilities doesn't add up to 1 because 'a' is the ending character so technically, $P("" \mid a)$ = $\frac{1}{11}$.
...
**Note:** Again, the probabilities doesn't add up to 1 because 'aa' is the ending string so techinically, $P("" \mid aa)$ = $\frac{1}{5}$.


## `calculate_probability(sequence, char, tables)`

### Formula
- $P(X_1 = x_1, X_2 = x_2, X_3 = x_3) = P(x_1)*P(x_2 \mid x_1)*P(x_3 \mid x_1, x_2)$

### Code analysis

- For each of the probability terms, we need to calculate the numerator and denominator. Therefore, we need to take into account of the maximum context window (n-1) so we know where to start in a document. We then just obtain the frequency of the term from the start of the context window to our current char in the document and divide that by the frequency of the term from the start of the context window to our previous char. Keep in mind that if we are on the first letter of our document, we need to constrain our context window such that it's not negative and that we aren't accessing the frequency of the term from the start of the context window to our previous char since it's invalid. Instead we divide by the sum of all the occurrences of 1-grams (characters).
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - = $$P(X_1=a) * P(X_2=a \mid X_1 = a) * P(X_3 = a \mid X_1 = a, X_2 = a) = \frac{11}{20} * \frac{5}{11} * \frac{1}{5} = \frac{1}{20}$$
2. $P(X_1=a, X_2=a, X_3=b)$
   - = $$P(X_1=a) * P(X_2=a \mid X_1 = a) * P(X_3 = b \mid X_1 = a, X_2 = a) = \frac{11}{20} * \frac{5}{11} * \frac{2}{5} = \frac{1}{10}$$
3. $P(X_1=a, X_2=a, X_3=c)$
   - = $$P(X_1=a) * P(X_2=a \mid X_1 = a) * P(X_3 = c \mid X_1 = a, X_2 = a)= \frac{11}{20} * \frac{5}{11} * \frac{1}{5} = \frac{1}{20}$$
...
- For each character in the vocabulary, we need to check the $P(char | sequence)$, so we have to calculate $P(sequence+char)/P(sequence)$. However, I noticed that every character has $P(sequence)$ in the denominator so we don't have to actually calculate it since we are only interested in the larger probability, so if the numerator is larger, then we want to return the character corresponding to that probability. Therefore, we just calculate $P(sequence+char)$ by using calculate_probability and updating the character and highest_prob if that probability is larger than our current highest_prob.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - 'b' since it has the largest probability of 1/10.
...
<hr>
The bigger the corpus files, the better it works. For instance, since in the "Alice's Adventures in Wonderland.txt" doesn't have any occurrences of "absolutely", when I set my initial sequence to be "This pizza is absolutely", it doesn't give me any suggestions as to what my next character should be. However, when I changed it to the "warandpeace.txt", it gave me suggestions since it has that word in its text. 

However, when I set n = 5, it gave me something nonsensical. When k = 40, it gave me this final updated sequence: "This pizza is absolutely and the said the said the said the said". When I set n = 10, it gave me this instead: "This pizza is absolutely necessary to see him and the same time" which might still not make too much sense but it's grammatically correct unlike when n = 5. This took around 8 seconds and I tried setting n = 20 and it took around 20 seconds respectively to compute, which is pretty time consuming considering the fact that the whole purpose of an autocomplete feature is to help someone finish what they're trying to type faster.

...
- The `create_frequency_tables` function generates frequency tables for n-grams from a given text document, allowing for statistical analysis of character sequences. By extracting substrings of increasing lengths (from unigrams to n-grams), the function counts how often each substring appears in the document. This helps capture the structure and patterns within the text, which can be useful for tasks like predictive text generation or language modeling. Ultimately, it provides a foundational understanding of character relationships, aiding in the development of models that can predict subsequent characters based on observed sequences.
...
- The `calculate_probability` function estimates how likely a character is to follow a given sequence based on previously learned n-gram frequency tables. It determines the relevant context from the sequence (up to n−1 characters) and calculates the conditional probability of the character appearing next. If no context is available, it uses the overall frequency of the character (unigram probability). For longer contexts, it checks how often the combined sequence (context + character) appears compared to the context alone. This method enables context-aware predictions and helps capture dependencies in text, making it valuable for tasks like text completion or generation.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   ...
   - $\frac{11}{20} \cdot \frac{1}{2} \cdot \frac{1}{4}$
   - $\frac{11}{160}$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $P(X_1) \cdot P(X_2 \mid X_1) \cdot P(X_3 \mid X_1, X_2)$
   - $P(a) \cdot P(a \mid a) \cdot P(b \mid a, a)$
   - $\frac{11}{20} \cdot \frac{1}{2} \cdot \frac{1}{2}$
   - $\frac{11}{80}$
...
- The `predict_next_char` function predicts the most likely character to follow a given sequence using previously constructed n-gram frequency tables. It extracts the most relevant context from the end of the sequence (up to n−1 characters) and evaluates the probability of each character in the vocabulary occurring next. By calling calculate_probability for each possible next character, it selects the one with the highest probability. This approach allows the model to make informed predictions based on learned character patterns, enabling applications like autocomplete, text generation, or language modeling with a focus on statistical likelihoods.
...
- The prediciton was better with Alice's Adventures in Wonderland because of its consistent style of storytelling I believe. I feel values of n being 3 or 4 work best with 5 and 6 being a balance between performance and predictive quality before computational efficiency becomes a concern.

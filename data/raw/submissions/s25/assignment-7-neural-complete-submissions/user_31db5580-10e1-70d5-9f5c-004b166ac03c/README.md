...
I started by training the model on the simple alphabe sequence. The goal was for the RNN to figure out the pattern and predict the next letter. After adjusting some parameters, I noticed the loss went down a lot over time. The model learned the pattern pretty quickly and could generate the alphabet sequence almost perfectly using generate_text().
Next, I trained it on warandpeace.txt, which was a lot more complicated. Natural language has way more variation, so I had to tune the hyperparameters more carefully. Training took longer, and the loss stayed higher compared to the alphabet experiment. The generated text wasn’t totally clear, but it came up with fake words and letter combinations that looked kind of like real language.
I tried different values for n (length of generated text), k (top-k sampling), and temperature to see how they affect the output. I noticed that higher k values made the text less random but sometimes repetitive. Lower temperature produced more predictable text, while higher values led to more creative but less coherent output. 

2. Analysis on final train and test loss for both datasets
For the alphabet dataset, the model learned the pattern really quickly. By the end of 5 epochs, the training loss dropped from 0.4770 in the first epoch to just 0.0003 in the last epoch. The test loss was even lower at 0.0002. This shows the model was able to memorize and predict the repeating sequence with almost perfect accuracy.
For the warandpeace.txt dataset, the model showed some improvement over 2 epochs. The training loss dropped from 1.6826 to 1.5617, and the test loss ended up at 1.6120. Even though the loss went down a bit, it was still much higher than with the alphabet dataset because the language is a lot more unpredictable and complicated. The model started to learn patterns, but it would probably need more epochs and tuning to get better results.

3. Explain impact of changing temperature
It controls the randomness of character. A lower temperatre gives more safe, predictable, repeating answers. A lower range is typically under 0.7. Anything higher ranging higher than 1.0 makes the model be more creative but often innacurate.

4. Reflection
This assignment helped me understand how text generation models balance randomness and predictability. I also learned how hypertuning parameters changes the style and accuracy of generated text. It also helped me reinforce my understanding of RNNs and sequence modeling. Implementing the model form scratch definitely felt like an achievemnt. There's a very big difference between how the model works on the alphabet sequence as comapred to the real world text "war and peace" which is given to us. Both required different hyperparameter tuning, it highlights how model complexity and hyperparameter tuning affect the outcomes. SUbtle changes in the epoch number or values of k & temperature can change how the model outputs. Helped understand the design of LLMs and trade offs. Definitely a very hands on project, built intuition and understanding.

...
- Going through from N = 1 to n for N-gram with n is the parameter, my implementation first converts the document to lowercase to take care of case sensitive issue. For each value of i from 1 to n, I create a frequency table that counts the occurrences of each i-gram in the document. The algorithm slides a window of size i over the document, extracting all possible i-grams and incrementing their counts in the corresponding frequency table. This produces n tables that each contains frequencies of sequences of different lengths, which are essential for calculating conditional probabilities in the n-gram model later.
...
2. ***Write down your probability table 3***:
   - You got this!
        ...
        | $P(a \mid cb)$ | $1$ |
        | $P(a \mid cc)$ | $1$ |
## Note: There are some combinations which yield probability value of 0 have been trimmed off the table!
...
- ***Write the formula for sequence likelihood as described in section 2***
  ...
  For an n-gram model, the probability of the next character given a sequence is:
  
  $$P(X_{t+1}=x_{t+1} \mid X_{t-n+2}=x_{t-n+2}, \ldots, X_t=x_t) = \frac{f(x_{t-n+2}, \ldots, x_t, x_{t+1})}{f(x_{t-n+2}, \ldots, x_t)}$$

### Code analysis

My `calculate_probability()` implementation applies the chain rule to compute the joint probability P(sequence, char). We will first combines the input sequence and character into a full new sequence. There are 2 cases and for the first character: calculates P(x₁) using unigram frequencies. For subsequent characters: calculates P(xᵢ|context) where context includes up to n-1 previous characters. At the same time, multiplies all terms to get the joint probability. My function also handles edge cases like unknown sequences (which will then return 0) based on position and the n-gram model size, making sure that it is make correct probability calculations and not getting error(like division by 0).
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - Using the chain rule: $P(X_1=a, X_2=a, X_3=a) = P(X_1=a) \cdot P(X_2=a|X_1=a) \cdot P(X_3=a|X_1=a,X_2=a)$
   - From our tables: $P(X_1=a) = \frac{11}{20}, P(X_2=a|X_1=a) = \frac{5}{11}, P(X_3=a|X_1=a,X_2=a) = \frac{1}{5}$
   - Therefore: $P(X_1=a, X_2=a, X_3=a) = \frac{1}{20} = 0.05$

2. $P(X_1=a, X_2=a, X_3=b)$
   - Using the chain rule: $P(X_1=a, X_2=a, X_3=b) = P(X_1=a) \cdot P(X_2=a|X_1=a) \cdot P(X_3=b|X_1=a,X_2=a)$
   - From our tables: $P(X_1=a) = \frac{11}{20}, P(X_2=a|X_1=a) = \frac{5}{11}, P(X_3=b|X_1=a,X_2=a) = \frac{2}{5}$
   - Therefore: $P(X_1=a, X_2=a, X_3=b) =\frac{1}{10} = 0.1$

3. $P(X_1=a, X_2=a, X_3=c)$
   - Using the chain rule: $P(X_1=a, X_2=a, X_3=c) = P(X_1=a) \cdot P(X_2=a|X_1=a) \cdot P(X_3=c|X_1=a,X_2=a)$
   - From our tables: $P(X_1=a) = \frac{11}{20}, P(X_2=a|X_1=a) = \frac{5}{11}, P(X_3=c|X_1=a,X_2=a) = \frac{1}{5}$
   - Therefore: $P(X_1=a, X_2=a, X_3=c) = \frac{1}{20} = 0.05$
...
The `predict_next_char` function will first iterates through all possible characters in the vocabulary. Then, for each character, calculates its probability following the input sequence. We will have to keep track of the character with the highest probability, and finally returns the optimal character( the one with highest probability) as the prediction.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  From our calculations above for the sequence "aa", both 'a' and 'c' have the same probability of 0.05 while b has probability of 0.1, which is the highest one we have. Hence, we will pick b as our next possible character, and hence the updated string will be "aab"
...
In my experiments with different corpus files and n-values, I found that larger corpus files (in terms of number of words) provide more predictions compared to smaller texts. This is because they contain more examples of different character sequences, giving the model better statistics to work with. However, the time it takes also vary significantly. For example, I observe that Alice's ADventure text file can run with big n pretty fast while it will take a while for warandpeace text when running with same n. The reason for this is clearly Alice text file is larger in sie and has more words count compared to warandpeace file.

Regarding the n-value, I observed that around n=4 or 5 is the best in general. With n=1 or n=2, the predictions will be too narrow and lacked context. As n increased to 4-5, predictions became much more contextually informative and often matched with the text patterns. The predicted words make more sense and natural. However, beyond n=6, I noticed prediction quality is dropping while computation time increased significantly. At n=8 and above(like 10), the frequency tables became extremely sparse as many sequences had zero occurrences in the corpus, and it takes so much longer. In other words, the context is too narrow for the model so it does not generate very well. Most of the time it will just give me whitespace instead of letters too.

Hence, we learnt that deciding and coming up with an optimal n value and having a good text file as the document is very important.

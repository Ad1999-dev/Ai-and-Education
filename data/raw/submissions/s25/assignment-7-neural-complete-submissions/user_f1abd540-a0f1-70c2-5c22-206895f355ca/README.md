...
1. Describe your experiments and observations-In the experiment, I trained a character-level RNN (CharRNN) to predict the next character in a sequence based on prior characters. The model was trained on a dataset constructed from repeated sequences of the English alphabet and then tested with varying initial inputs to generate characters.
During training, I observed:
The model converged quickly over just one epoch, yielding a final training loss of approximately 1.7956.
The test loss was quite similar at 1.7995, indicating that the model was not overfitting significantly on the training data and was generalizing well to unseen sequences.
The model generated text in response to initial prompts, and the variability in outputs depended on the specified temperature parameter.

2. Analysis on final train and test loss for both datasets-The training loss of 1.7956 and the test loss of 1.7995 indicate that the model performs similarly on both training and test data. This is a positive sign as it suggests a good level of generalization. If the test loss were significantly higher than the training loss, it would hint at overfitting, where the model memorizes the training data without learning to generalize.
In this scenario:
Low Training Loss: Indicates the model has learned the sequence patterns to a reasonable extent.
Similar Test Loss: Suggests that the model can predict sequences outside the data it was explicitly trained on, supporting claims of generalization.

3. Explain impact of changing temperature-The temperature parameter in sampling affects the randomness of predictions significantly:
Temperature = 1: This represents the standard case where the model samples directly from the distribution created by the softmax layer applied to logits. You will see reasonably coherent but not perfectly deterministic outputs.
Temperature > 1: A higher temperature inflates the logits, which effectively smooths the probability distribution. Consequently, this leads to more random and diverse outputs, often resulting in nonsensical or creative text (e.g., "chaebl" from "Cha"). This randomness is evident when using a temperature of 2 or 3, where outputs can include unexpected characters or patterns.
Temperature < 1: A lower temperature sharpens the probability distribution, leading to more deterministic sampling. The model tends to favor higher-probability predictions, generating sequences that are often more coherent but potentially repetitive (e.g., "stroomed" from "str" at temperature 0.5).

4. Reflection-This project has highlighted several key takeaways:
Understanding RNNs: Developing an understanding of how RNNs can model sequential data and their strengths and limitations in text generation.
Hyperparameters: The significant influence of model parameters (e.g., embedding size, hidden size, learning rate) on performance and final outputs. Experimentation with these values is critical for optimizing model performance.
Model Behavior: Observing the effect of temperature on sampling reinforces the importance of randomness in generative models and how it can be manipulated to achieve desired output characteristics.
Overall, integrating feedback loops such as temperature adjustment provides a powerful way to control the creativity and repetition in generated text, allowing for tailored outputs based on user intent.
...
5. IMPORTANT: Include screenshot of output for generate_text() function: ![alt text](image.png)

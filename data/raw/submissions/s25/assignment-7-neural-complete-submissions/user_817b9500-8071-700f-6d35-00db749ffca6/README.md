...
1. Describe your experiments and observations
    For the training of the 26 alphabets 100 times these were my hyperparameters.
    ![alt text](<Screenshot 2025-05-01 133224.png>) 
    The experiments I ran were primariliy by giving 'abc' and telling it to complete the sequence for a particular number of characters.

    For war and peace below are the hyperparameters that were used while training.
    ![alt text](image.png)
    I ran a lot of experiments for this, both an attempt to complete words and sentences.

2. Analysis on final train and test loss for both datasets
    The model was trained with an accuracy of 100% and the test loss was 0.0006 for the alphabets data. The very low loss value indicates the model easily learned its simple and repetitive structure.

    For the training data, the maximum accuracy that it reached was 53.2% with a loss of 1.59, while the testing data had an accuracy of 51.67% with a loss of 1.66. This means the higher complexity of the document led to higher losses and moderate accuracy. However, despite the higher loss, the model managed to capture reasonable language structure and vocabulary.
3. Explain impact of changing temperature
    For the alphabets data, the model gave correct output for as high as of a temperature till 2. But when tested with 4, the output generated was far more random.

    Unlike the alphabet data, a temperature higher than 1 or sometimes even 1 led to random text generation for the war and peace data file. However, for temperature lower than one, the model generated pseudo sentences which lacked meaning but often generated words that were meaningful.
4. Reflection
    Working with the alphabet dataset highlighted how effectively an RNN can learn highly structured and repetitive sequences. Since the pattern of 26 characters was completely deterministic, the model quickly achieved near-perfect accuracy with a very low loss of 0.06%.At low to moderate temperatures (up to 2), the model reliably reproduced the correct alphabetic sequence. However, increasing the temperature to 4 led to erratic outputs, showing how higher randomness can quickly break down pattern retention in simple datasets.

    Training on War and Peace was more complex compared to the alphabet dataset. Unlike the repetitive and predictable structure of the alphabet sequence, this dataset required the model to learn real language patterns, grammar, and contextual relationships. As expected, the model did not achieve high accuracy—peaking at around 53%—and the loss remained relatively high (~1.6). Despite this, the generated output often resembled grammatically structured English and occasionally formed complete words, demonstrating the model's ability to learn partial language features. I also noticed how sensitive the text generation became to the temperature parameter: even a value of 1 sometimes caused outputs to devolve into incoherence, while lower temperatures (like 0.1–0.5) resulted in more structured, albeit repetitive, sentences.

### Terminal Outputs
Image of traning the alphabet sequence
![Image of traning abcd](<Screenshot 2025-05-01 133159.png>)

Image of training war and peace
![alt text](<Screenshot 2025-05-03 142242.png>) 
![alt text](<Screenshot 2025-05-03 142328.png>) 
![alt text](<Screenshot 2025-05-03 142258.png>) 
![alt text](<Screenshot 2025-05-03 142314.png>)

These are all the experiments I ran

...
Alphabet:
The alphabet sequence performed well with the parameters set to sequence_length=30, stride=5, embedding_dim=16, hidden_size=32, learning_rate=0.01, num_epochs=8, and batch_size=32. Its train loss went from 2.5765 to 0.0080, with the test loss at 0.0047 and 100% test accuracy. It was able to reliably finish the alphabet given the letter a, or any other starting letter. I played around with the parameters and it usually completed the sequence still. When I tested smaller parameters ({10, 3, 4, 8, 0.05, 3, 16}, respectively), it was able to reproduce the alphabet sequence when starting with a, but sometimes failed to do so when starting with other letters at temperature=1.0; lowering the temperature corrected this.

War and Peace:
The war and peace dataset had mixed experimental results. With the parameters at  sequence_length=150, stride=3, embedding_dim=200, hidden_size=512, learning_rate=0.0005, num_epochs=3, batch_size=64, and data size=800K characters, the training loss went from 1.4114 to 1.0315, and I got a 1.5719 test loss with 57.02% accuracy. It took about an hour on my M1 Macbook Air. It generated many quasi-words with some real words from the war and peace text, and it had bad punctuation. For example, given the input "The war" and temperature=1.0, it generated: "the warkwere shall me an existenses the auntscrouping line inthinks into herself. drawing with a stupid, pr". Temperature=2.0 yielded more random characters, like "were some privjus.throno.as prathrown". Temperature=0.25 yielded more realistic words, like "sometimes with the proposition could not always such". Temperature=0.05 yielded repetitive common words like "she had to him and she had to him".

2. Analysis on final train and test loss for both datasets

Alphabet:
The final train loss was 0.0080 and the final test loss was 0.0047. This was using the first parameters described in question 1. The convergence to this train loss slowed down after the first 4 epochs, so maybe 8 epochs was unnecessary. The test loss being lower at 0.0047 shows that the model's training generalized very well to the 'unseen' text. After all, the alphabet sequence is effectively the same in training and testing.

War and Peace:
The final train loss was 1.0315 and the final test loss was 1.5719. This was using the parameters described in question 1. It probably could have used more epochs as it went from 1.4114 to 1.1310 to 1.0315, but I didn't want to wait longer. The test loss being higher at 1.5719 is interesting because it shows that the model learned patterns in the training data that don't generalize well to unseen text. This is to be expected considering the complexity of natural language.

3. Explain impact of changing temperature

The temperature is used to scale the logits before applying softmax. Low temperatures (<1.0) produce more deterministic results that are more likely to be real words from the corpus. Really low temperatures may produce repetitive common words. High temperatures (>1.0) produce more diverse results, including novel word combinations and grammatical errors. Really high temperatures may produce seemingly random characters. The default temperature (1.0) uses the original probability distribution from the model without modification.

4. Reflection

Implementing this character-level RNN was challenging but rewarding. I found it interesting how lean the model could get and still produce the alphabet. For the war and peace dataset, I found it interesting to see firsthand how the test loss was significantly higher than the train loss, highlighting the complexity of natural language. I was surprised that even with relatively large parameters, the model still struggled to generate completely coherent text, though it did learn character patterns that produced quasi-words. If I had more time I'd be curious to try bigger parameters with a higher temperature to see if I can get it to produce more creative yet coherent words.

5. IMPORTANT: Include screenshot of output for generate_text() function: 

#### Alphabet:
![Alphabet output](./Alphabet.png)
#### War and Peace:
![War and Peace output](./WarAndPeace.png)
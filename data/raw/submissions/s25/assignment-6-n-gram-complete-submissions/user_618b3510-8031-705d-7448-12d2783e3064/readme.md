...
For the create_frequency_tables function, we wanted to record how often every seq of length 1 to n appears in the text. For each position in the document we slide it by n and increment the substring's count, allowing us to get a list of n tables. In the calculate_probability function, we use a maximum likelihood and shorten the given sequence to at most n-1 characters. If it's empty, we divide its count by the toal number of characters. If not, we look up the joint freq of sequence + char in the length-(|sequence|+1) table and normalize the freq of sequence in the prev table. In the predict_next_char function, we iterate over all possible char in our vocabulary. We calculate P(char | sequence) and track the char with the highest probability. If all proabilities are zero, we return None.
...
First, pick the right table for calculation. Your numerator will be joint = tables[k][ sequence + char ] and for the denominator, sum over all the possible continuations. Then, return joint/denominator. 
...
1. $P(X_1=a, X_2=a, X_3=a)$
   ...
   $\frac{11}{20} \times \frac{5}{10} \times \frac{1}{4} = \frac{11}{160}$ 
   
2. $P(X_1=a, X_2=a, X_3=b)$

   $\frac{11}{20} \times \frac{5}{10} \times \frac{2}{4} = \frac{11}{80}\$
...
We iterate over the vocabulary and call the calculate_probability function to get P(c | sequence). This keeps track of which c has the highest probability and returns that character as the model's prediction. 
...
When both algorithms were run on Alice's Adventures in Wonderland and WAr and Peace for various n values, I found that for n values 1 to 3, the tables built under 1 sec for Alice and 2 secs for War and Peace. When n was 4, Alice built in around 3 secs and War and Peace took a while longer, at approx 10 seconds. At n = 5, Alice jumped to approx 10 sec and War and Peace to 30 sec. At n is 6, the table sizes get too large and the model takes too long to build. We find that larger n values can work well with longer context but fails with data sparcity and high computation cost. Looking at Alice's Adventures in Wonderland, the higher n adds little predictive power after n = 3. On the other hand, War and Peace gave good improvement at n = 4, but after it got greater than 5, it got too slow. The best values of n to use are 3 or 4.

...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        # DONE?/TODO: Initialize your model parameters as needed e.g. W_e, W_h, etc.
    ...
    def forward(self, x, hidden):
        ...
        output = []
        for t in range(l):
            #  DONE/TODO: Implement forward pass for a single RNN timestamp, append the hidden to the output 
            e_t = x_embed[t]  # [b, e]
            h_t = torch.tanh(h_t_minus_1 @ self.W_h + e_t @ self.W_e)
            output.append(h_t)
            h_t_minus_1 = h_t
            #pass
        ...
        # DONE/TODO set these values after completing the loop above
...
if len(test_loader) > 0:
    ...
    print("Test set too small — skipping test loss calculation.")
model.train()
# ===================== Text Generation =====================
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    with torch.no_grad():
        for _ in range(k):
            ...
            # Use sample_from_output to get a character index
            predicted_char_idx = sample_from_output(last_char_logits, temperature)

            # Ensure that predicted_char_idx is an integer, not a tensor
            predicted_char_idx = predicted_char_idx.item()  # Convert tensor to a scalar integer

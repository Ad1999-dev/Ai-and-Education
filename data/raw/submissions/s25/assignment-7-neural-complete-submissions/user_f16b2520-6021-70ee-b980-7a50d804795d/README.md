...
This is most easily understood with an example. Lets say your training data is the sequence "abcdefgh" and you are learning a model for `sequence_length=3`. 
...
I started out by trying to reach a zero loss model for the alphabet dataset with the most efficient compute power. I did this by keeping epoch at 1, and starting out with a learning rate of 0.1 and stride of 26, and length of 26. This made the most sense to me because the sequence repeats itself after 26 characters, so training once for each repetition would suffice. If the model learned too slowly, I would reduce the stride even further. I kept batch size at 64, and embedding_dim and hidden_size at 30.

I quickly noticed that the test_loss decreased as stride got closer to 1, but only hit exactly 0 when stride was one. This made sense because only with stride 1 was every character covered in the training.

Once stride was at 1, I could easily decrease the learning rate to very low levels.

Eventually, I hit an optimal learning rate/stride combination of:
learning_rate = 0.05
stride = 1

This was an extremely efficient training combination. After testing different characters, it was clear that this model perfectly replicates the alphabet for a *very* large character generation length.

Now, I had a very different task: training based on the war and peace model. I still started with a learning rate of 0.05, but now began with a stride of 100 and sequence length of 100. Parsing the entire length of war and peace with a stride of 1 is very inefficient and takes too long. I increased my embedding size to 200 to account for punctuation, and increased hidden features size to 200 to allow the model to derive more complex connections. I began with 10 epochs because any more took too long.

After playing around with different learning rates, it seemed like 0.05 was still the sweet spot. However, the training loss didn't improve much at all after the first epoch. I got basic gibberish as output from the model. I tried a different approach, by reducing stride to 5, increasing sequence_length to 200, and setting epoch to 1. This worked better, and I got some reasonable outputs.

2. Analysis on final train and test loss for both datasets

With the alphabet dataset, it is very easy to hit a low train_loss and a test_loss of zero. With the war and peace dataset, it's still possible to hit a low training loss easily, but hitting a low test_loss is tricky and getting zero is practically impossible. A low training loss does not necessarily correlate to a low test loss, it was important to not overtrain the model (keep it generalizable).

With my final parameters described in part 1, I was able to reach a test_loss of 2.1361 for war and peace.

3. Explain impact of changing temperature

When working with the alphabetical dataset, increasing temperature just makes some characters random instead of always using the top prediction as the next character. Then the model continues going in alphabetical order from that character. Since the dataset was so repetitive, it takes a large temperature (about 10) to get a lot of randomness.

When working with larger texts like war and peace, increasing temperature produces more random and creative outputs. This makes it so the model acts less like a sentence autocompleter for war and peace and more like a random sentence generator (sometimes gibberish, if the temperature is too high). Since it's a more diverse dataset, even slightly increasing the temperature to a value like 3 has clear visible effects.

4. Reflection

The difference between training on the alphabet dataset and war and peace was very obvious. 

The alphabet dataset only produced simple character generation in the same order as the input (on temp 1). This is because the training data was just the same alphabet repeated over and over. This also made training the model very simple: the model often reached zero loss on the training data after just one epoch. It was also important to have a low learning rate because the model needed to have very specific weight parameters (embeddings) for each character. The zero loss was expected because no matter how we split the dataset, the test set was the same characters in the same order as the test set.

The war and peace dataset produced more interesting results, but still diverged into gibberish as temperature increased.

5. IMPORTANT: Include screenshot of output for generate_text() function: 
![alt text](alphabet_generation.png)
![alt text](war_peace_generation.png)

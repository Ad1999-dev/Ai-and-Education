...
class CharDataset(Dataset):
    ...
    def __getitem__(self, idx):
        sequence = torch.tensor(self.sequences[idx], dtype=torch.long).clone().detach()
        target = torch.tensor(self.targets[idx], dtype=torch.long).clone().detach()
        return sequence, target

# ===================== Model =====================
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=128):  # Increased embedding size
        super().__init__()
        self.hidden_size = hidden_size
        self.embedding = nn.Embedding(output_size, embedding_dim)
        
        # Initialize model parameters
        self.W_xh = nn.Parameter(torch.randn(embedding_dim, hidden_size) * 0.01)  # input-to-hidden weight
        self.W_hh = nn.Parameter(torch.randn(hidden_size, hidden_size) * 0.01)    # hidden-to-hidden weight
        self.W_hy = nn.Parameter(torch.randn(hidden_size, output_size) * 0.01)    # hidden-to-output weight
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            x_t = x_embed[t]  # [b, e]
            h_t = torch.tanh(x_t @ self.W_xh + h_t_minus_1 @ self.W_hh + self.b_h)  # [b, h]
            output.append(h_t)
            h_t_minus_1 = h_t  # Update previous hidden state
...
char_to_idx = {char: idx for idx, char in enumerate(vocab)}  # Mapping from characters to indices
idx_to_char = {idx: char for idx, char in enumerate(vocab)}  # Reverse mapping from indices to characters
...
for epoch in range(num_epochs):
    ...
    for batch_inputs, batch_targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        hidden = hidden.detach()  # Detach hidden state to prevent backprop through full history
...
with torch.no_grad():
    ...
    for batch_inputs, batch_targets in tqdm(test_loader, desc="Testing"):
        ...
        output, hidden = model(batch_inputs, hidden)
        hidden = hidden.detach()  # Detach hidden state to prevent backprop
        loss = criterion(output.reshape(-1, vocab_size), batch_targets.reshape(-1))
        test_loss += loss.item()
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    with torch.no_grad():
        ...
        last_char_idx_tensor = input_tensor[:, -1:]
        
        for _ in range(k):
            output, hidden = model(last_char_idx_tensor, hidden)
            last_logits = output.squeeze(1)  # [1, vocab_size]
            sampled_idx_tensor = sample_from_output(last_logits, temperature)
            next_char = idx_to_char[sampled_idx_tensor.item()]
            generated_text += next_char
            last_char_idx_tensor = sampled_idx_tensor

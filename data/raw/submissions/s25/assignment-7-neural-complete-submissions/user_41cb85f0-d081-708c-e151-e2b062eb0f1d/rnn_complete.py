...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        self.W_h = nn.Parameter(
            torch.randn(embedding_dim + hidden_size, hidden_size) * 0.01
        )
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            h_t = torch.tanh(combined @ self.W_h + self.b_h)  
            output.append(h_t) 
            h_t_minus_1 = h_t
            #pass
        ...
        logits = self.output_layer(output) # [b, l, vocab_size=v] 
...
idx_to_char = {idx: ch for ch, idx in char_to_idx.items()} # TODO: Create the reverse mapping
data = [char_to_idx[char] for char in sequence]

# TODO Understand and adjust the hyperparameters once the code is running
#the commented out hyperparameters are those for the sequence = "abcdefghijklmnopqrstuvwxyz" * 100
sequence_length = 100 #26 # Length of each input sequence
stride =  5 #1            # Stride for creating sequences
embedding_dim = 200 #10      # Dimension of character embeddings
hidden_size = 256 #32        # Number of features in the hidden state of the RNN
learning_rate =  1e-3 #1e-3    # Learning rate for the optimizer
num_epochs = 1 #20         # Number of epochs to train
batch_size =  64 #16        # Batch size for training
...
# Test the model
def test_model(model, test_data, sequence_length, stride, batch_size):
    ...
    with torch.no_grad():  # no grads needed
        for batch_inputs, batch_targets in test_loader:
            ...
            total_characters += batch_targets.numel()

    avg_loss = total_loss / len(test_loader)
    accuracy = total_correct / total_characters

    print(f"Test set average loss: {avg_loss:.4f}")
    print(f"Test set accuracy:     {accuracy * 100:5.2f}%")


# Call the test function
test_model(model, test_data, sequence_length, stride, batch_size)
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    for _ in range(k):
        ...
        sampled_index = sample_from_output(logits, temperature)
        next_idx = sampled_index.item()  
        
        generatedChar = idx_to_char[next_idx]

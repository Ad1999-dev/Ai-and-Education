...
- ***The function create_frequency_tables constructs n-gram frequency tables by sliding a window of size n across the training document and counting occurrences of each (prefix, next character) pair. It uses a defaultdict to keep track of the frequency of every possible character that follows a given n-gram prefix. This frequency data is then used to build conditional probability tables later, allowing us to estimate the likelihood of any character following a given context.***
...
1. ***What is your vocabulary in this case***
   - The vocabulary consists of the unique characters present in the document, which are:
   - Vocabulary: `{'a', 'b', 'c'}` 
...
1. ***Write down your probability table 2***:
   - as in your probability table should look like (wait a second, you should know what I'm talking about)
        ...
        | $P(\odot)$ | Probability value |  
        | ------ | ----------------- |
        | $P(a \mid a)$ |	$\frac{5}{10} = 0.5$ |
        | $P(b \mid a)$ |	$\frac{3}{10} = 0.3$ |
        | $P(c \mid a)$ |	$\frac{2}{10} = 0.2$ |
        | $P(a \mid b)$ | $\frac{2}{4} = 0.5$ |
        | $P(c \mid b)$ |	$\frac{2}{4} = 0.5$ |
...
$x_1, x_2, \dots, x_t$
The probability of the next character $x_{t+1}$ is calculated as:
$$P(x_{t+1} \mid x_1, x_2, \dots, x_t) = \frac{P(x_1, x_2, \dots, x_t, x_{t+1})}{P(x_1, x_2, \dots, x_t)}$$

Where:
 - $P(x_1, x_2, \dots, x_t, x_{t+1})$ is the joint probability of the sequence $x_1, x_2, \dots, x_t, x_{t+1}$
 - $P(x_1, x_2, \dots, x_t)$ is the probability of the sequence $x_1, x_2, \dots, x_t$
 
This formula can be generalized for different values of `t` (the length of the sequence), using the corresponding frequency tables for n-grams.

### Code analysis

- ***This function uses n-gram frequency tables to calculate the joint probability of a new character appearing after a sequence. It divides the frequency of the extended sequence $(x_1, ..., x_t, x_{t+1})$ by the frequency of the sequence alone $(x_1, ..., x_t)$ to get the conditional probability.***
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - = $P(X_3=a | X_1=a, X_2=a) = \frac{2}{5}$
2. $P(X_1=a, X_2=a, X_3=b)$
   - = $P(X_3=b | X_1=a, X_2=a) = \frac{2}{5}$
3. $P(X_1=a, X_2=a, X_3=c)$
   - = $P(X_3=c | X_1=a, X_2=a) = \frac{1}{5}$
...
- ***The `predict_next_char(sequence, tables, vocabulary)` function predicts the next character in a given sequence based on the calculated probabilities for each possible character in the vocabulary. It returns the character with the highest probability, thus allowing for probabilistic completion of sequences. The function iterates over the vocabulary and computes the probability of each character occurring next given the input sequence. It selects the character with the highest probability using the trigram (or appropriate n-gram) frequency table.***
...
- Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?
  - Next possible characters after `aa` and their associated probabilities:
      - For `char = a`: 2/5
      - For `char = b`: 2/5
      - For `char = c`: 1/5

Thus, the most likely next character in the sequence `aa` would be either `a` or `b`.
...
- I experimented with two different corpora: Alice's Adventures in Wonderland and War and Peace. I tested multiple values of n in the n-gram model, ranging from unigrams (n=1) to 6-grams (n=6). I found that Alice's Adventures in Wonderland, being smaller and more linguistically playful, worked well with lower values of n such as 2 or 3. It provided reasonably accurate next-character predictions without significant computation time. In contrast, War and Peace, being a much longer and more complex text, performed better with higher n values (n=4 or n=5), where the model could take better advantage of the larger context. However, beyond n=5, the table calculation became increasingly time-consuming—especially for War and Peace, due to the exponential growth in the number of unique n-grams. Ultimately, I found that Alice’s Adventures was more suitable for quick testing and faster iteration, while War and Peace offered richer language patterns but required more processing time and memory as n increased.

...
I did a experiments using two different datasets: alphabet sequence and War and Peace text. For the alphabet sequence, I decided to use a smaller model with 
...
The model was trained quickly and I make sure to edit the size and check for percentage of the loss. This allow the model to run perfect. 

For War and Peace file, I decided to increase the model capacity with 
...
Apparently, the War and Peace required more computational resources and display lower accuracy due to its complexity.

2. Analysis on final train and test loss for both datasets

For the alphabet sequence, the model achieved almost perfect performance with a final training loss of approximately 0.9913 with 89.65% and test loss of 0.0000, with 100% accuracy. This is expected since the alphabet sequence is deterministic and has a simple pattern. For War and Peace, the model achieved a training loss of approximately 1.5895 with 53.45% and test loss of 1.8707 with 46% accuracy. The higher loss and lower accuracy for War and Peace shows the complexity and variability of natural language compared to the simple alphabet sequence.

War and Peace

Epoch 1, Loss: 1.5895, Accuracy: 53.45%
Test Loss: 1.8707, Accuracy: 46.00%

Alphabet

Epoch 1, Loss: 0.9913, Accuracy: 89.65%
Test Loss: 0.0048, Accuracy: 100.00%

3. Explain impact of changing temperature

The temperature parameter affects text generation. With a low temperature (e.g., 0.1), the model becomes more deterministic, consistently selecting the most probable next character. This results in more predictable and repetitive text. 

With a high temperature (e.g., 2.0), the model becomes more random, giving lower probability characters a better chance of being selected. This produces more diverse but with less coherent text. For the alphabet sequence, a low temperature can produce accurate alphabetical order, while a high temperature might introduce occasional deviations. For War and Peace, a low temperature can produce common English words and patterns, while a high temperature creates more unusual character combinations.

4. Reflection

Implementing this character-level RNN can give insights into sequence modeling. The most challenging aspects were implementing the forward pass with correct tensor dimensions and tuning hyperparameters for different datasets. I learned that RNNs can capture patterns in sequential data, but their performance varies depending on the complexity of the patterns. The alphabet sequence was easily learned  because it is simple. In contrast, natural language from War and Peace has complex dependencies that are harder to capture. This project demonstrated how RNNs maintain state across sequence elements, allowing them to learn context-dependent patterns. I also gained experience with embedding layers, hidden state propagation, and temperature-controlled sampling for text generation.

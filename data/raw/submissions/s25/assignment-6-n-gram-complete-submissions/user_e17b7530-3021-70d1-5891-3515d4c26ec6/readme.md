...
- first I create a list of counters that will eventually contain the frequencies of all the substrings found in the document. I then iterate through the document, starting with the smallest ngram, and counting the occurance of each substring of size n in the list i created in the first line. I then return the list once the loop has completed and every substring of length 1 to n has been counted. 
...
1. ***Write down your probability table 2***:
   - as in your probability table should look like (wait a second, you should know what I'm talking about)
        ...
        | $P(b \mid a)$ | $\frac{2}{4}$ |
        | $P(c \mid a)$ | $\frac{3}{5}$ |
       -I accidentally did way more work than I had to so im not getting rid of it, enjoy. 
        | $P(\odot)$ | Probability value |  
        | ------ | ----------------- |
        | $P(b \mid a)$ | $\frac{2}{4}$ |
        | $P(a \mid b)$ | $\frac{3}{11}$ |
...
### Formula
- $$P(X_n = x_n \mid X_1 = x_1, X_2 = x_2, X_3 = x_3,... X_{n-1}=x_{n-1})   $$
- $$=P(x_1) \cdot P(x_2 \mid x_1) \cdot P(x_3 \mid x_1, x_2) \cdot P(x_4 \mid x_1, x_2, x_3)\cdot ... \cdot P(x_n \mid x_1, x_2, x_3,...,x_{n-1})$$
-$$=\frac{f(x_1, x_2, x_3, x_4,...,x_n)}{f(x_1, x_2, x_3,...,x_{n-1})}$$
### Code analysis

- Basically my code grabs the frequency of the new sequence (sequence plus char) and the frequency of the original one and finds the probability by dividing the first by the second. 
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $P(X_1=a)*P(X_2 = a | X_1=a)*P(X_3 = a | X_2 = a, X_1=a) = \frac{11}{20}*\frac{5}{11}*\frac{1}{5}=1/20$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $P(X_1=a)*P(X_2 = a | X_1=a)*P(X_3 = b | X_2 = a, X_1=a) = \frac{11}{20}*\frac{5}{11}*\frac{2}{5}=1/10$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $P(X_1=a)*P(X_2 = a | X_1=a)*P(X_3 = c | X_2 = a, X_1=a) = \frac{11}{20}*\frac{5}{11}*\frac{1}{5}=1/20$
...
- This method is super simple, I just loop through the vocabulary, at each character calculating the probability that that is the next character while tracking the one with the highest prob. once the loop is done I return that and thats all. (if no char in the vocab is found in the document following the sequence, I just return the first vocab entry)
...
<hr> The alice in wonderland file works significantly better than the war and peace one. This is not really a shocker as the war and peace file contains way way way more text that the program has to parse through. If i had started the project earlier I would have liked to experiment with some kind of organizational system to streamline processing because the way my program works as of now is by just going through all the tables and calculating the probability of each letter in the vocabulary for every new prediction which is really computationally heavy. In my experiments I found that with alice in wonderland I could compute up to about an n value of 500 in under a minute. Contrastingly, with war and peace, my computer struggled getting past a meager 20. 

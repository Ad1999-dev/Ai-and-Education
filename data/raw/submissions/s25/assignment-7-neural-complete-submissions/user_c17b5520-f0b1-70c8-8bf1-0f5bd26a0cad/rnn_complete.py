...
class CharRNN(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        logits = self.output_proj(output) + self.b_o  # [b, l, vocab_size]
...
char_to_idx = {char: i for i, char in enumerate(vocab)} # Create a mapping from characters to indices
idx_to_char = {i: char for i, char in enumerate(vocab)} # Create the reverse mapping
...
train_data = data_tensor[:train_size] # Split the data into 90:10 ratio with PyTorch indexing
...
for epoch in range(num_epochs):
    ...
    for batch_inputs, batch_targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        logits = output.reshape(-1, output.size(-1)) # Flatten the output to [batch_size * seq_len, vocab_size]
        targets = batch_targets.view(-1) # Flatten the targets to [batch_size * seq_len]
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    with torch.no_grad():
        for i in range(len(input_tensor[0])):
            input_char = input_tensor[:, i].unsqueeze(1) # Shape [1, 1]
            output, hidden = model(input_char, hidden)

    generated = start_text
    current_input = input_tensor[:, -1:].to(device) # Shape [1, 1]
    ...
    for _ in range(k):
        with torch.no_grad():
            ...
            generated += next_char
            current_input = sampled_idx.unsqueeze(0).to(device) # Shape [1, 1]
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    with torch.no_grad():
        for i in range(len(input_tensor[0])):
            input_char = input_tensor[:, i].view(1, 1) # Shape [1, 1]
            output, hidden = model(input_char, hidden)
    ...
    for _ in range(k):
        with torch.no_grad():
            ...
            next_char = idx_to_char[sampled_idx.item()]
            generated += next_char
            current_input = sampled_idx.view(1, 1).to(device) # Shape [1, 1]

...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    input_idxs = torch.tensor([char_to_idx[c] for c in start_text], dtype=torch.long).unsqueeze(0).to(device)
    model.eval()
    hidden = None
    generated = start_text
    with torch.no_grad():
        for _ in range(k):
            output, hidden = model(input_idxs, hidden)
            last_logits = output[:, -1, :]
            next_idx = sample_from_output(last_logits, temperature).item()
            generated += idx_to_char[next_idx]
            input_idxs = torch.cat([input_idxs[:, 1:], torch.tensor([[next_idx]]).to(device)], dim=1)
    return generated

...
- **Using the alphabet** 
  - The accuracy and the loss is not always exactly the same, though it seems to always be above 95% accuracy and has very little loss. Occasionally, I'll get 1.0 as the accuracy. The mode accuracy seems to be about 99% after running it many times. Here is my generated text from the alphabet:
  ![alt text](image-2.png)
- **Using War and Peace**
- It takes a lot longer to train using War and Peace and also for some reason the output is all nans for the logits, which I still have to debug. The accuracy is also a lot lower than the letters.

2. Analysis on final train and test loss for both datasets
- **Using the alphabet** With the following hyperparameters
```
...
embedding_dim = 5      # Dimension of character embeddings
...
```
I get an accuracy and average loss of the following:
![alt text](image-1.png)
- **Using War and Peace** With the following hyperparameters
```
sequence_length = 100 # Length of each input sequence
...
I got an accuracy of about 0.16. With more tuning of hyperparameters, I believe I could get higher. 

3. Explain impact of changing temperature \
Increasing the temperature increases the randomness of the output, allowing letters that are less likely to be sampled to be sampled.
4. Reflection \
I think that I should have alloted more time to tuning the hyperparameters for the war and peace text generation because there are outputs that are nans for the logits and therefore text generation does not work as expected. However, the code works for the alphabet sequence and does not run into any errors.
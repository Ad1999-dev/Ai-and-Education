...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        self.W_x = nn.Parameter(torch.randn(embedding_dim, hidden_size) * 0.01)
        self.b_h = nn.Parameter(torch.zeros(hidden_size))
        self.W_y = nn.Parameter(torch.randn(hidden_size, output_size) * 0.01)
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            x_t = x_embed[t] # [b, e]
            h_t = torch.tanh(x_t @ self.W_x + h_t_minus_1 @ self.W_h + self.b_h) # [b, h]
            output.append(h_t)
            h_t_minus_1 = h_t # Update hidden state for next step
            # pass
        output = torch.stack(output) # [l, b, h]
        output = output.transpose(0, 1) # [b, l, h]
        
        # TODO set these values after completing the loop above
        final_hidden = h_t_minus_1.clone() # [b, h] # Use the last computed hidden state
        logits = output @ self.W_y + self.b_y # [b, l, v] 
...
for epoch in range(num_epochs):
    ...
    for batch_inputs, batch_targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        optimizer.zero_grad()
        loss = criterion(output.reshape(-1, output_size), batch_targets.view(-1))
        loss.backward()
        optimizer.step()
...
with torch.no_grad(): # Disable gradient calculation for testing
    for batch_inputs, batch_targets in tqdm(test_loader, desc="Testing"):
        batch_inputs, batch_targets = batch_inputs.to(device), batch_targets.to(device)
        output, hidden_test = model(batch_inputs, hidden_test)
        ...
        test_total += batch_targets.numel()
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    input_indices = torch.tensor([char_to_idx[char] for char in start_text], dtype=torch.long).unsqueeze(0).to(device) # [1, n]
    
    hidden = None
    # First pass with the initial start_text to get the hidden state
    with torch.no_grad():
        output_logits, hidden = model(input_indices, hidden)
    
    # Get the last character's logits and sample the next character
    last_logits = output_logits[:, -1, :] # [1, vocab_size]
    next_char_idx = sample_from_output(last_logits, temperature).squeeze() # [1]
    
    # Append the first generated character
    generated_text.append(idx_to_char[next_char_idx.item()])
    
    # Update the input for the next steps
    current_input_idx = next_char_idx.view(1, 1) # Reshape to [1, 1]

    # Generate k-1 more characters
    for _ in range(k - 1):
        with torch.no_grad():
            output_logits, hidden = model(current_input_idx, hidden)
            hidden = hidden.detach()
        
        # Sample the next character
        next_char_idx = sample_from_output(output_logits[:, -1, :], temperature).squeeze()

...
To build n frequency tables for n-gram language model, it will first iterating through the document and counting sequences of characters followed by their respective subsequent characters. After collecting all of these frequency counts, I normalize them by dividing each by how many times in total their respective context occurs to produce conditional probabilities. 
...
The code choose the appropriate frequency table based on the sequence length and looks up the probability of the character occurring after that sequence. When encountering cases where the sequence or char is missing from the table, it will return 0, thus effectively handing edge cases 
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - Chain rule: P(a) · P(a|a) · P(a|aa)
   - P(a) = $\frac{11}{20}$
   - P(a|a) = $\frac{6}{11}$
   - P(a|aa) = $\frac{2}{6}$
   - Thus: P(X_1=a, X_2=a, X_3=a) = $\frac{11}{20} \cdot \frac{6}{11} \cdot \frac{2}{6} = \frac{11 \cdot 6 \cdot 2}{20 \cdot 11 \cdot 6} = \frac{132}{1320} = \frac{1}{10} = 0.1$

2. $P(X_1=a, X_2=a, X_3=b)$
   - Chain rule: P(a) · P(a|a) · P(b|aa)
   - P(a) = $\frac{11}{20}$
   - P(a|a) = $\frac{6}{11}$
   - P(b|aa) = $\frac{2}{6}$
   - Thus: P(X_1=a, X_2=a, X_3=b) = $\frac{11}{20} \cdot \frac{6}{11} \cdot \frac{2}{6} = \frac{11 \cdot 6 \cdot 2}{20 \cdot 11 \cdot 6} = \frac{132}{1320} = \frac{1}{10} = 0.1$

3. $P(X_1=a, X_2=a, X_3=c)$
   - Chain rule: P(a) · P(a|a) · P(c|aa)
   - P(a) = $\frac{11}{20}$
   - P(a|a) = $\frac{6}{11}$
   - P(c|aa) = $\frac{2}{6}$
   - Thus: P(X_1=a, X_2=a, X_3=c) = $\frac{11}{20} \cdot \frac{6}{11} \cdot \frac{2}{6} = \frac{11 \cdot 6 \cdot 2}{20 \cdot 11 \cdot 6} = \frac{132}{1320} = \frac{1}{10} = 0.1$
...
This code goes through every character in the vocabulary and determines the character with the highest probability of following the given sequence. If no character has a non-zero probability, it falls back to the most frequently occurring character from the unigram table for reliability. 
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  -  The calculations from above for sequence "aa" shows that the probabilities for the next character are:
    - P(a|aa) = $\frac{2}{6} = \frac{1}{3}$
    - P(b|aa) = $\frac{2}{6} = \frac{1}{3}$
    - P(c|aa) = $\frac{2}{6} = \frac{1}{3}$
  - They all have equal probability of $\frac{1}{3}$, so 'a', 'b', or 'c' could be the next character.
...
After experimenting with both file, I notcied that the "War and Peace" text work better than "Alice's Adventure.." text. When testing the code, I noticed that n = 3 or 4 provides the accuracy than other value. For instance, n = 100 with "Alic" returns "Alics, Alicss, Alicss" while n = 4 returns "Alice, Alice, Alice t, Alice th, Alice the". I also noticed that n = 1 returns prediction that were either too simple or didn't even capture the word patterns. N = 2 returns better prediction but duplicate the word in the end like "Alice the --> Alice the the. N = 5 or more gave decreasing returns in prediction quality while exponentially increasing computation time and memory usage. Anything greater than 5 returns inaccurate prediction resulting in overfitting and poor generalization to new sequences. 
<hr>

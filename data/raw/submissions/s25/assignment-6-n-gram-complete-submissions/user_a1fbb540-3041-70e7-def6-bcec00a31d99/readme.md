...
- The code iterates through each character then the document, and for each length 1-n adds to or creates an entry in the frequency table. tTe table is a dictionary whose key is the name of the entry and value is the frequency.
...
- The code adds the next char to the sequence to create the new sequence which can then be searched for within the corresponding probability table. If the sequence does not exist within our probability tables, it is assigned a probability of 0.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $\frac{1}{18}$ using the table
2. $P(X_1=a, X_2=a, X_3=b)$
   - $\frac{2}{18}$ using the table
...
- The code predicts the next character by predicting the probability of each character in the vocabulary and returning the character with the greatest probability.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - The next character should be B, because it has the highest probability of occuring after 'a' and 'a'.

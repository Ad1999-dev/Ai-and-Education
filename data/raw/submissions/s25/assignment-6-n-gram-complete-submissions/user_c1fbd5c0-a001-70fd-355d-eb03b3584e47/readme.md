...
The algorithm first looks to see if no grams are being considered, returning an empty frequency table if so. The algorithm then considers a base-case for unigrams, counting how many times each character from the vocabulary appears. The empty string is used to signify that there is no context for a unigram. For each k from bigrams to n-grams, the algorithm takes every subset of k-length and extracts the prefix as context and the last char in the sequence as the target. If the target is no in the k-gram table, it is added. If the target doesn't have the prefix in the table, the prefix is added to the target, otherwise the frequency for that prefix for the character is incremented. Finally, all the tables are appended so that frequency_tables holds unigrams to n-grams in increasing order.
...
1. ***What is your vocabulary in this case***
   - The vocabulary is the three letters: a, b, and c.
...
$$P(X_1=x_1, X_2=x_2,...,X_k=x_k) = P(x_1) \cdot P(x_2 \mid x_1) \cdot P(x_3 \mid x_1,x_2) \cdot ... \cdot P(x_{n-1} \mid x_1,x_2..x_{n-2}) \cdot \prod_{i=n}^{k}P(x_i \mid x_{i-n+1}...x_{i-1})$$

### Code analysis

- ***Put the intuition of your code here***

The algorithm loops through the sequence plus the character that follows it. For each iteration, it considers the previous
k-1 characters as context for the current kth character. It then finds the appropriate frequency table for the current
kth character and calculates the probability for that character using the context, multiplying that probability with the
accumulated, total probability for the sequence. If the target character isn't found in the frequency table or the 
prefix isn't ever present for the target character, then the probability is 0 and that is returned.
...
The algorithm finds the probability of each character being next in the sequence and then takes the character with the highest probability as the next character.
...
Based on the distribution above, the next character in the sequence would be b.
...
For both texts, increasing the n-gram count was neccessary to avoid putting the text in a loop of repeating the same word/words. However, the value of n that worked for the texts differed. For Alice's Adventures in Wonderland, around n=75 was the most effective, generating with only a slight delay, and n=100 still worked well. On the other hand, for War and Peace, n could only reach around 15 before performance degraded rappidly. This is likely because of the difference in size between the texts - Alice in Wonderland at it's longest is around 320 pages, meanwhile War and Peace totals around 1,400. The much longer text is going to neccessarily require a lot more time to calculate and create frequency tables, which will affect the performance.

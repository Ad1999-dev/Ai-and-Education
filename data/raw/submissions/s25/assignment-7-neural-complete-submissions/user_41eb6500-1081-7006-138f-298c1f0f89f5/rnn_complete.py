...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embedding_dim=30):
        ...
        # Renamed model parameters to match lecture notation:
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            x_t = x_embed[t]
            h_t = torch.tanh(x_t @ self.W_e + self.b_e + h_t_minus_1 @ self.W_h + self.b_h)
            output.append(h_t)
            h_t_minus_1 = h_t
            pass
        ...
        logits = output @ self.W_o + self.b_o
...
char_to_idx = {ch: i for i, ch in enumerate(vocab)}  # Map each character to a unique index
idx_to_char = {i: ch for ch, i in char_to_idx.items()}  # Reverse mapping for decoding

data = [char_to_idx[char] for char in sequence]

# TODO Understand and adjust the hyperparameters once the code is running
sequence_length = 75 # Length of each input sequence
...
test_data = data_tensor[train_size:]  # Remaining 10% for testing
...
with torch.no_grad():
    for batch_inputs, batch_targets in test_loader:
        ...
        correct_preds += (predictions == batch_targets).sum().item()
        num_preds += batch_targets.numel()
print(f"Test Loss: {total_test_loss/len(test_loader):.4f}")
print(f"Test Accuracy: {100 * correct_preds / num_preds:.2f}%")
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    input_sequence = [char_to_idx[c] for c in start_text[-n:].lower()]  # Encode start_text
    input_tensor = torch.tensor(input_sequence, dtype=torch.long).unsqueeze(0).to(device)
    hidden = None

    with torch.no_grad():
        for i in range(n - input_tensor.shape[1]):
            _, hidden = model(input_tensor, hidden)

        generated = start_text
        for _ in range(k):
            logits, hidden = model(input_tensor, hidden)
            last_logits = logits[:, -1, :]  # Get last timestep's output
            next_idx = sample_from_output(last_logits, temperature).item()
            next_char = idx_to_char[next_idx]
            generated += next_char
            input_tensor = torch.cat([input_tensor[:, 1:], torch.tensor([[next_idx]], dtype=torch.long).to(device)], dim=1)

...
class CharRNN(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        logits = output @ self.W_hy + self.b_y  # [b, l, v]
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    input_indices = [char_to_idx[c] for c in start_text[-n:]]
    input_tensor = torch.tensor(input_indices, dtype=torch.long).unsqueeze(0).to(device)  # [1, n]
    hidden = None
    ...
    for _ in range(k):
        ...
        sampled_idx = sample_from_output(last_logits, temperature=temperature).item()
        next_char = idx_to_char[sampled_idx]
        generated += next_char

        input_indices.append(sampled_idx)
        input_indices = input_indices[-n:]  # keep only last n

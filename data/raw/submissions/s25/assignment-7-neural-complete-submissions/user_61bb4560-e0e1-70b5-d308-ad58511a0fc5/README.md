...
In this assignment, I implemented a character-level recurrent neural network (RNN) using PyTorch to predict the next character in sequences. I started with a simple dataset consisting of the English alphabet repeated 100 times. Over the course of 10 epochs, I observed the loss values remained relatively stable, indicating that the model may have struggled to converge effectively on this simple dataset. The training losses for the individual epochs were as follows:
Epoch 1 Loss: 3.2582
Epoch 2 Loss: 3.2582
Epoch 3 Loss: 3.2582
Epoch 4 Loss: 3.2581
...
Epoch 8 Loss: 3.2584
Epoch 9 Loss: 3.2581
Epoch 10 Loss: 3.2580
After training, I evaluated the performance of the model on the test dataset, yielding a Test Loss of 3.2498. These losses indicate the RNN was able to learn some patterns, but likely encountered issues with convergence and stability.
2. Analysis on final train and test loss for both datasets
The training loss showed slight variations but did not significantly decrease across epochs, suggesting that the model was not learning effectively from the dataset. The final training loss for the alphabet dataset was approximately 3.2580, while the test loss was slightly lower at 3.2498. This indicates a relatively small gap between training and test loss, but overall high loss values suggest that the model had not fully learned the character sequence patterns.

| Dataset  | Final Training Loss | Final Test Loss |
|----------|---------------------|-----------------|
| Alphabet | 3.2580              | 3.2498          |

3. Explain impact of changing temperature
The temperature parameter was a crucial hyperparameter that influenced the text generation results. When generating text at a lower temperature (e.g., 0.5), the model produced more predictable and coherent outputs. For example, starting with the input "abc", the generated text was:abcdipqbwwhzbitzdwsnxrpekbzxcyccadumogypojlbkgqdnwqqmgsdggavldivaqvigwfwenvkjfjwvabyswronjpnxqzxzxuuadf
This output maintains some resemblance to the alphabetic structure.
In contrast, increasing the temperature to 1.5 led to more diverse and unpredictable outputs:abcsehiopsxmplbemefnmhvayptpriuhojhqgoyxbusmgzjvutgapnfgtpdaplvfwkyjmchggdttfbvxtmehunfkvguywvuoqcdmsbr
The higher temperature allowed for greater creativity but resulted in text that often diverged from recognizable patterns typical of the alphabet.
4. Reflection
This assignment provided valuable insights into the workings of recurrent neural networks and the complexities associated with training them on different datasets. The transition from a structured but simplistic dataset to a more complex format (like natural language) highlighted the nuances in language modeling. I learned the importance of hyperparameter tuning, especially regarding learning rates and temperatures in text generation, as these significantly impacted the results. I realized that although the simpler dataset was easier for learning patterns, generating coherent text remained a challenge even with observed slight improvements in losses throughout training.

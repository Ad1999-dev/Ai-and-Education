...
class CharRNN(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        for t in range(l):
            ...
            x_t = x_embed[t]
            h_t = torch.tanh(
                x_t.matmul(self.W_xh) +
                h_t_minus_1.matmul(self.W_hh) + self.b_h
            )  
            output.append(h_t)
            h_t_minus_1 = h_t
        ...
        logits = output.matmul(self.W_hy) + self.b_y     
...
def generate_text(model, start_text, n, k, temperature=1.0):
    ...
    logits, hidden = model(input_tensor, None)
    for _ in range(k):
        last_logits = logits[:, -1, :]                   # [1, vocab_size]
        next_idx = sample_from_output(last_logits, temperature)  # [1,1]
        idx_item = next_idx.item()
        generated += idx_to_char[idx_item]

        # Feed the sampled index back into the model
